...
/**
 * ORÁCULO de persistencia server-side de la CARAVANA ELECTRÓNICA de los TERNEROS de un parto (delta
 * bastoneo-captura-alta-parto, RCF.6 generalizado al parto). Pollea vía service_role hasta que el conjunto de
 * `animals.tag_electronic` de los terneros de la madre CONTENGA todas las `expectedTags` esperadas. Cadena:
 * reproductive_events (birth de la madre) → birth_calves.calf_profile_id → animal_profiles.animal_id →
 * animals.tag_electronic. Prueba que el `tagRaw` capturado por el bastoneo del CalfBlock llegó a
 * `registerBirth({ calves:[{ tag }] })` → RPC register_birth → animals.tag_electronic (índice unique global).
 * `birth_calves` es server-only (sin GRANT de INSERT) → un ternero con esa tag SOLO existe si la RPC corrió.
 */
export async function waitForServerCalfTags(
  motherProfileId: string,
  expectedTags: string[],
  opts: { tries?: number; delayMs?: number } = {},
): Promise<string[]> {
  const tries = opts.tries ?? 30;
  const delayMs = opts.delayMs ?? 2000;
  const want = new Set(expectedTags);
  let lastSeen: string[] = [];
  for (let i = 0; i < tries; i++) {
    ...
    const { data: events, error: evErr } = await admin
      ...
      .eq('event_type', 'birth')
      .is('deleted_at', null);
    if (evErr) throw new Error(`waitForServerCalfTags events: ${evErr.message}`);
    const eventIds = (events ?? []).map((e) => e.id as string);
    if (eventIds.length > 0) {
      ...
      const { data: bridge, error: bErr } = await admin
        ...
        .in('birth_event_id', eventIds);
      if (bErr) throw new Error(`waitForServerCalfTags birth_calves: ${bErr.message}`);
      const calfProfileIds = (bridge ?? []).map((b) => b.calf_profile_id as string);
      if (calfProfileIds.length > 0) {
        // 3) tag_electronic de los terneros (via animal_profiles.animal_id → animals).
        const { data: profiles, error: pErr } = await admin
          .from('animal_profiles')
          .select('animals(tag_electronic)')
          .in('id', calfProfileIds);
        if (pErr) throw new Error(`waitForServerCalfTags profiles: ${pErr.message}`);
        lastSeen = (profiles ?? [])
          .map((p) => (p as { animals: { tag_electronic: string | null } | null }).animals?.tag_electronic ?? null)
  ...
  throw new Error(
    `waitForServerCalfTags(${motherProfileId}, [${[...want].join(', ')}]): las caravanas de los terneros NUNCA ` +
      `aparecieron completas en el server (${tries} intentos; última lectura: [${lastSeen.join(', ')}]). El ` +
      `tag capturado por el bastoneo no llegó a register_birth / la outbox no drenó.`,
  );
}
