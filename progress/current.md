...
**PENDIENTE: Puerta 1 humana (Raf)** con 3 decisiones: **D1** `.xlsx` fuera de MVP (recom. sí), **D2** inserts directos/Escenario A vs RPC bulk/B (recom. A — Gate 1 coincide), **D3** placeholder de categoría "a completar" (por sexo override=false vs code neutro; roza dominio→Facundo). Aprobada la Puerta 1 → implementer (migración `import_log` ≥0073 + utils puros + service + UI) → reviewer → Gate 2 → puerta de código. PowerSync (siguiente literal del plan) bloqueado por infra de Raf.
