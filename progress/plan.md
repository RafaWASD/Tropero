...
| 03-modo-maniobras | ✅ done | ✅ done (2026-06-19, Puerta 2) | feature CORE **cerrada**. Backend 0050-0057 + M5-BACKEND 0093-0097 + M6-BACKEND 0098-0100. Cliente completo M1-M6 (wizard/identify-first/10 maniobras/offline/datos custom/circunferencia escrotal). Pendiente no-bloqueante: sync nativo (15) + bastón spp-android (04), gateados por hardware |
| 04-bluetooth-baston | 🟡 spec_ready (v2 ADR-024) | ❌ pending | **Transporte RESUELTO (ADR-024, s22)**: contrato de ingesta transport-agnóstico + 5 adaptadores (RS420 = Classic SPP+MFi, NO BLE — supuesto BLE de ADR-002 refutado en el día de campo). Spec v2 + Gate 1 PASS + Puerta 1 aprobada (2026-06-03). Pendiente IMPLEMENTAR. Bloqueante para BUSCAR ANIMAL + MODO MANIOBRAS |
...
| 07-reportes-basicos | ✅ done | ✅ done (2026-06-24, Stream C) | **STREAM C del modelo reproductivo, CERRADA.** Backend `0106` (9 RPC SECURITY DEFINER, aplicada) + frontend (4 KPIs: resumen sesión, %preñez, %parición=paridas/servidas base servidas, peso prom/categoría + comparativa; distribución CCL cabeza/cuerpo/cola tacto-vs-nacimientos; 2 alertas dosis-vencida/sin-pesar). Online-only server-side. Reviewer + Gate 1/2 + diseño aprobado por Raf. Limitación: R7.2.3 cache-offline (opcional, no impl). POST-MVP: razas agronómicas, IEP/tendencias multi-año, mortandad fina, kg/ha |
...
- **Implementar:** ~~02 backend~~ ✅ → ~~01 fe~~ ✅ (B.1, sesión 21-22) → **02 fe (+ puerta manual find-or-create de 09)** → PowerSync → [04 bastón · import masivo] → 09 resto → 05 → 03 → 08 → 06 → 07. *(Reordenado sesión 22: el alta de animal = find-or-create, NO un form pelado; se adelanta la puerta MANUAL de 09 al chunk de 02 fe. Ver changelog + `specs/active/02-modelo-animal/context-frontend.md`.)*
...
- **Estado**: ✅ **`done` (sesiones 21-22)** — Fases 3-6 implementadas, gateadas (reviewer + Gate 2) y commiteadas; Raf probó en web. Auth + establecimientos + invitaciones/miembros + perfil (editar nombre/tel/email + eliminar cuenta vía edge `delete_account`/RPC `0058`). Saludo consolidado (ProfileContext). Suite Playwright E2E (13 tests verdes) cubre los flujos. **Fases 7 (PowerSync) y 8 (E2E Detox nativo) diferidas** (PowerSync se hace en el chunk de datos de campo; Detox device-blocked). Commits: `1893513`/`4686ac0`/`4728a7b`/`1a5d6aa`/`876614a`/`063ab79`/`736656c`/`c30c8e3`/`acf1d3d`.
- **Dueño**: `implementer` → `reviewer`.
...
### B.3 — Implementación de spec 04 (transporte del bastón) — supuesto BLE INVALIDADO (s20) → RESUELTO por ADR-024
- **Estado**: `spec_ready` (2026-06-06). **Bloqueante levantado**: el transporte se resolvió por **ADR-024** (s22) — contrato de ingesta de EID transport-agnóstico + 5 adaptadores. La spec 04 se reescribió a **v2** (context/requirements/design/tasks reconciliados a ADR-024, 15 reqs), **Gate 1 (security) PASS + Puerta 1 aprobada por Raf (2026-06-03)**, committeada (`f3e30a4`). **Pendiente: IMPLEMENTAR** (no arrancado) — orden: buildable-hoy (contrato + web-serial + manual + mock + provider/hooks) → dev build Android (spp-android) → HID tras el gate físico (ADR-024 §4).
...
- **Estado**: ✅ **`done` (2026-06-19, Puerta 2 código aprobada por Raf)**. Feature CORE cerrada: backend (0050-0057) + M5-BACKEND (datos/maniobras custom, 0093-0097) + M6-BACKEND (circunferencia escrotal, 0098-0100) + cliente completo M1-M6 (wizard de jornada · identify-first BLE/manual/find-or-create + carga rápida · 10 maniobras MVP · offline/reanudación/surfacing de rechazos · datos/maniobras custom por campo · circunferencia escrotal con rueda inercial). Todo gateado por chunk (veto design-review + reviewer + Gate 2). Pendiente NO-bloqueante (no reabre la feature): sync nativo de tablas de maniobra (spec 15-nativo) + bastón spp-android real (spec 04) — gateados por el dev build Android de Raf.
- **Dueño**: `spec_author` → Raf aprueba → `implementer` → `reviewer`.
- **Dependencias**: B.2 (modelo animal), B.3 (bastón — capa buildable-hoy + mock), C.1 (balanza — quedó FUERA del MVP, peso manual), B.4 (BUSCAR ANIMAL — find-or-create compartido).
- **Notas**: feature CORE. El research de Auravant (sesión 9) lo identificó como molde funcional. El cliente se construyó JIT (sesión 26 → 2026-06-19) reconciliado contra el as-built de 02/04/09/10/11/15.
...
- **2026-06-01 (sesión 22 — cierre B.1 + reordenamiento del frontend de campo)** — Cerrado el frontend de spec 01 (B.1: Fases 3-6 done, 7/8 diferidas) + suite Playwright E2E. Raf eligió arrancar el frontend del core ganadero apuntando a MODO MANIOBRAS. **Hallazgo (leader)**: MODO MANIOBRAS es la cima de un stack sin construir (`app/src/features/` vacío) — depende de spec 02-cliente + 09 + PowerSync + bastón (04, bloqueado ADR-024). **Decisión de Raf (reordenamiento)**: arrancar por la base = **frontend de spec 02** (crear rodeo/animal/eventos/ficha) → **PowerSync** → capas (04 bastón · import masivo) → resto de 09 → **03 MODO MANIOBRAS** al final. **Decisión de diseño clave (Gate 0, Mobbin + principios)**: "crear animal" = **find-or-create** (identificar→crear si no existe), NO un form pelado (que crea duplicados + no matchea el modelo mental); el caso masivo (usuario nuevo con rodeo entero) = **importar** (feature aparte, roza 08 SIGSA/RENSPA). → **se adelanta la puerta MANUAL de find-or-create de spec 09** al chunk de 02 fe (la puerta BLE + dedup avanzado + import quedan después). Gate 0 del frontend de 02 documentado en `specs/active/02-modelo-animal/context-frontend.md` (02 nunca tuvo context.md). Decomposición: C1 Rodeos (wizard crear rodeo) → C2 alta find-or-create manual + lista → C3 ficha + eventos → C4 lotes → C5 PowerSync. Mitigación de retrofit: services swappables para que el swap a PowerSync sea localizado.
