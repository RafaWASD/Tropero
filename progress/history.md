...
## 2026-06-19 — spec 03 MODO MANIOBRAS — cliente COMPLETO → feature CORE `done` (Puerta 2 aprobada por Raf)

Cierre de la **feature core** del producto. El backend de spec 03 estaba `done` desde la sesión 18 (Puerta 2, migraciones 0050-0057); el cliente se construyó **JIT a lo largo de la sesión 26 → 2026-06-19**, reconciliado contra el as-built de specs 02/04/09/10/11/15 (el `design.md` original predataba todo ese stack). Corrido en modo pipeline autónomo por el leader (Opus), pausando solo en las puertas humanas, chunk por chunk gateado (veto design-review del leader + reviewer + Gate 2):

- **M1** — wizard de config de jornada (inicio + 3 etapas) + preconfig de tanda inline + bottom sheet (`ManeuverConfigSheet`). Maniobras reordenables por drag (orden en `sessions.config` jsonb + presets).
- **M2** — identify-first (pantalla crítica 🔴): bastón BLE (spec 04, mock/web-serial) + manual + find-or-create (overlay de spec 09) + `useBusyMode` (suspende el listener BLE global). Carga rápida con **botones gigantes** (≥83% del alto, token `$11` hero): Tacto binario (2 pasos PREÑADA/VACÍA → tamaño) + Pesaje numérico (coma es-AR). M2.0 design spike aprobado por Raf como base visual de la manga (vetado contra 8 refs de Mobbin).
- **M3** — las 10 maniobras MVP cableadas.
- **M4** — offline/reanudación (landing ofrece retomar la jornada abierta, R10.5/R10.6) + **surfacing de rechazos de sync** (R10.8, store observable + banner terracota, que un rechazo no se pierda en silencio) + **M4.3** verificación offline (E2E jornada 100% offline → reconexión → oráculo server-side, R10.1/R10.2/R10.3/R10.7).
- **M5** — **datos/maniobras CUSTOM por campo** (diferenciador: medir lo que la competencia no permite). Dato custom = fila de `field_definitions` con `establishment_id`; los 2 `+` convergen en el mismo objeto; storage genérico `custom_measurements` (append-only) + `custom_attributes` (current-value). Reabrió schema → **M5-BACKEND** 0093-0097 (RLS de `field_definitions` + 2 tablas + triggers SECURITY DEFINER de gating/validación/audit/inmutabilidad) con **Gate 1 PASS**; render genérico desde `ui_component`.
- **M6** — **circunferencia escrotal** (toritos/toros enteros): rueda inercial (snap determinístico sobre `onScroll` settle en react-native-web) + edad auto-prefill ajustable + **M6-BACKEND** 0098-0100 (tabla typed `scrotal_measurements` + data_key + seed cría + gating, Gate 1 PASS) + tarjeta de mini-tendencia en la ficha + CE en el timeline.

**Cierre final** (commits `3186215`→`7f5181c`): **R8.4** (preview de transición de categoría offline en el resumen) + **R9.x** (asignación de lote opcional/manual desde el wizard) + reconciliación del **ledger de tasks.md** + cierre de **Gate 2** + **T2.12** (re-check find-or-create cross-spec) + **2 MEDIUM** de maxLength UX en inputs de texto libre. **Puerta 2 (código) APROBADA por Raf** (`fd36a4f`).

- **Verificación:** suites unit (>1500 client) + E2E Playwright verdes por chunk. El rojo recurrente de `check.mjs` = flake `animals_tag_unique` (23505) por la suite backend de la otra terminal corriendo SIGSA en paralelo — NO regresión (spec 03 es frontend). Capturas de manga a 360/412 web táctil en `tests/modo-maniobra/`.
- **Aprendizajes encodeados en skill/memoria durante el cliente:** 3 reglas UX de sheets/forms (título no se recorta al CRECER el contenido / sheet header-fijo+body-scroll+footer-fijo / validación scroll-al-campo+borde-rojo+inline) → skill `design-review` + `feedback_ux_basicos_sheets_forms`. Gotchas de react-native-web (snap de rueda, tap-through) → `reference_rn_web_pitfalls`.
- **Estado:** feature 03 `in_progress`→**`done`** en `feature_list.json`; ningún feature queda `in_progress` (08 SIGSA sigue `spec_ready`, lo lleva la otra terminal). **Pendiente NO-bloqueante** (no reabre la feature, fuera del alcance de spec 03): sync NATIVO de las tablas de maniobra (spec 15-nativo) + bastón **spp-android** real (spec 04) — ambos gateados por el **dev build Android** de Raf, que es el cuello de botella para validar el core on-device en el beta de Chascomús. **Balanza (spec 05) confirmada FUERA del MVP** (peso manual, Gate 0 del cliente). Detalle de cada chunk en `progress/impl_03-*.md`.

---

## 2026-06-20 — spec 03 (delta M7) — gestión de rutinas y datos custom (editar/borrar) → DONE + commiteado (`e26f80b`)

Delta additivo de spec 03 (feature ya `done`), pedido por Raf en vivo: *"si creé mal una rutina o una maniobra personalizada, ¿cómo la borro/corrijo?"*. El modelo de datos ya contemplaba el borrado (soft-delete de presets vía RPC `0057`; `deleted_at`/RLS/guard de `field_definitions` custom de M5) pero **no había UI ni service de borrado/edición custom** → M7 cabla esa gestión y completa R13.19/R13.26. Ciclo SDD completo corrido por el leader (Opus), pausando solo en las puertas humanas:

- **Rutinas (presets):** menú ⋯ por fila en "Tus rutinas" → editar (renombrar + reconfigurar reabriendo el wizard precargado → `updatePreset`, no arranca jornada) y eliminar (`softDeletePreset`, confirmación). Cualquier rol operativo (R2.6–R2.11).
- **Datos custom (owner-only):** ⋯ solo en filas custom de "Editar plantilla del rodeo" → editar `label`/opciones (append-only) y eliminar con **confirmación que advierte** (R13.28–R13.34). Service nuevo `softDeleteCustomField` (UPDATE plano offline; la SELECT-policy de `field_definitions` no filtra `deleted_at` → sin gotcha RLS-on-RETURNING, no necesita RPC).
- **Fix HIGH (reviewer, R13.26 borrar+recrear):** el índice UNIQUE `(establishment_id, data_key)` de `0093` no era parcial sobre `deleted_at` → recrear un dato con el mismo slug tras borrarlo colisionaba `23505`. **Migración `0101`** lo hace parcial (`WHERE establishment_id IS NOT NULL AND deleted_at IS NULL`), aplicada al remoto + verificada.
- **R13.30 = Opción B (decisión de Raf):** preservar el histórico de un dato borrado en la ficha NO es viable sin cambiar la sync-stream (que prunea la definición del device); en MVP el histórico deja de verse y el diálogo lo advierte. **Opción A** (cambio de stream) + un **bug pre-existente de M5** (`setCustomAttribute` falla al editar in-place por carrera de la VIEW de PowerSync) quedan en `docs/backlog.md` como fast-follow.
- **Gates + verificación:** Gate 0 + Puerta 1 + reviewer (tras reconciliación de docs) + Gate de seguridad (PASS, 0 HIGH; `0101` segura, owner-only server-side confirmado). Client unit 251/251, e2e M7 9/9, backend custom 19/19 (incl. borrar+recrear contra remoto). **Commiteado `e26f80b` en main** (33 archivos, solo la superficie de M7). 03 sigue `done` (M7 = delta additivo). Detalle: `progress/{impl,review,security_code}_03-m7.md`.

## 2026-06-23/24 — MODELO REPRODUCTIVO "puesta en servicio" (Streams A+B+C) → COMPLETO de punta a punta

Lo que arrancó como "spec 07 reportes" destapó un **cambio de modelo reproductivo cross-spec** (servicio = nivel-rodeo, no per-vaca) que se descompuso en 3 streams y se corrió con el **pipeline SDD autónomo** (leader-orquestado, Opus), pausando solo en las puertas humanas. Fuente de verdad: `docs/modelo-reproductivo-puesta-en-servicio.md` (gradúa a ADR cuando Facundo cierre §9). Detalle por chunk en `progress/{impl,review,security_*}_0{2,3,7}-*.md`.

- **Stream A — backend (delta spec 02), migraciones `0102`-`0105` APLICADAS al remoto.** `service_months smallint[]` por rodeo + CHECK (vía función immutable `service_months_is_valid` — subquery en CHECK no se permite); `create_rodeo` +param + `set_rodeo_service_months` (anti-IDOR por derivación); `compute_category` SIN el backstop `servicio→vaquillona` (diff quirúrgico vs 0062); 3 funciones de derivación servidas/entoradas (SECURITY DEFINER tenant-scoped). Veto del leader cazó un bug real (denominador excluía `vaquillona_prenada` → inflaba %preñez). Gates: veto + Gate 1 PASS + reviewer + Gate 2 PASS + Puerta 2/3 + OK de deploy de Raf. Suite no-bypass `puesta-en-servicio` TPS.1-16 verde. `feature_list` 02 → `deferred` (Stream A = delta).

- **Stream B — cliente (delta spec 03), 4 chunks commiteados.** B4 espejo de categoría sin `service` (`11f1d1d`) · B1 selector de meses de servicio **período CONTIGUO** (alta paso-4 + edición offline; RPC por outbox; `a80ea30`) · B2 tacto adaptativo (botones de tamaño según meses: 2→cabeza/cola, 3..11→CCL, 1/12→solo preñada/vacía; config "¿medir tamaño?"; `43a40eb`) · B3 baja de monta natural de la carga manual (IA/TE quedan; `a91be3e`). Diseños vetados por el leader + aprobados por Raf; reviewer + Gate 1/2 por chunk. **Constraint clave de Raf**: 1 período contiguo por rodeo (primavera Y otoño → rodeos separados). **Fix de infra reusable**: el e2e booteaba en blanco porque un spec importaba `test` de `@playwright/test` en vez de `./helpers/fixtures` (shim de env) → fix de 1 línea + `app/.env.example` + `EXPO_PUBLIC_POWERSYNC_URL` propagado a `app/.env.local` (memoria `reference_e2e_fixtures_import`).

- **Stream C — reportes (spec 07), feature 07 → `done`.** Spec redactada (14 grupos R7.x) + Gate 1 PASS + 4 MEDIUM foldeados + 5 decisiones de Raf en la Puerta de spec (%preñez/%parición base ÚNICA servidas sin selector + denominador explícito; sin-pesar 180d parametrizado [alcance sigue Facundo]; comparativa por sesiones; año = última campaña con datos; archivados incluidos). **Backend**: `0106` (9 RPC SECURITY DEFINER, cómputo server-side online-only) APLICADA por el leader vía `scripts/apply-migration-mgmt.mjs` (Management API — nuevo helper reusable para cuando el MCP tiene token viejo) + 2 fix-loops post-apply (la suite cazó 2 bugs SQL contra datos reales: `sort_order` en GROUP BY + `id` ambiguo vs OUT param); suite `reports` 14/14. **Frontend**: `reports.ts` online-only + componentes (KpiCard/CclBars/AlertList/ReportStates) + 4 pantallas (4 KPIs + distribución CCL tacto-vs-nacimientos + 2 alertas) + 34 tests puros; diseño vetado + aprobado por Raf. Reviewer + Gate 2 PASS en backend y frontend. Commits `ad9c0ba`+`c5e3eb6`. Limitación conocida: R7.2.3 cache-offline (opcional, no impl).

- **Token de Management API**: el PAT (`SUPABASE_ACCESS_TOKEN`) caducó a mitad de sesión (401 en `operaciones_rodeo` + MCP) → Raf lo rotó en `.env.local` (la suite quedó verde) pero el MCP cacheó el viejo → se trabajó por CLI/Management API (memoria `reference_check_red_rate_limit` ampliada).
- **Pendientes**: Facundo §9 (bucketing CCL 4-11 meses; alcance/categorías de la alerta sin-pesar) → abren ADR cuando cierren. Backlog: R7.2.3 cache-offline; `establishment_unweighed` sin LIMIT (perf); contigüidad server-side como defensa-en-profundidad. **Memoria** `project_modelo_reproductivo_done` creada.

## 2026-06-24/25 — SPEC 08 EXPORT SIGSA → code-complete + committeada (`9c372a8`), `in_progress` (gated por Facundo)

Implementación end-to-end de la exportación SIGSA (genera el TXT `RFID-SEXO-RAZA-MM/AAAA` importable en SIGSA web, Res. 841/2025, offline-first + audit trail) corrida con el **pipeline SDD autónomo** (leader-orquestado, Opus), pausando solo en las puertas humanas. Entrada: `spec_ready` + Puerta 1 aprobada + capa pura (T8-T10) ya gateada. Detalle por chunk en `progress/{impl,review,security_*}_08-*.md`.

- **Las 4 decisiones abiertas CERRADAS** (el leader las cuestionó críticamente, no pasamanos): #1 raza desconocida = soft-block + `OR` NO promovido (sort_order 28); #2 markAsDeclared al MVP (copy "Marcar como ya declarado por otro medio"); #3 **RENSPA texto opcional SIN unique** (el unique global causaba colisión + fuga cross-tenant y el RENSPA ni va en el TXT; anti-fraude → post-MVP); #4 gate duro de formato lo hace **Facundo** (upload real → confirma 5 incógnitas → gatea el `done`; ⚠ riesgo: confirmar si es dry-run o declaración legal firme).

- **6 capas, cada chunk gateado (reviewer + Gate 1/2 + veto design-review):** Pura T8-T10 · **DB** T1-T6 migraciones **0107-0112** (breed_catalog 28 razas + breed_id en animal_profiles/reproductive_events + establishments.renspa + sigsa_declarations + export_log) · **PowerSync** T7 (3 streams: catalog_breed global + sigsa_declarations/export_log `org_scope`; **deployadas por Raf** en el dashboard) · **Servicio** T11/T12/T19/T20 + hook · **UI** T13-T18 (pantalla export flagship + sticky CTA + BreedPicker + RENSPA + checklist) · **Run 3 / `0113`** trigger derive-breed_id + edición de raza en la ficha (cierra T18). Migraciones 0107-0113 APLICADAS al remoto vía Management API (deploy gateado, Raf autorizó en sesión + agregó el permiso del script).

- **2 hallazgos load-bearing (el implementer no se quedó en lo pedido):** (a) **Bug crítico** — el connector de PowerSync ruteaba TODO el CRUD-plano por `.upsert()` (= `INSERT ON CONFLICT DO UPDATE`); las tablas append-only (sin grant UPDATE, R11.3) daban 42501 → **descarte silencioso → la feature estaba rota end-to-end** (ninguna declaración llegaba al server). Fix quirúrgico: `isAppendOnlyInsertTable` → `.insert()` (retry-safe: 23505 en reintento se descarta = idempotente). Gate 2 PASS sobre la superficie compartida. (b) **Gap de breed_id** — el BreedPicker seteaba el `breed` texto pero no `breed_id` (la RPC `create_animal` no lo lleva); en vez de tocar firmas, un **trigger** (`0113`) deriva `breed_id` desde `breed` por match de nombre (alta + import + edición de una), con guard `NEW.breed IS NOT NULL` para no pisar la raza heredada del ternero al pie (verificado por test §T18(c)).

- **Reconciliaciones del leader contra el as-built:** migraciones renumeradas 0089-0094 → **0107-0112** (la DB había avanzado a 0106); la query de export NO hace `JOIN animals` (no entra al sync set, ADR-026 → rompía offline → usa columnas denormalizadas `animal_profiles.animal_*` de 0079); `reproductive_events.breed` (texto) NO existía → best-effort de R1.6 = no-op; las 2 redefiniciones de objetos deployados (trigger ternero mono 0048 + RPC mellizos register_birth 0075) diffeadas byte-a-byte antes de aplicar (idénticas salvo las 3 adiciones de breed_id).

- **Decisiones de diseño del leader** (Raf delegó "elegí la mejor y seguí" → memoria `feedback_decide_dont_offer_options`): terracota "N a completar" → `$textMuted`; CTA export → sticky-bottom; markAsDeclared → action-sheet por fila; filtro de fechas R9.3 agregado; metadata categoría/rodeo en fila → diferida. NULL-on-no-match del trigger = decisión documentada (fail-safe + consistencia breed↔breed_id).
