...
> **RECONCILIACIÓN AS-BUILT (2026-06-15, VÍA ELIMINADA):** la pre-config de la vacunación masiva es **SOLO el producto**. El campo **"Vía de aplicación" se eliminó por completo** del flujo de vacunación — decisión de producto cerrada por Raf tras consultar a Facundo (el vet socio): el **producto YA implica la vía** (cada vacuna/antibiótico/antiparasitario la trae en su etiqueta) → capturarla aparte es redundante e innecesaria. El mismo criterio aplica a antibióticos y antiparasitarios (flujos de tratamiento/desparasitación que aún no existen en UI → nacen sin vía). Esto **revierte** el delta vía-intranasal del 2026-06-14 (curado de la vía a 3 opciones SC/IM/Intranasal). La columna `sanitary_events.route` (enum `public.sanitary_route`, incl. el valor `intranasal`) queda **DORMIDA**: nullable, no se escribe; **NO se dropeó** (dropear sería destructivo y podría romper la RPC del timeline — una columna NULL no molesta). El INSERT de vacunación omite la columna `route` → queda NULL por default. (Sub-detalle del pre-config — no abre un EARS propio, igual que la vía nunca tuvo uno.)
