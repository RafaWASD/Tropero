...
> **AS-BUILT — chunk UI-B (selección masiva + bottom-sheet, 2026-06-12; T-UI.4/T-UI.5).** Hace funcionales **Castrar** y **Destetar** (eran stubs). Toda la lógica/services de Fase 2+3 se REUSARON (cero re-implementación):
> - **Pantalla de selección (T-UI.4):** `app/app/seleccion-masiva.tsx` REEMPLAZA el stub. Datos: service nuevo `app/src/services/bulk-selection-data.ts` (`fetchGroupSelectionProfiles` — rodeo→`fetchAnimals({rodeoId})`, lote→`fetchGroupMembers`; ambos traen la categoría del espejo C6 + el display; mergea los 3 flags que `AnimalListItem` NO expone vía el builder PURO nuevo `buildGroupCandidateFlagsQuery` en `local-reads.ts`: `is_castrated`/`category_override`/`has_weaning`=EXISTS un `weaning` vivo synced|pending). Candidatos+selección = Fase 2 intacta (`buildBulkCandidates`/`buildBulkSelectionState`; defaults R11.3/R11.4 sin tocar). Presentación PURA nueva: `app/src/utils/selection-display.ts` (`sortByIdentifier` numérico/es-AR + desempate por profileId — R11.9; `SEARCH_THRESHOLD=20`/`shouldShowSearch`/`filterBySearch` — buscador solo >20; `pluralCategoryLabel` — desglose) con test (`selection-display.test.ts`). Header con back + título + **contador vivo** (R11.5); secciones por categoría con "Todos/Ninguno" (`sectionCheckState`/`toggleSection`); fila = `AnimalRow compact` + checkbox; **resaltado ⭐** = prop NUEVA `highlight` de `AnimalRow` (borde izq terracota + fondo $surface — lenguaje terracota-signal as-built, SIN token nuevo) cuando `operation==='castrate' && futureBull && checked` (R11.6, SIN modal); **CTA fijo abajo** (thumb-zone, Button primary) con número vivo, disabled en 0 (R11.7); empty-state es-AR. **R7.2 (destete cross-rodeo)** cableado: para wean+lote resuelve `rodeoWeaningEnabled` (`fetchRodeoGroupActions` por rodeo distinto) → `buildBulkCandidates` excluye + cuenta → InfoNote "N terneros excluidos por config del rodeo" (fail-open de DISPLAY si el gating no resuelve — la authz es server-side).
> - **Bottom-sheet de confirmación (T-UI.5):** `app/src/components/BulkConfirmSheet.tsx` (presentacional). Scrim = token NUEVO `$scrim` en `tamagui.config.ts` (= textPrimary @ 45%, mismo patrón derivado que `fabHalo`); sheet anclado abajo con grip; backdrop tappable cierra. Desglose por categoría (`summarizeSelection` Fase 2 → `pluralCategoryLabel`); "⚠ N futuros toritos incluidos" solo castración + `futureBullCount>0` (R11.8); aviso de override + acción "Quitar la fijación…" → `revertCategoryOverride` as-built (C6, por animal con override, luego recarga) (R5.6); **copy reversible literal** "Podés corregirlo después desde la ficha de cada animal." — PROHIBIDO "no se puede deshacer" (R11.8/Gate 0 v2); CONFIRMAR/Volver. Al CONFIRMAR → `applyBulkCastration`/`applyBulkWeaning` (Fase 3 REUSADOS — `author_id` NUNCA en el payload de la observación, lo fuerza el server) → **panel de progreso in-screen** `app/src/components/BulkProgressPanel.tsx`: "Castrando X de N…" (`onProgress` del encolado local) → "N listos, se sincronizan en segundo plano" + rechazos LOCALES por animal (R10.3; el "X de N sincronizados" por animal lo superficia uploadData as-built, spec 15) → "Listo". Error antes de encolar → reintentar.
> - **Remoción de la card "Datos que se cargan acá" (decisión del leader, Nielsen #8):** se sacó `GroupConfigSummary` de `GroupViewScreen.tsx` (era redundante con las 3 acciones del grupo que están debajo). El **gating de las acciones NO cambia** (se sigue resolviendo en `group-data.ts`); solo se quitó el chip visual. Sin código muerto (los íconos `Syringe`/`Milk` y el tipo `GroupActionsAvailability` solo los usaba esa card → removidos del import de ese archivo).
> - **Capturas (design/spec10-ui-b/):** `seleccion-castracion.png` (secciones + defaults + ⭐ tildado resaltado + CTA), `bottom-sheet.png` (desglose + ⚠ + override-revert + copy reversible), `vista-grupo-sin-card.png`. Generadas con `e2e/captures/spec10-uib-screenshots.capture.ts`.

> **AS-BUILT — chunk UI-B2 (vacunación masiva, 2026-06-12; T-UI.6).** Hace funcional **Vacunar** (era stub). Modelo Gate 0 ORIGINAL (filtro+preview+skip-report — NO selección por checkbox, R4.1; "vacunación NO cambia"). Toda la lógica/services de Fase 2+3 se REUSARON:
> - **Pantalla (T-UI.6):** `app/app/vacunacion-masiva.tsx` REEMPLAZA el stub. Recibe el grupo (`groupType`+`groupId`). Datos: `fetchGroupSelectionProfiles` (REUSADO de UI-B — todos los activos del grupo, SQLite local, offline). Pre-config: **SOLO `FormField` producto** (obligatorio, `product_name` — texto libre legítimo, columna `text`). **La vía se eliminó** (ver DELTA: VÍA ELIMINADA abajo). Filtro OPCIONAL (R4.1): chips de **categoría** (≥`$chipMin`, mismo lenguaje que la tab Animales; solo si >1 categoría presente) + **sexo** (Machos/Hembras; solo si ambos presentes) — default = TODOS, sin filtro. Conjunto candidato = `buildBulkCandidates('vaccinate', profiles, { filter })` (Fase 2 REUSADO — filtro categoría/sexo).
>   - **DELTA: VÍA ELIMINADA (2026-06-15, as-built — REVIERTE el delta vía-intranasal del 2026-06-14).** El campo **"Vía de aplicación" se sacó por completo** del flujo de vacunación. **Decisión de Facundo** (el vet socio), cerrada por Raf: el **producto YA implica la vía** (cada vacuna/antibiótico/antiparasitario la trae en su etiqueta) → capturar la vía aparte es redundante e innecesario. **Aplica a vacunas + antibióticos + antiparasitarios** (los flujos de tratamiento/desparasitación que aún no existen en UI nacen sin vía). **Qué cambió:** el campo se sacó de la **UI** (`vacunacion-masiva.tsx`: ya no hay estado `route`/`setRoute`/`toggleRoute` ni el bloque de chips de vía) y del **path del servicio** (`VaccinationParams` sin `route`; `planVaccination` y `buildAddVaccinationInsert` sin el parámetro; el INSERT a `sanitary_events` OMITE la columna `route`). Se **eliminó** el módulo `app/src/utils/sanitary-route.ts` + su test (desregistrado de `run-tests.mjs`). **DB DORMIDA, sin migración:** la columna `sanitary_events.route` (enum `public.sanitary_route`) + el valor de enum `intranasal` (agregado por 0090) quedan **DORMIDOS** — la columna es nullable y no se escribe; **NO se dropeó** la columna ni el valor del enum (dropear sería destructivo y podría romper la RPC del timeline; una columna NULL no molesta). `event-timeline.ts` (`humanizeRoute`/`ROUTE_LABELS`, incl. el label `intranasal`) se **conserva intacto**: muestra la vía de eventos LEGACY que ya la tengan; un evento nuevo tendrá `route=null` → `humanizeRoute(null)` devuelve null → no se muestra (display defensivo, cero costo).
> - **Preview PURO NUEVO (R4.2/R4.3/R4.4):** `app/src/utils/vaccination-preview.ts` (`buildVaccinationPreview` → `{ toApply, animalsToApply (M), eventsToApply (N=M, 1 por animal), skipped: { alreadyApplied, rodeoDisabled }, skippedTotal }`; **precedencia rodeoDisabled > alreadyApplied** sin doble-conteo; `deriveCategoryFilterOptions` — opciones de filtro derivadas de los activos presentes con conteo). Test `vaccination-preview.test.ts` (10 casos: N=M sin skips, already_applied por UUIDv5, rodeoDisabled cross-rodeo, precedencia, R4.4 toApply excluye saltados, opciones de filtro). El **preview se recalcula EN VIVO** al cambiar el filtro (no al tipear producto → sin thrashing de queries).
> - **Service NUEVO `previewVaccination`** (`bulk-operations.ts`): I/O — resuelve los ids de `vaccination` ya aplicados localmente (`buildExistingVaccinationIdsQuery` REUSADO, barrera idempotente UUIDv5, R6.3) + delega el conteo en el util puro. El gating por rodeo (lote cross-rodeo, R7.2) lo resuelve la pantalla (`fetchRodeoGroupActions.vaccinate` por rodeo distinto) y lo pasa; fail-OPEN de DISPLAY si no resuelve (la authz es el gating capa 2 server-side de 0054, fail-closed).
> - **Apply + progreso:** al confirmar (CTA "Vacunar M animales", explícito, disabled sin producto o sin animales) → `applyBulkVaccination` (Fase 3 REUSADO) sobre `preview.toApply` (R4.4: los saltados —already_applied + rodeoDisabled— NO se encolan) → `BulkProgressPanel` (REUSADO de UI-B, verbo "Vacunando" → rechazos LOCALES por animal). Re-ejecutable (R6.3): los ya-vacunados de esa fecha se saltan por el UUIDv5 + skip already_applied → el preview refleja solo los nuevos. Empty-state si el grupo no tiene activos. `author_id` no aplica (sanitary_events); no se mandan columnas que el server fuerza. Offline-first (CRUD plano → uploadData; NO se toca el connector).
> - **Capturas (design/spec10-ui-b2/):** `vacunacion-preview.png` (**SOLO producto** + filtro + "N eventos sobre M animales" + skip "1 animal ya tiene esta vacunación cargada hoy" + CTA), `vacunacion-skip.png` (chip Ternero aplicado → preview del subconjunto + skip). Generadas con `e2e/captures/spec10-uib2-screenshots.capture.ts`. *(Delta VÍA ELIMINADA 2026-06-15: la capture ya NO asserta chips de vía — solo llena Producto y sigue al preview/CTA. El `operaciones-vacunacion.spec.ts` tampoco selecciona vía. El leader regenera los PNG.)*

> **AS-BUILT — chunk UI-C (la FICHA del animal, 2026-06-12; T-UI.7 + T-UI.8).** Toca SOLO `app/app/animal/[id].tsx` (+ un util puro nuevo + builders/proyección de soporte). Toda la lógica/services de Fase 3 se REUSARON (cero re-implementación de write-paths):
> - **Castrado + futuro torito (T-UI.7):** sección NUEVA **"Manejo"** (`ManagementSection`, ícono `Scissors`, firma RAFAQ DetailSection) que se renderiza **SOLO para machos** (`detail.sex==='male'` — para hembras ni se monta: la castración no aplica, future_bull es solo-machos). (1) `CastrationRow` "Castrado Sí/No" + "Cambiar" (solo macho activo) → confirmación INLINE que ANTICIPA el recálculo ("La categoría se recalcula: {destino}", mismo patrón que `CategoryOverrideCard` de C6) → al confirmar `setCastrated` (Fase 3 REUSADO: UPDATE is_castrated + future_bull=0 + observación automática R13.7). (2) `FutureBullRow` ⭐ "Futuro torito" + "Marcar"/"Quitar" → `setFutureBull` (Fase 3 REUSADO, sin observación); badge ⭐ vía `shouldShowFutureBullBadge` (REUSADO de AnimalRow — solo positivo + oculto en `toro`, R12.3). La fila ⭐ se OCULTA si `isCastrated` (un castrado ya no es futuro torito; el server lo auto-limpia R12.4). Ambas recargan la ficha → el espejo C6 con el is_castrated REAL refleja la categoría nueva AL INSTANTE offline (R10.6). **La anticipación del recálculo NO duplica el espejo:** service NUEVO `previewCastrationCategory(profileId, nextValue)` (animals.ts, orquesta I/O igual que `previewRevertCategory`) + **función PURA nueva `resolveCastrationTargetCategory`** (animal-category.ts — `computeCategoryCode` con el is_castrated NUEVO + resuelve name en el catálogo; respeta override→null). La línea se OMITE si el destino == categoría actual (ternero no transiciona) o hay override (R5.6) — el flip + la observación se aplican igual. `author_id` NUNCA en el payload (invariante de Fase 3, intacta).
> - **Corrección individual de eventos (T-UI.8):** INVESTIGACIÓN de spec 02 R6.8.1 → el server-side ya está COMPLETO (deleted_at en `sanitary_events`/`reproductive_events`; RLS UPDATE owner|autor; trigger 0046 recalcula al borrar un `weaning`), pero **la ficha NO tenía edit/delete de eventos** (`TimelineEvent` era display-only). GAP cerrado con el MÍNIMO = **BORRADO (soft-delete), NO edición**: wrapper local `DeletableTimelineEvent` (envuelve el `TimelineEvent` canónico SIN tocar su contrato) con botón `Trash2` + confirmación inline → `deleteTypedEvent({kind, eventId})` (NUEVO en `events.ts`) → `buildSoftDeleteEventUpdate(table, eventId)` (NUEVO en `local-reads.ts`: UPDATE `deleted_at=datetime('now')` guard `deleted_at IS NULL` idempotente; tabla resuelta por la const `DELETABLE_EVENT_TABLE[kind]` — sin inyección SQL). Gating de cliente best-effort (`canDeleteEvent`: solo vacunación/destete activos + owner del campo o autor del evento); la **RLS UPDATE server-side (owner|autor) es la barrera real**. El `created_by` de sanitary/reproductive se PROYECTA en `buildTimelineQuery` (json_object) + se expone en `TimelineItem.createdBy` (parser de `event-timeline.ts`). Offline-safe (CRUD plano → uploadData; un rechazo de autz re-aparece el evento). La castración se corrige por T-UI.7 (no por el timeline). **Limitación conocida (UX, no seguridad):** un evento creado OFFLINE por el propio usuario tiene `created_by` NULL local hasta el sync-down → el autor no-owner no ve el botón hasta que sincronice (best-effort; la RLS lo permitiría). Alcance MVP: borrado, no edición.
> - **Capturas (design/spec10-ui-c/):** `ficha-macho-manejo.png` (torito con "Castrado: No / Cambiar" + "Futuro torito: Sí ⭐ / Quitar"), `ficha-macho-confirmar.png` (la confirmación que anticipa "La categoría se recalcula: Novillito"), `ficha-hembra-sin-manejo.png` (vaquillona SIN la sección Manejo — invariante solo-machos). Generadas con `e2e/captures/spec10-uic-screenshots.capture.ts`.
> - **Fix de comentario (MED del Gate 2 de UI-B):** en `seleccion-masiva.tsx` (~líneas 140-150) el comentario del fail-open del gating de destete decía que lo respalda "la barrera server-side" — FALSO para el GATING: `tg_sanitary_events_gating` (0054) NO gatea `weaning` (excluido a propósito, decisión spec US-8 / R7.3). Corregido a la verdad: el gating de destete es **DISPLAY-ONLY**; el server NO lo enforce-a; lo único server-side es la RLS de AUTORIZACIÓN (has_role_in), que NO mira el data_key. Solo el comentario; lógica intacta.
> - **RECONCILIACIÓN VIA-ENUM-MISMATCH (fix-loop de Gate 2 HIGH, 2026-06-12; T-UI.6).** El campo **"Vía (opcional)"** se construyó originalmente como `FormField` (TextInput) de **texto libre** (placeholder "Ej. Subcutánea") y el string crudo viajaba sin mapeo al INSERT de `sanitary_events.route`. Pero `route` es el ENUM `public.sanitary_route` (0027) con SOLO 5 valores (`intramuscular|subcutaneous|oral|topical|other`): cualquier string fuera del enum hacía que Postgres rechazara el INSERT con `22P02` → `upload-classify.ts` lo clasificaba **permanente** → la op se descartaba del queue → **pérdida de los N eventos de la masiva server-side** después de que la UI dijo "Vacunando ✓" (pérdida de datos sanitarios en el happy path). **As-built del fix:** la vía ahora es un **selector de chips** (mismo `FilterChip`/`FilterChipRow` que el filtro categoría/sexo — consistencia visual; ≥`$chipMin`) con los 5 valores del enum (single-select toggle: tocar el seleccionado lo deselecciona → vuelve a `null`; vía sigue **OPCIONAL** = `null` permitido y ya soportado). El estado pasó de `string` ('') a `SanitaryRoute | null`. Util PURO nuevo `app/src/utils/sanitary-route.ts` (`SANITARY_ROUTES` anclado al enum 0027 con anti-drift, `routeOptions()` con labels es-AR — `other`→"Otra", `isValidRoute` type-guard, `toRouteValue` barrera dura que normaliza a código-válido-o-`null`) + test `sanitary-route.test.ts` (6 casos: anti-drift contra 0027, opciones, rechazo de texto libre / `intravenous` / case / no-strings, invariante de `toRouteValue`). El INSERT recibe `toRouteValue(route)` → **SIEMPRE un código del enum o `null`, nunca texto libre**. Auditados los demás campos de las masivas: ninguno manda texto libre a una columna enum/constrained (`product_name`→`text` libre legítimo OK; `weaning`/observación → literales o `text`). Captura regenerada (`vacunacion-preview.png`). **OBSOLETO (2026-06-15):** el campo de vía completo —chips, módulo `sanitary-route.ts` y su test— se **eliminó** por decisión de producto (ver DELTA: VÍA ELIMINADA arriba). Este registro queda como histórico de por qué la vía nunca debió viajar como texto libre; ya no aplica porque la vía no se captura.

> **AS-BUILT — chunk UI-D (E2E Playwright de las 3 operaciones masivas, 2026-06-12; T-UI.9/10/11).** TEST-ONLY: cero cambio de producto. 3 specs nuevos en `app/e2e/`, contra el export de prod (:8099) + Supabase remoto + PowerSync, auto-descubiertos por el glob `**/*.spec.ts` (sin registro manual). Datos namespaced (RUN_TAG), cleanup en afterAll + global-teardown, aserción solo sobre datos propios.
> - `operaciones-castracion.spec.ts` (T-UI.9): seed 2 terneros comunes + 1 ternero ⭐ + 1 torito adulto (helper `seedAnimal` extendido con `isCastrated`/`futureBull`, test-only). Defaults R11.3 verificados por CONTADOR+CTA; ⭐ resaltado sin modal; bottom-sheet con ⚠ "1 futuro torito incluido" + copy reversible + ausencia de "no se puede deshacer"; post-masiva: torito → Novillito (espejo C6 offline R10.6) + observación "Castrado" (timeline R13.7); revert desde la ficha → Torito (recompute simétrico R13.5) + "Corrección: marcado como no castrado". La transición se prueba sobre el TORITO (un `ternero` castrado NO transiciona, 0062).
> - `operaciones-destete.spec.ts` (T-UI.10): seed 2 terneros (mellizos) + 1 ternera + 1 ternera con `category_override`. Todos pre-tildados (R11.4, contador=4); bottom-sheet avisa override (R5.6); post-masiva: ternera→Vaquillona, mellizos→Torito (R5.5; cada mellizo con su "Destete" en el timeline, R3.5); el de override SIGUE Ternera + "Categoría fijada manualmente" PERO con "Destete" en su timeline (el weaning igual se aplicó, R5.6).
> - `operaciones-vacunacion.spec.ts` (T-UI.11): seed 3 activos. Producto (SOLO producto — la vía se eliminó) → preview "3 eventos sobre 3 animales" (R4.2) → confirmar → "3 animales listos" (R3.1); RE-EJECUTAR la misma → "Ningún animal nuevo para vacunar" + "3 animales ya tienen esta vacunación cargada hoy" + CTA disabled (R6.3 idempotencia).
> - **OBSERVACIÓN a11y NO-BLOQUEANTE (descubierta en UI-D, NO se parchea acá — TEST-ONLY).** El checkbox de la fila compacta de `AnimalRow` (selección masiva) pasa `accessibilityState={{checked}}` crudo al `Pressable`; **react-native-web NO lo traduce a `aria-checked`** en el DOM del export de prod (el `role="checkbox"` y el `aria-label` SÍ aparecen, pero `aria-checked` queda ausente). NO es un bug funcional: la selección opera bien y su señal user-facing (el CONTADOR vivo + el CTA + el resaltado ⭐) es correcta — por eso los E2E verifican los defaults por el contador, no por `aria-checked`. Es un gap de a11y de lectores de pantalla (no anuncian tildado/destildado). Para cerrarlo bien, `RowCheckbox`/`AnimalRow` deberían emitir el estado vía un helper tipo `switchA11y` (que ya emite `aria-checked` correcto en web). Anotado para una pasada de a11y de `AnimalRow` (chunk de pulido o backlog) — el leader decide; NO entra en el alcance test-only de UI-D.
...
> **RECONCILIACIÓN OPTIMISTA-EN-SITIO (as-built, fix Raf 2026-06-12).** Toda acción de la ficha (`animal/[id].tsx`) que mutaba estado (toggle Castrado, toggle ⭐, borrar evento, revertir override, asignar lote) disparaba un `load()` que seteaba `loading=true` → **desmontaba el contenido (pantalla en blanco) y reseteaba el scroll al tope** (UX reportada por Raf). Corregido al patrón correcto de **actualización optimista en sitio**:
> - `load(opts: { silent?: boolean })` separa **carga inicial** (puede blanquear: toca `loading`) de **refresh post-acción** (`silent: true`: NO toca `loading` — el ScrollView queda montado, el scroll se mantiene, solo cambian los datos). El `useFocusEffect` blanquea solo en la 1ra carga (mount / cambio de `profileId`); los re-focus posteriores son silenciosos.
> - **Castrado**: se anticipa la categoría destino con `previewCastrationCategory` (espejo C6) ANTES de escribir → patch optimista de `detail` (isCastrated, future_bull se limpia al castrar R12.4, categoría si transiciona) → `setCastrated` → refresh silencioso (trae la observación automática al timeline). El server es la verdad (LWW reconcilia al subir); el optimista solo adelanta lo que el server confirmará.
> - **⭐**: patch optimista de `future_bull` (sin observación) → refresh silencioso. **Borrar evento**: se quita el ítem del timeline optimistamente → refresh silencioso (reconcilia el recálculo de categoría de un destete borrado). **Revertir override / asignar lote**: refresh silencioso (lote además con patch del nombre).
> - Si la escritura es **rechazada**, se **revierte** el optimismo (snapshot de `detail`/timeline) + se muestra el motivo — no se deja un estado mentido.
> - Cubierto por E2E `operaciones-castracion.spec.ts` (tras el toggle: no reaparece "Cargando ficha…" + el scroll NO se resetea al tope + la categoría se refleja en sitio).

### 3.4 Gating en cliente (capa 1)

- **Rodeo**: Vacunar si `vacunacion` enabled; Destetar si `destete` enabled (`rodeo-config.ts` as-built, cacheado offline). Castrar: no se gatea por config (R1.5).
...
> **RECONCILIACIÓN GATING-POR-CANDIDATOS (as-built, fix Raf 2026-06-12).** Al gating de CONFIG se le suma un gating por **presencia de candidatos**: una acción NO se ofrece en la vista de grupo si abriría una pantalla de selección VACÍA. Es una dimensión ortogonal a R1.5/R1.6 (que hablan de config), no las contradice — la castración sigue sin gatearse por `rodeo_data_config`, solo se le agrega el requisito de candidatos.
> - **Vacunar**: SIN cambio — aplica a todos los activos (si hay animales, hay candidatos) → sigue gateada solo por config.
> - **Destetar**: config destete habilitado **Y** ≥1 candidato a destete (ternero/ternera sin `weaning`; en lote cross-rodeo, en un rodeo con `destete`). Pedido explícito de Raf.
> - **Castrar**: ≥1 candidato a castración (macho entero ternero/torito/toro). Sin candidatos → no se ofrece (extensión del mismo principio por consistencia — decisión del leader).
> - **As-built**: la decisión es PURA en `utils/group-actions.ts` — `applyCandidateGating(config, counts)` (vaccinate=config; wean=config.wean && counts.wean>0; castrate=counts.castrate>0). El tipo `GroupActionsAvailability.castrate` pasa de literal `true` a `boolean`. Los conteos los computa `services/group-data.ts` (`fetchRodeoGroupActions(rodeoId, animals)` / `fetchLoteGroupActions(animals)`) con `buildBulkCandidates` (Fase 2) sobre los perfiles del grupo ya cargados (flags de candidatura vía `buildGroupCandidateFlagsQuery`, SQLite local, offline). Config FAIL-SOFT: si no se lee config, Castrar se gatea igual por candidatos. Nueva `fetchRodeoConfigGating(rodeoId)` (config-only, sin candidatos) para los predicados de exclusión cross-rodeo de las pantallas de masivas (que necesitan "¿el rodeo tiene `destete`?", no "¿tiene candidatos?"). `GroupViewScreen` oculta la card "Acciones del grupo" si ninguna acción queda disponible. Cubierto por `group-actions.test.ts` (applyCandidateGating) + E2E `operaciones-destete.spec.ts` ("un rodeo sin terneros NO ofrece Destetar").
