...
> - **Pantalla (T-UI.6):** `app/app/vacunacion-masiva.tsx` REEMPLAZA el stub. Recibe el grupo (`groupType`+`groupId`). Datos: `fetchGroupSelectionProfiles` (REUSADO de UI-B — todos los activos del grupo, SQLite local, offline). Pre-config: `FormField` producto (obligatorio, `product_name` — texto libre legítimo, columna `text`) + **vía (opcional, `route`) como SELECTOR DE CHIPS** del enum `sanitary_route` (NO texto libre — ver reconciliación VIA-ENUM-MISMATCH abajo). Filtro OPCIONAL (R4.1): chips de **categoría** (≥`$chipMin`, mismo lenguaje que la tab Animales; solo si >1 categoría presente) + **sexo** (Machos/Hembras; solo si ambos presentes) — default = TODOS, sin filtro. Conjunto candidato = `buildBulkCandidates('vaccinate', profiles, { filter })` (Fase 2 REUSADO — filtro categoría/sexo).
>   - **DELTA VÍA-INTRANASAL (2026-06-14, as-built).** El selector de vía de VACUNACIÓN ofrece **EXACTAMENTE 3 vías curadas** — **Subcutánea · Intramuscular · Intranasal** — vía `vaccineRouteOptions()` (subconjunto de `app/src/utils/sanitary-route.ts`), **NO las 6 del enum**. Razonamiento de producto (decisión cerrada con Raf): `topical` es pour-on antiparasitario, no vía de vacuna; `oral` prácticamente no existe como vía de vacuna bovina; `other` es innecesario con las 3 reales. Se AGREGÓ `intranasal` (vacunas respiratorias vivas IBR/BRSV/PI3, una vía real que faltaba) → **migración `0090_sanitary_route_intranasal.sql`** (`alter type … add value if not exists 'intranasal'`, idempotente). El enum CONSERVA `topical`/`oral`/`other` porque `sanitary_events.route` es compartido por desparasitación/tratamiento futuros (ahí el tópico/oral SÍ aplican); para esos forms futuros queda `routeOptions()` (las 6 genéricas). La barrera `toRouteValue`/`isValidRoute` (anti-22P02) sigue operando sobre el enum COMPLETO (las 6 siguen siendo valores de DB válidos). **El deploy de 0090 es prerequisito de que la UI pueda mandar `intranasal`** (sin el valor en la DB, un INSERT con `route='intranasal'` daría 22P02). Tests: `sanitary-route.test.ts` (anti-drift enum 6 valores; `vaccineRouteOptions` = exactamente SC/IM/Intranasal, sin topical/oral/other; topical/oral siguen `isValidRoute`/`toRouteValue`). `event-timeline.ts ROUTE_LABELS` mapea `intranasal → 'Intranasal'` (display del timeline).
...
> - **Capturas (design/spec10-ui-b2/):** `vacunacion-preview.png` (producto + **vía como chips, "Subcutánea" seleccionada** + filtro + "5 eventos sobre 5 animales" + skip "1 animal ya tiene esta vacunación cargada hoy" + CTA), `vacunacion-skip.png` (chip Ternero aplicado → "2 eventos sobre 2 animales" + skip). Generadas con `e2e/captures/spec10-uib2-screenshots.capture.ts`. *(Delta vía-intranasal 2026-06-14: la capture ahora asserta las **3 vías curadas** —SC/IM/Intranasal— y que Oral/Tópica/Otra YA NO aparecen; sigue seleccionando "Subcutánea". El `operaciones-vacunacion.spec.ts` no cambia: ya seleccionaba "Subcutánea", que permanece en el subconjunto.)*
...
> - `operaciones-vacunacion.spec.ts` (T-UI.11): seed 3 activos. Producto + vía por chip → preview "3 eventos sobre 3 animales" (R4.2) → confirmar → "3 animales listos" (R3.1); RE-EJECUTAR la misma → "Ningún animal nuevo para vacunar" + "3 animales ya tienen esta vacunación cargada hoy" + CTA disabled (R6.3 idempotencia).
