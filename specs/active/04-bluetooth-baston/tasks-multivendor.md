...
| MV.5 — `adapter-spp-android` escrito (RMV5) | ✅ **código + tests puros** | conexión SPP real + dev build | — |
...
- [ ] T-MV.5.5 — *(GATED por hardware — NO en la pasada buildable-hoy)* Montar `adapter-spp-android` en el provider para Android device (`instantiateTransport('spp-android')` → `new SppAndroidAdapter(RS420_DRIVER)`; `selectTransportAdapter({android,auto})` → `'spp-android'`) + **prueba real con el RS420** (pairing/stream/reconexión/dedup/asignación masiva/fallback manual). Sin tocar el contrato ni los otros adaptadores. **Aceptación**: documentado en `progress/impl_04-multivendor.md`. Cubre: RMV5.9, RMV2.7 (montaje).
...
- [ ] T-MV.7.1 — Suite node:test del delta (`driver-registry.test.ts`, `selection-priority.test.ts`, `adapter-simulator.test.ts`, `demo-gate.test.ts`, `connection-view.test.ts`, `adapter-spp-android.test.ts`) enganchada en `scripts/run-tests.mjs`; `node scripts/check.mjs` verde end-to-end. Cubre: RMV1, RMV2, RMV3 (puro), RMV4, RMV5 (puro).
