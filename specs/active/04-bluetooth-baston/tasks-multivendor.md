...
| MV.5 — `adapter-spp-android` (RMV5) | ✅ **código + tests + dep nativa instalada + montado en Android** (2026-07-29) + **stream/dedup/reconexión con el emulador ESP32** (T-MV.5.7, pendiente de flasheo) | solo las **idiosincrasias** del RS420 físico (T-MV.5.6) | — |
...
- [ ] T-MV.5.7 — **Prueba con el emulador ESP32** (`firmware/baston-emulator/`, `MODO_SPP`) — *NO gated por hardware: el ESP32 ya está en la mesa*. Con el dev build Android + el emulador emparejado (PIN `1234`): (a) `read`/`seq on` → stream de lecturas distintas ingeridas; (b) `same 5` → **una sola** ingesta (dedup por ventana); (c) `burst 8` → nada perdido ni duplicado; (d) `drop` → `disconnected` + backoff + reconexión; (e) `off 8000` → la cadena de reintentos **sobrevive al primer fallo** (regresión del bug 3 de `dad711f`); (f) `flap 4 3000` → backoff creciente sin trabarse ni duplicar suscripciones; (g) `mute 30` → sigue `connected` con cero ingestas; (h) las 10 `bad <caso>` → descartadas en silencio, sin crash ni tag inválido en la UI; (i) `split`/`double` → una y dos lecturas respectivamente; (j) `term lf` y `stx off` → mismo resultado. **Pendiente de flasheo** (lo corre Raf: a partir de ese momento el ESP32 deja de tener el firmware de la balanza, que está respaldado en `firmware/backup/`). **Aceptación**: resultado documentado en `progress/impl_baston-emulator-esp32.md` + `field-findings.md`. Cubre: RMV5.9 (la parte que el emulador destraba), RMV1.5/RMV2.8 (matching por nombre real anunciado: el emulador se anuncia `RS420-EMU`, que matchea el `deviceMatch` del driver).
