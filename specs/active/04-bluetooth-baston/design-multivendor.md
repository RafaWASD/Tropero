...
> **Reconciliación as-built (2026-08-06 — «el estado del bastón se dice UNA sola vez por pantalla»).** Toca RMV3.5. Disparador: Raf — *"siento que el pill de «Conectando…» o «Reconectando…» estorba demasiado en esa parte de la pantalla"*. La unidad se abrió para **mover** el indicador arriba a la derecha; lo que se entregó es la mitad que la medición sostiene (la supresión donde hay superficie propia) y la otra mitad quedó **BLOQUEADA con evidencia** (ver el bullet final). Detalle completo del barrido en `progress/impl_pill-arriba-derecha.md`.
>
> - **Supresión por RECLAMO, no por lista de rutas.** El indicador global se calla cuando la pantalla ENFOCADA ya muestra el estado del bastón. La señal la emite la superficie: `useStickStatusSurface(kind)` (`src/hooks/`), sobre un store observable de módulo (`src/services/ble/stick-status-surface.ts`, mismo patrón que `upload-rejections.ts`). Reclaman hoy: **`BleConnectionChip`** (`'header-chip'`, y solo cuando SE PINTA — sin transporte no renderiza, y reclamar sin pintar apagaría el chrome a cambio de nada) y **`StickConnectionScreen`** (`'screen-card'`). El indicador lee `useStickStatusSurfaceClaimed()`.
> - **El literal `pathname === '/baston'` SE FUE del indicador**, y no como cosmética: era la misma clase de `BLE_OWNED_ROUTES` que este delta ya había descartado a propósito para el scanner acotado (reconciliación de RMV3.1) — una propiedad de una pantalla escrita como string en otro archivo, que al mover o renombrar la ruta se rompe **en silencio** (el indicador vuelve a duplicar sobre la card y nada se pone rojo). Ahora la declara el dueño. El comportamiento observable en `/baston` es idéntico y lo sigue asertando `baston-multivendor.spec.ts` (c).
> - **Por qué FOCO y no montaje** (el bug que el mecanismo obvio habría introducido): las tabs visitadas quedan MONTADAS el resto de la sesión y las pantallas del stack quedan montadas al navegar encima. Con un `useEffect`, entrar UNA vez a "Animales" habría dejado el reclamo vivo → **el indicador global apagado en TODA la app, sin un solo síntoma**. Va con `useFocusEffect`, igual que el scanner acotado. Costo declarado: `useFocusEffect` corre en la fase pasiva, así que al navegar A una pantalla que reclama el indicador puede quedar visible **un frame**.
> - **`mas.tsx` NO reclama, y está declarado** (registro del guard): su fila "Dispositivos → Bastón" muestra el estado en vivo, pero es una fila de lista a media pantalla, no un indicador de pantalla, y el indicador global —anclado abajo— ni la tapa ni la repite en el mismo golpe de vista. **Si el chip se mueve arriba a la derecha, esta entrada pasa a `reclama: true`** (ahí sí quedarían dos "Conectado" en la misma pantalla).
> - **Los guards** (`src/services/ble/stick-status-surface.test.ts` **10 casos** + `stick-status-surface-guard.test.ts` **13 casos**, los dos en la lista explícita de `scripts/run-tests.mjs`): el primero fija las reglas del store (N reclamos simultáneos, liberación en cualquier orden, idempotencia, snapshot estable por identidad — sin eso `useSyncExternalStore` re-renderiza infinito). El segundo está escrito **sobre la ausencia**: la población son los call sites de `useBleConnectionStatus()` —la única fuente del estado en la app— y cada uno tiene que estar en el registro declarando si reclama y por qué; **(C)** el indicador no puede volver a gatear por ruta (`usePathname`/`useSegments`/`'/baston'`/`BLE_OWNED_ROUTES`); **(D)** el reclamo tiene que colgar de `useFocusEffect`; **(E)** nadie llama la primitiva imperativa salteándose el hook. **8 mutantes lanzados, 8 muertos.**
> - **La E2E que ningún guard estático puede dar** (`e2e/baston-indicador-unico.spec.ts`): con la conexión viva, (a) en "Más" el indicador se ve, (b) en "Animales" el chip informa y el indicador desaparece —con `getByText('Bastón conectado')` en **count 1**, que es el invariante real: *hay exactamente una voz*, no *no hay ninguna*—, y (c) **al volver a "Más" el indicador VUELVE**. (c) es el caso que caza el reclamo pegado: falsificado mutando el hook a `useEffect` → la E2E **falla exactamente ahí** (las demás aserciones pasan, o sea el bug habría entrado con todo lo demás en verde).
> - **SE MUDÓ ARRIBA A LA DERECHA, Y CAMBIÓ DE FORMA (segunda mitad de la unidad).** El primer pase de esta unidad reportó BLOQUEADO el movimiento: la esquina superior derecha es el slot que la app usa para la acción secundaria y está ocupada en la home (avatar `$avatar`, medido x=[353,393] y=[13,52] @412), en `/maniobra` (la ✕ que cierra el modal), en `mis-campos` ("+ Crear campo"), en `lote/[id]` en selección ("Todos/Ninguno") y en todo el flujo de manga (`SpikeIdentityHeader`: pill "Saltear ‹maniobra›" + el "⋮" de saltar animal, ahí **a propósito por Fitts**; medido "⋮" en x=[363,368]). La premisa que motivaba el movimiento —*"los títulos van a la izquierda, así que la esquina está libre"*— es **cierta para los títulos** (todos truncan con `flex:1`/`numberOfLines`) y **falsa para la esquina**. Raf resolvió el bloqueo con una forma que no existía en las opciones: **círculo permanente que se estira a pill al cambiar el estado**, anclado **DEBAJO** de la fila del header. As-built:
>   1. **Anclaje**: `top = insets.top + $3*2 + $avatar`, `right = $4`, contenedor a ancho completo con `alignItems="flex-end"`. El alto de la fila del header se **deriva** (su `paddingVertical` `$3` ×2 + el elemento más alto que vive en ella, `$avatar` = 66), así que el indicador la despeja por construcción y sigue solo cualquier cambio de esos tokens. Lo verifica con números `(F-banda)` del guard, en las **cuatro reservas superiores reales** (web 0 · Android 24 · notch 47 · Dynamic Island 59), incluyendo que el anclaje **dependa** de la reserva (un número fijo se rompe en el teléfono con notch → rojo).
>   2. **Las dos formas**: círculo de `$navIcon + $2*2 + borde` = 40 (derivado del contenido: el indicador **no es un target**, así que su tamaño no sale de `$chipMin`) ↔ pill con el texto corto de `connectionRowStatus` (el mismo vocabulario del trailing de la fila de "Más", sin inventar un cuarto). El estirado es `withTiming` 220 ms sobre el ancho (reanimated, el idiom del repo), con el ancho natural **medido** por `onLayout` (el texto cambia con el estado). **Reduce-motion**: sin recorrido, salta al valor final — la app ya respeta esa preferencia en `Skeleton`, y el hook se extrajo a `hooks/useReduceMotion.ts` (la copia inline de `useSkeletonPulse` queda declarada, no migrada).
>   3. **El CUÁNDO, puro y testeado** (`features/ble-stick/indicator-morph.ts`): se anuncia por **clase de noticia**, no por `ConnectionStatus` crudo — `connecting` y `scanning` son la MISMA clase, así que el ciclo del backoff de reconexión (minutos) es **un solo aviso** en vez de un parpadeo permanente; y una misma clase no se repite antes de 8 s (amortiguador del link que titila, el `flap` del banco). 8 casos en `node:test`, con reloj inyectado.
>   4. **El estado no viaja solo por color**: lo lleva el ÍCONO (`iconFor`) y el color refuerza (WCAG 1.4.1 + ~8 % de daltonismo rojo-verde en usuarios mayoritariamente varones). Lo fija `(D-color)` del guard: si el mapeo `status → ícono` deja de discriminar, rojo.
>   5. **Donde la banda no está libre, la pantalla la RECLAMA** (`useStickStatusSurface('screen-band')`, misma señal que la supresión por redundancia, otra clase): `GroupViewScreen` (buscador a ancho completo bajo un header de un solo chevron: input y=[58,108] vs indicador y=[66,106]), `SpikeIdentityHeader` (chip "Animal N" en x=[317,393] y=[91,112]) y `(tabs)/reportes` (el selector de rodeo — lo encontró el sondeo E2E, no la lectura). Cada entrada del registro del guard lleva su medición.
>   6. **La banda de abajo quedó VACÍA y el guard sigue en pie**: `BOTTOM_BAND_TENANTS_EXPECTED = 0` declarado, `(B-banda)` compara la población real contra ese número (agregar un inquilino sin declararlo → rojo) y **ejercita el medidor contra un inquilino sintético** en cada corrida (uno que despeja y otro anclado al pico del FAB), así que "no hay a quién mirar" y "el resolvedor se rompió" dejaron de verse igual. El modelo puro renombró `stickPillBand` → `bottomAnchoredBand` y sus campos a `tenant*`: el invariante no es del pill, es de la banda.
>   7. **Dos defectos los encontró la CAPTURA, no un test** (y por eso ahora hay aserción): con el contenedor anclado `right={$4}`, la pill se estiraba **hacia afuera de la pantalla** ("Conectado" cortado contra el borde) porque el contenedor tomaba el ancho del hijo colapsado; y el texto interno se **comprimía** en vez de quedar tapado (medido: la fila reportaba 84 px donde el contenido pide 113) hasta ponerle `flexShrink={0}` a la fila **y** al texto. La red que quedó: la E2E espera a que el estirado TERMINE y exige `scrollWidth <= clientWidth` (nada recortado) + borde derecho dentro del margen.
>   9. **LA MEDICIÓN QUE ESTABA BIEN HECHA Y ERA UNA CONCLUSIÓN FALSA (2026-08-07).** El sondeo dio la banda LIBRE en la home… con un fixture cuyo usuario se llama **"E2E"**. El saludo de la home crece con el nombre (hasta `NAME_MAX_LENGTH`), y medido con nombres reales: con **14 caracteres** el primer renglón llega a **x=355** contra una banda que arranca en **x=354** → pasaba por debajo del indicador. (Curiosidad útil: el peor caso NO es el nombre más largo — con 80 el texto envuelve y sus renglones caen por debajo de la banda; el que choca es el que llega justo antes de envolver, ~13-20 caracteres.) **Arreglo: la home RESERVA** (`paddingRight={stickIndicatorBandReserve()}` en el saludo, con el número derivado del mismo token que dimensiona el círculo), en vez de truncar un nombre propio o de que la home reclame la banda y el chrome quede mudo justo donde el operario abre la app. **Falsificado con el mutante "sacarle la reserva a la home": se ponen rojos los TRES detectores** — la spec de peor caso, el sondeo (cuyo fixture pasó a un nombre de 16) y el guard estático `(F-reserva)`. El criterio quedó escrito: **la banda se verifica contra el RANGO de lo que un texto puede medir, no contra el valor del fixture**; el tope sale de `NAME_MAX_LENGTH` importado, no copiado.
>   8. **El sondeo de la banda** (`e2e/baston-indicador-unico.spec.ts`): en cada pantalla visitada lista lo que quede pintado bajo el círculo — controles por su caja, textos por sus **renglones reales** (`Range.getClientRects`, porque la caja de un `<Text>` a ancho completo con las letras a la izquierda daba falsos positivos) y con **hit-test** (`elementFromPoint`) para descartar pantallas MONTADAS pero no visibles (react-navigation deja la tab anterior en el árbol: su buscador se reportaba estando en otra pantalla). Auto-falsificado in-place: se inyecta un texto dentro de la caja y se exige que el sondeo lo vea.
