...
>
> **RECONCILIACIÓN 2026-07-30 (🟡-4 del review de `dad711f`).** Esta fase se construyó en el delta multivendor y en las dos unidades del bastón Android (`dad711f` + los bloqueantes del 2026-07-30), pero sus tasks se quedaron todas en `[ ]` — el T10 de reconciliación de aquella pasada cerró sobre los archivos `*-multivendor.md` y dejó afuera este `tasks.md`. Se marcan las que **están hechas y verificadas**: **T4.0** (veto CERRADO = COMPATIBLE, con build Gradle real — ver T-MV.5.1), **T4.1** (el adapter abre el RFCOMM y su stream se verificó **en device** contra el emulador ESP32: la app ingirió la trama de campo byte por byte, banco §2), **T4.5** (permisos as-built: solo `BLUETOOTH_CONNECT` en API ≥31, y `ACCESS_FINE_LOCATION` **topeado** a `maxSdkVersion=30` en el config plugin propio — este camino no descubre; ver la nota de RMV5.8) y **T4.6** (montado, con el guard de `isSppNativeAvailable()`).
>
> **T4.3** también pasó a `[x]` en la **segunda pasada** del 2026-07-30: R6.4 (reconexión automática al abrir la app) estaba sin implementar y sin un solo test; Raf lo decidió (*"que se reconecte sola al abrir, sí"*) y se construyó con la regla "el arranque no pide nada". Ver su nota.
>
> Las que siguen abiertas y **por qué**: **T4.2** (el pairing programático se eliminó a propósito — `pairDevice()` no resuelve nunca sobre un device ya emparejado; el emparejamiento lo hace Android y la pantalla lo dice con el PIN. La parte de "listar y elegir" **sí** está hecha, en RMV3.2); **T4.4** (dos RS420 cerca: necesita dos lectores); **T4.7** (QA con el RS420 físico — sigue GATED, ver T-MV.5.6/T-MV.7.3).

- [x] T4.0 — **Vetar** la compatibilidad del config plugin de `react-native-bluetooth-classic` con Expo SDK 56 (o prebuild manual) + estado de mantenimiento de la lib (Pendientes del context). Si incompatible, **PARAR y reportar al leader**. **Aceptación**: dev build documentado como viable o bloqueante. Cubre: R12.2 (prerrequisito).
- [x] T4.1 — `adapter-spp-android.ts` — abre RFCOMM SPP (UUID `SPP_UUID`) con `react-native-bluetooth-classic`, lee líneas ASCII → `ingestRawLine` (reusa parser, R6.5); baud-independiente (R6.8). **Aceptación**: stream del RS420 real parsea EIDs en el Android de pruebas. Cubre: R6.1, R6.5, R6.8.
...
- [x] T4.3 — `remembered-device.ts` — persistir el bastón elegido (R6.3, sobrevive reinicios); reconexión automática con backoff al abrir/volver a rango (R6.4); foreground-only (R6.9). **Aceptación**: tras elegir, reconecta solo sin volver a la pantalla. Cubre: R6.3, R6.4, R6.9.
  **[as-built 2026-07-30 (segunda pasada de la unidad de bloqueantes).** Las tres partes: persistencia (ya estaba), *"vuelve a rango"* (la cadena de reintentos, arreglada en la primera pasada) y *"al abrir"* (**esto**: `SppAndroidAdapter.autoConnect()`, que el provider llama una vez al montar el transporte). Decisión de Raf: *"que se reconecte sola al abrir, sí"*. **La regla que el enunciado no decía y es la que importa: el arranque NO PIDE NADA** — sin device recordado no toca la radio; el permiso se CONSULTA (`PermissionsAndroid.check`) y no se pide; con el Bluetooth apagado no muestra el diálogo de activar; y un gate que no pasa deja el estado en `'off'` (nunca se intentó) con el motivo en el log, en vez de un `'disconnected'` mentiroso. Detalle completo en la nota de reconciliación de R6.4. **13 casos nuevos** en `adapter-spp-android.test.ts` (el review había marcado que R6.4 no tenía ninguno, y no podía tenerlo) + 2 guards en `wiring.test.ts` (que el provider LLAME a `autoConnect`, y que los otros cuatro adapters NO lo implementen — con el motivo de cada uno).]**
...
- [x] T4.5 — Permisos Android (R12.1): 12+ `BLUETOOTH_SCAN`/`BLUETOOTH_CONNECT` (`neverForLocation`); <12 `BLUETOOTH`/`BLUETOOTH_ADMIN` + location; config en `app.json`/plugin (R12.2). **Aceptación**: permisos solicitados por versión; denegado → CTA + manual operativa. Cubre: R12.1, R12.2.
- [x] T4.6 — Montar `adapter-spp-android` en el provider para Android device (R10.3). **Aceptación**: en Android device, el provider usa SPP; en web sigue usando web-serial. Cubre: R10.3 (Android).
...
- [ ] T5.0 — **GATE FÍSICO (BLOQUEANTE de esta fase)** — conseguir un lector HID-capable (test rig: AgriEID BT Ultra importado, o un genérico AR —Montetech ME-BL01 / Smart LFID— **si se verifica que hace HID**) y validar en **iPhone real**: (a) tipea los 15 dígitos completos, (b) emite terminador Enter, (c) la supresión del teclado en pantalla de iOS no rompe la UX de manga, (d) el `TextInput` de RN con foco programático captura confiable entre versiones. Repetir en Android. Si falla, **NO implementar el adaptador**; reevaluar el camino iOS-abierto (el contrato sobrevive). **Aceptación**: resultado del gate documentado en `field-findings.md` + `progress/impl_04-bluetooth-baston.md`. Cubre: R8.7 (gate).
  **[RECONCILIACIÓN 2026-07-29 — el test rig YA EXISTE y no hubo que comprarlo.** El enunciado exigía *"conseguir un lector HID-capable (AgriEID BT Ultra importado, o un genérico AR si se verifica que hace HID)"*, que era el bloqueo real de este gate (USD 595+ sin canal AR). El ESP32 del bridge de la balanza se reprogramó como emulador de bastones y su `MODO_HID` **es un teclado BLE HID de verdad** (`BLEHIDDevice`, bonding just-works, tipea el EID + Enter con demora configurable entre teclas): sirve para correr (a), (b), (c) y (d) en un iPhone real y en Android, que es todo lo que este gate pide del lado nuestro. Lo que el emulador NO decide es **qué tipea exactamente un lector comercial** (prefijos, Tab vs Enter, su propia supresión de teclado) — eso sigue necesitando el device real, pero ya no bloquea arrancar. Ver `firmware/baston-emulator/README.md`.]**
...
>
> **RECONCILIACIÓN 2026-07-30.** La fase dejó de ser tentativa: **T6.1** (`StickConnectionScreen`, en "Más" por ADR-018 — la ruta existe y es alcanzable por deep-link; la FILA de "Más" que navega ahí sigue sin cablear, la agrega quien sea dueño de `mas.tsx`), **T6.2** (estados con CTA, en `connection-view.ts`, puros y testeados) y **T6.3** (`StickStatusIndicator` en el chrome) están construidas y pasaron Gate 2.5 en el delta multivendor. **T6.4** (toggle de beep) y **T6.5** (fallback manual en 1 tap ante desconexión, como UI explícita) seguían abiertas. **T6.4 se cerró el 2026-08-06** (unidad 🟡-11, ver T7.6): la tarjeta «Aviso de lectura» vive DENTRO de `/baston`, no en "Más". **T6.5 sigue abierta.**
>
> Nota de la unidad de bloqueantes (2026-07-30): la pantalla ahora toma la **propiedad exclusiva** del bastón mientras está enfocada, así que un bastonazo en `/baston` no abre el sheet global encima (BENCH-3, ver la nota de RMV3.1).

- [x] T6.1 — `StickConnectionScreen.tsx` en la sección "Más" (ADR-018), **específica por adaptador**: SPP (listar/elegir/olvidar — R6.2/R6.6), web-serial (`requestPort` + lista `getPorts` — R5.2/R5.4), HID (instrucción de parear en el SO + campo de scan — GATED). Cubre: R9.1.
- [x] T6.2 — Estados de conexión con CTA (R9.2): apagado / permiso denegado / buscando / conectado / desconectado; reconexión automática con backoff donde aplique (R9.5); todos no bloqueantes (R9.6). Cubre: R9.2, R9.5, R9.6.
- [x] T6.3 — `StickStatusIndicator.tsx` — indicador global en el chrome, alimentado por `useBleConnectionStatus()` (R9.3). Cubre: R9.3.
- [x] T6.4 — Toggle de **sonido** de lectura, dentro de `/baston` (lee/escribe `feedback-pref.ts`, R4.3). Cubre: R4.3 (UI). **AS-BUILT 2026-08-06** (unidad 🟡-11): tarjeta «Aviso de lectura» en `StickConnectionScreen`, después de "Lecturas" — copy puro en `feedbackPrefView()` (`connection-view.ts`), toggle con la anatomía canónica del repo, fila entera tappable ≥`$touchMin`, `switchA11y`. **NO va en "Más"**: es la casa del bastón y la única pantalla donde el operario puede probar el cambio en el acto. Hasta esta fecha `writeBeepEnabled` **no tenía un solo call site** → R4.3 estaba incumplido por ausencia de UI.
...
| R4.1, R4.2, R4.5 | T1.5, **T7.1**, **T7.6** | OK — canal real en device desde T7.6 |
| R4.3, R4.4 | T1.4, **T6.4**, **T7.6** | OK (UI construida 2026-08-06) |
| **R4.8, R4.9** (nuevos) | **T7.6** | OK |
...
- [x] **T7.6 (🟡-11 + 🟡-12)** — El bastón **suena y vibra de verdad** en la manga, "no sirvió" tiene señal propia, y la preferencia sale del camino caliente. Cubre: **R4.1 (reconciliado), R4.2 (reconciliado), R4.3 (UI, cierra T6.4), R4.8, R4.9, R4.5, R4.7 (guard ampliado)**.
  - **EL BUG**: en device el feedback era **solo** `Vibration.vibrate(50)` — el beep nativo era un **no-op declarado** (`playFeedback` exigía `beepChannel === 'web-audio'`), `writeBeepEnabled` **no tenía un solo call site** (R4.3 incumplido por ausencia de UI), y `readBeepEnabled()` cruzaba a `SecureStore` **por bastonazo** para alimentar un flag que en device no hacía nada. El peón mira al ANIMAL, no a la pantalla: 50 ms con guante, a pleno sol y con el teléfono en el bolsillo se pierden, y la redundancia sensorial que pedía `context.md` (decisión 3) no existía.
  - **DEPENDENCIAS**: `expo-haptics ~56.0.3` + `expo-audio ~56.0.13` (vía `expo install`, alineadas al SDK 56). El motivo que el código daba para NO tenerlas ("superficie de postinstall, ADR-011") es **falso** y se verificó ejecutando `npm view … scripts dependencies`: cero deps, cero postinstall. Corregido en `utils/haptics.ts` y `feedback.ts`; ADR-011 y `CONTEXT/06` **no** repetían el motivo (grep), y **ADR-013 §Capa 4 ya listaba `expo-haptics` en el stack**. El config plugin de `expo-audio` **NO se engancha** (evita `RECORD_AUDIO` + micrófono + background audio) — fijado por `app.config.test.ts`.
  - **GUARDS**: el punto único (R4.7) se sostiene con **tres capas**: nombres (`SENSORY_EMIT` en `read-dispatch.test.ts`), **MÓDULOS app-wide con tabla de dueños** (`SENSORY_OWNERS` en `feedback-guard.test.ts` — el alcance acotado a `services/ble/**` se probó insuficiente: una indirección un directorio afuera lo esquivaba con 2837 tests en verde), y assets. R4.9 se sostiene con **observación** (la E2E cuenta los accesos reales al storage en 10 bastonazos y exige cero) + **allowlist de lo invocable** en `handleReading` + tres reglas de forma.
  - **AS-BUILT**: ver `design.md` §"AS-BUILT del feedback sensorial". Resumen: un solo `playFeedback(outcome, beepEnabled)` para los tres desenlaces, aguas abajo del gate de `read-dispatch`; `classifyReadOutcome` + `decideFeedback(platform, beep, outcome)` puros; assets propios generados por `scripts/gen-baston-sounds.mjs`; caché en memoria de la preferencia (`beep-pref-cache.ts`) con árbitro de la carrera lectura-en-vuelo vs. toque; tarjeta «Aviso de lectura» en `/baston`.
  - **DECISIÓN — el silencio de los otros desenlaces se conserva y está fundamentado** en R4.8 (re-lectura, bastón mudo, gatillo mal apretado, `drop_no_consumer`). Solo `eid_rejected` recibe señal propia: es el único caso en que sabemos que llegó algo y no sirvió.
  - **CAMBIA EL FINGERPRINT DEL BUILD**: las dos deps traen módulo nativo → hace falta un **build de EAS nuevo** para verlo en device. Mientras tanto el APK instalado cae al respaldo (`Vibration`) y el audio queda apagado sin crashear (todo el acceso a los módulos es `require()` perezoso dentro de un `try` → también OTA-safe). ⚠️ **El respaldo táctil estuvo MUERTO en la primera pasada** (🔴 del review) — ver la corrección bajo R4.1 en `requirements.md`.
  - _Verificación_: `feedback.test.ts` (decisión pura por desenlace × plataforma × preferencia, con el invariante "la preferencia nunca toca la háptica"), `beep-pref-cache.test.ts` (invalidación + la carrera), `feedback-guard.test.ts` (módulos sensoriales + assets: existen, son WAV PCM mono, **no son silencio**, y el negativo **no es una copia** del positivo), `read-dispatch.test.ts` (patrón `SENSORY_EMIT` ampliado a `play[A-Z]\w*` + las APIs de los módulos nuevos, allowlist del provider por TOKEN), `connection-view.test.ts` (copy), `app.config.test.ts` (sin micrófono). E2E `app/e2e/baston-feedback-sensorial.spec.ts`: el oráculo pasa de "¿sonó?" a **"¿QUÉ sonó?"** (graba las frecuencias de los osciladores) → aceptada `[3150]`, rechazada `[1300,850]`, duplicada `[]`; + el switch de punta a punta (silencia sin romper la ingesta, persiste al reload, vuelve a prender). **Falsificado con 25 mutantes, 25 muertos** (18 propios + los 7 que el reviewer dejó VIVOS en la primera pasada): incluye `playRejectFeedback()` (que el patrón viejo no veía), copiar un .wav sobre el otro, el respaldo táctil inalcanzable, romper el orden `seekTo`→`play`, devolver `readBeepEnabled()` al camino caliente, y la indirección `src/utils/manga-buzz.ts` un directorio afuera del barrido.
  - Capturas del Gate 2.5: `app/e2e/captures/baston-feedback-sensorial.capture.ts` — **6 capturas = 4 frames distintos + 2 pares idénticos A PROPÓSITO** (apagado == apagado-tras-recargar es LA prueba de la persistencia; aceptada == rechazada **es el hallazgo 🟡-12**).
  - **Queda para el device (no verificable en CI ni en web)**: que el pip de 3150 Hz se oiga de verdad sobre el ruido de una manga real y que los dos patrones hápticos se distingan con guante. Es T7.4 (QA de campo).
