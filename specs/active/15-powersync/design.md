...
>   - **ACOTADO por la feature 20 (2026-07-20).** Sigue vigente como deuda GENERAL —la app no tiene ni una watched query; toda la reactividad es una emulación manual sobre la señal de status—, pero **ya NO aplica a `EstablishmentContext`, `RodeoContext` ni `lotes.tsx`**: esos tres re-leen ahora en CADA avance de sync (`useStatus()` + `lastSyncedAt.getTime()` como dep primitiva, el patrón canónico que ya usaban `ProfileContext`, `animales.tsx`, `(tabs)/index.tsx`, `useGroupView`, `useManeuverGating` y `mas.tsx`). Los dos contextos tenían la variante ROTA del patrón (latch de un solo disparo `lastHasSynced`, más un segundo candado `isWaitingRef` en el de rodeos) y `lotes.tsx` era mount-only. Migrar de verdad a `useQuery`/`db.watch` sigue siendo una decisión de arquitectura aparte (ADR), explícitamente fuera del alcance de la 20.
