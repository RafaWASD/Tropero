...
sync-streams/
  mitropero.yaml         → definición de las sync streams (deploy a la instancia, gateado por Gate 1)
```
...
> Fuente de verdad as-built: **`sync-streams/mitropero.yaml` (V3 JOIN-FREE, paso 1, validado en vivo + Gate-1-PASS 2026-06-09)**. **NO se deploya antes de Gate 1 PASS** (regla de cierre). `auth.user_id()` = `sub` del JWT Supabase. El predicado de scoping reusa la MISMA lógica que la RLS as-built (`has_role_in`, `is_owner_of`, `establishment_of_profile`), traducida a SQL inline porque la stream no puede llamar funciones SECURITY DEFINER del schema (corre del lado de PowerSync sobre el WAL).
...
> **As-built (reconciliación, Run backlog-flake-repro 2026-06-11 — estado reproductivo vigente DETERMINÍSTICO el mismo día):** dos cambios al camino de eventos repro para cerrar el flake del estado reproductivo (backlog 2026-06-11: tacto + parto/aborto el MISMO `event_date`, columna `date` sin hora → el desempate caía al `eventId` UUID random ⇒ ~50/50 "Preñada"/"Vacía"):
>   1. **`created_at` de CLIENTE en los INSERT CRUD-plano de `reproductive_events`** (tacto/service/abortion en `events.ts`/`local-reads.ts`): a diferencia del resto de las tablas de evento (donde created_at queda al `default now()` del server, ver el bullet de `created_by`/etc.), los reproductivos SÍ setean `created_at = new Date().toISOString()` al insertar. Server-side `reproductive_events.created_at` es `default now()` SIN trigger de force (migración 0026) → el valor del cliente **PERSISTE al subir**. Es semánticamente correcto y deseable: es el instante de CREACIÓN en el dispositivo (fiel al orden de creación), mejor que el now() de SUBIDA para un evento cargado offline. `created_by`/`establishment_id` los sigue forzando el trigger (no se tocan). Efecto: TODOS los determinantes repro (tacto/service/abortion CRUD-plano + el parto del overlay `pending_reproductive_events`, que ya traía un created_at de cliente) tienen un instante real de creación → el estado repro se desempata por created_at sin ambigüedad por NULL.
>   2. **`buildTimelineQuery` ordena `event_date ASC, created_at IS NULL ASC, created_at ASC`** (antes `event_date DESC`, solo cosmético — el orden VISUAL lo hace siempre el cliente con `parseTimeline`), en un SELECT EXTERNO que envuelve el UNION (en un compound SQLite no acepta expresiones en el ORDER BY). Ese orden de lectura es la fuente del `seq` (índice de fila) que `fetchTimeline` asigna a cada `TimelineItem`: proxy fiel del orden de inserción local. `deriveCurrentState`/`isNewerRepro` y el desempate final de `parseTimeline` usan ese `seq` —en vez del `eventId` UUID random— como desempate estable a igualdad de `event_date`. Con (1) el caso realista es "ambos created_at presentes" → el seq sale del orden por created_at = orden de creación (el insertado después gana); el `created_at IS NULL ASC` (NULL al final = recién insertado = más reciente) y el fallback por created_at en `isNewerRepro` cubren el caso defensivo de una fila sin sellar.
>
> Resultado: el parto/aborto del mismo día que el tacto, creado DESPUÉS, GANA siempre ("Vacía") — DETERMINÍSTICO (e2e `events.spec.ts` verde con `--repeat-each`). NO toca schema/RLS/migraciones (frontend puro). Detalle: `progress/impl_backlog-flake-repro.md`.
...
>   > **ACTUALIZACIÓN (la "reactividad diferida" se está entregando incremental):** el patrón NO es `db.watch(sql)` sino **`db.onChange(handler,{tables})`** (imperativo) + **`useQuery`** (listas) — ver **ADR-030**. Migrados: `EstablishmentContext`/`RodeoContext`/`lotes.tsx` (feature 21) + `useManeuverGating` config/maniobra (feature 22). Además la feature 22 descubrió y arregló que en **nativo la conexión de descarga no reenganchaba** (deuda de conexión que este design no contemplaba) → **ADR-031** (liveness). Quedan por migrar: `miembros`, `use-reports`, `animal/[id]`, `export-sigsa`.
>   - **ACOTADO por la feature 20 (2026-07-20), luego RESUELTO para 3 consumidores por la feature 21 (2026-07-21).** Sigue vigente como deuda GENERAL —el RESTO de la app no tiene watched queries; su reactividad es una emulación manual sobre la señal de status—, pero **los 3 consumidores de la feature 20 (`EstablishmentContext`, `RodeoContext`, `lotes.tsx`) ya están MIGRADOS a watched queries REALES de PowerSync** (feature 21, ADR-030, R21.31): los 2 contextos usan `db.onChange(handler, { tables })` (watched query imperativa que re-corre su resolución existente ante el cambio de tabla del SQLite local) y `lotes.tsx` usa `useQuery(sql, args, { rowComparator })`. Esto REEMPLAZA el disparador intermedio de la 20 (`useStatus()` + `lastSyncedAt.getTime()` como dep primitiva), que era un proxy NO determinista (la señal `lastSyncedAt` se congelaba ~90 s+ detrás de la llegada del dato). La migración a watched queries del **resto** de la app es incremental (ADR-030) y sigue pendiente como deuda.
>   - **Lectura local nueva (feature 20):** `buildActiveRoleQuery(userId, establishmentId)` → `SELECT active FROM user_roles WHERE user_id = ? AND establishment_id = ? LIMIT 1`, consumida por `hasActiveLocalRole` (`services/establishments.ts`) vía `runLocalQuerySingle` con `emptyIsSyncing:false`. Es la **evidencia afirmativa** con la que el cliente decide si el campo activo se perdió, en vez de inferirlo de una ausencia. Se ejecuta SOLO cuando el campo activo no aparece en el set leído. Cero red.
>   - 🔒 **INVARIANTE DEL QUE DEPENDE LA FEATURE 20 — no filtrar `active` en `self_user_roles`.** La stream `self_user_roles` (`sync-streams/mitropero.yaml`) es `SELECT * FROM user_roles WHERE user_id = auth.user_id()`, **sin `org_scope` y sin `AND active = true`**. Esa ausencia es **load-bearing**: es lo único que hace que la fila de rol propia SOBREVIVA a una revocación en el SQLite local (con `active = 0`) cuando PowerSync ya borró el bucket del establecimiento. Agregarle el filtro `active` parece una optimización prolija y **rompe la feature 20 en silencio**: la fila desaparecería y el veredicto volvería a decidirse por ausencia → reaparecen los falsos `active_lost` por sync parcial que esa feature vino a eliminar (falla cerrada, no es fuga). El candado también está en el YAML, pegado a la stream. Ver `specs/active/20-reactividad-sync/design.md` §4.3.
...
- **Tests de no-bypass (R9.2)**: espejo de los runners RLS existentes (`supabase/tests/rls/`), pero validando el **contenido del sync set** por usuario (un usuario A no recibe filas del establecimiento de B; `user_private` de B no llega a A). Se ejecutan como integración Node contra la DB remota + la instancia (o simulando la query de la stream contra Postgres con el `user_id` de cada actor). **Caso obligatorio agregado por HIGH-1**: tras soft-deletear un establecimiento (UPDATE `deleted_at = now()`), un owner de ESE campo deja de recibir TODAS sus filas (animales, perfiles, eventos, sesiones, lotes, invitaciones, roles de coworkers) por el sync set — espeja R8.2.
  > **AS-BUILT (Run cierre-T7, T7.2 + T9.7):** la suite vive en `supabase/tests/sync_streams/run.cjs` (enganchada en `scripts/run-tests.mjs` → `check.mjs` la corre SIEMPRE). Tomó la 2da opción del párrafo: **simula el predicado de cada stream contra Postgres con el `user_id` de cada actor** (computa `org_scope`/`owner_scope` por actor con service_role — que bypassa RLS — y filtra cada tabla por `establishment_id IN scope`, replicando la query de la stream, NO la RLS). 25 subtests autocontenidos (2 campos/usuarios dedicados + un coworker en A): per-establishment (profiles/rodeos/lotes/establishments/animal_events), self-only (`user_private`/`user_roles`), owner-only (`est_members_roles`/`invitations`), catálogos globales, soft-delete (R4.5), el caso HIGH-1/R8.2 (soft-delete del campo → org_scope vacío vía trigger 0076), y el PASO 2 (las 5 tablas de evento + `birth_calves` + `rodeo_data_config` + `animal_category_history` denormalizadas; identidad b1; `animals`/`users` NO en ninguna stream; `member_name` c2). **MUTATION-PROBADA**: con una stream permisiva (sin el scope) 11 subtests cross-tenant FALLAN — los tests ejercitan la frontera real, no pasan triviales.
- **AS-BUILT del E2E del WRITE-PATH offline (R6.6/R6.8–R6.12, T7.3/T7.9/T7.8 in-vivo):** el camino offline outbox+overlay→RPC se ejercita END-TO-END contra el SQLite/PowerSync **real** del bundle web en `app/e2e/animals-offline.spec.ts` (8 tests). El principio rector es que **el oráculo SIEMPRE mira el SERVER** (helpers de `app/e2e/helpers/admin.ts` vía service_role: `waitForServerAnimalProfile`/`waitForServerWeightEvent`/`waitForServerBirth`/`waitForServerExit` + snapshots `getServerBirthState`/`getServerProfileStatus`), NO el overlay/UI — lección del bug de pérdida de datos invisible (backlog 2026-06-10), donde aseverar la UI (que muestra el overlay local) dejaba pasar altas que nunca aterrizaban. Cobertura: alta offline (tests 1+2, Run create-animal-rpc), evento simple offline (test 3, T7.3), **PARTO offline mono+mellizos** (overlay parto+N terneros → reconexión → 1 solo evento `birth` + N `birth_calves` server-side, NO duplicado — `birth_calves` es server-only ⇒ existe solo si la RPC corrió), **BAJA offline** (overlay oculta de la lista → reconexión → status egresado real), **ROLLBACK in-vivo** (precondición server-side rota mientras offline ⇒ soft-delete de la madre ⇒ `register_birth` levanta `23503` ⇒ `permanent_reject` ⇒ `rollbackOverlay`: el overlay se BORRA, NADA server-side, rechazo observable, no-loop) + **contraprueba transitorio** (sin red ⇒ overlay PERSISTE, intención en cola, drena al reconectar). Una desviación de mecanismo de TEST (no de producto): la lista lee one-shot (`getAll`, §5.1) ⇒ re-clickear la tab ya activa no re-dispara `useFocusEffect`; para ver el efecto de un cambio de overlay (p.ej. el rollback que limpia `pending_*`) el test fuerza un re-fetch rebotando por "Más" (`refreshAnimalesList`). Verificado que la aserción del rollback MUERDE: sin el re-fetch falla con `count=1`.
...
- **2026-06-08 — Reescritura del YAML de sync streams al patrón `with:`/INNER JOIN (fix runtime `[PSYNC_S2305]` too many buckets / regresión #611). FRONTERA DE AUTORIZACIÓN IDÉNTICA — solo cambió la ESTRUCTURA.** Al deployar las 26 streams (patrón canónico inline como subquery anidada) y conectar un cliente real, el server cerró el stream con `too many parameter query results / too many buckets (limit of 1000)`. Diagnóstico autoritativo (logs de la instancia): por la regresión powersync-service **#611**, una stream `auto_subscribe` con `IN (subquery)` **sin** bloque `with:` crea **un bucket por cada FILA de datos** de la subquery → ~22 streams per-est × ~100 establecimientos contaminados en la beta → ~2200 buckets > 1000 → 0 bytes + loop de reconexión. **Gate 1 había validado la autorización (correcta); el patrón de sintaxis revienta el límite operativo.** Reescrito sin tocar backend ni la frontera de read-authz:
  ...
  - **`sync-streams/mitropero.yaml`**: reescrito byte-faithful al §2 nuevo (mirror), con el header explicando #611 + el patrón `with:`/JOIN + las incertidumbres flaggeadas.
