...
- [x] **T1 — `app/src/services/powersync/local-reads.ts`: `buildActiveRoleQuery`.**
...
- [x] **T2 — `app/src/services/establishments.ts`: `hasActiveLocalRole`.**
...
- [x] **T3 — `app/src/utils/establishment.ts`: `assessDisappearance`.**
...
- [x] **T4 — `app/src/utils/establishment.ts`: `isManeuverRouteSegment`.**
  `isManeuverRouteSegment(segments: readonly string[]): boolean` → `segments[0] === 'maniobra'`. Puro, null-safe (array vacío → `false`).
  Cubre: R20.20, R20.24.

- [x] **T5 — `app/src/utils/establishment.ts`: `sameResolvedEstablishmentState`.**
...
- [x] **T7 — Reemplazar el latch por el patrón canónico.**
...
- [x] **T8 — Guarda E1 en `applyMemberships` (evidencia afirmativa).**
...
- [x] **T9 — Diferimiento D1 (revocación en caliente).**
...
- [x] **T10 — Guard de equivalencia antes de `setState`.**
...
- [x] **T11 — Reemplazar el latch + eliminar `isWaitingRef`.**
...
- [x] **T12 — `load` no tumba un estado resuelto en re-lecturas reactivas.**
...
- [x] **T13 — Evidencia afirmativa para `no_rodeos` + preservación del rodeo activo.**
...
- [x] **T14 — `app/app/lotes.tsx`: efecto reactivo silencioso.**
...
- [x] **T15 — `app/app/campo-perdido.tsx`: copy verdadero para ambas causas (E5).**
...
> **As-built (remediación 2026-07-20) — eventual-consistency: forzador + `retries: 2` (ambos honestos).**
> El diagnóstico A/B mostró que `lastSyncedAt` avanza de forma NO determinista por cambio (design
> §10-bis (g)). El archivo lleva `retries: 2` (herramienta estándar y honesta para E2E de
> eventual-consistency — el reviewer los objetó bajo el diagnóstico VIEJO/errado; ver el header del spec)
> **más** un forzador/timeout que BAJA la frecuencia de retry (todo escenario REAL de manga: la señal se
> corta y vuelve; la app sigue montada, no es reload), según el tipo de cambio:
> - **ADICIONES** (T17 campo / T18 rodeo / T19 lote) → `syncUntil` (blip-poll: `context.setOffline`
>   off→on → PowerSync reconecta y completa un checkpoint fresco → la señal avanza y la fila ya presente
>   en el servidor baja). El blip FUERZA el checkpoint que una adición no siempre dispara.
> - **REVOCACIONES** (T20 / T21) → tick NATURAL con timeout generoso, SIN blip. Una remoción de bucket
>   propaga por la conexión ESTABLE; un blip la DISRUPTA (medido). **T20** además usa `revokeSession:
>   false` (camino campo-borrado, indistinguible en la firma local — E5) porque revocar la sesión también
>   disrupta la propagación; la revocación de sesión + su ventana D1.2 las cubre **T21** (camino real
>   `revokeSession: true` + `assertServerSessionsRevoked`).

- [x] **T16 — Helpers de fixture en `app/e2e/helpers/admin.ts`.**
...
- [x] **T17 — `app/e2e/reactividad-sync.spec.ts` · caso 1: campo en caliente (EL criterio A8).**
...
- [x] **T18 — `reactividad-sync.spec.ts` · caso 2: rodeo en caliente (MULTI-CAMBIO).**
  Con la home montada: (a) `seedRodeo` server-side sobre el campo activo (INSERT) → aparece sin reiniciar; (b) renombrar OTRO rodeo vía service_role (UPDATE) → el nombre nuevo aparece y el viejo desaparece, sin reiniciar. Se asserta el **estado final combinado** (ambos reflejados) + el rodeo activo **no** cambia (R20.19).
  ⚠️ Reconciliado con la evidencia A/B (design §10-bis (g), progress/impl_20): el claim previo "un segundo cambio no se ve" era un **mal diagnóstico** — la fila SIEMPRE llega al SQLite en ~1,5 s y `lastSyncedAt` avanza, solo que NO de forma determinista por cambio. Un cambio POSTERIOR fuerza un checkpoint que barre lo pendiente, así que asertar el estado final combinado es MÁS robusto que un solo cambio (el rename empuja al alta si se hubiera estancado). El rename prueba además que la re-lectura refleja UPDATES, no solo INSERTS.
  Cubre: R20.2, R20.5, R20.19.

- [x] **T19 — `reactividad-sync.spec.ts` · caso 3: lote en caliente.**
...
- [x] **T20 — `reactividad-sync.spec.ts` · caso 4: revocación fuera de maniobra.**
  Usuario miembro de 2 campos, activo en A, parado en la home. `revokeMemberRole` sobre A (**`revokeSession: false`** — ver as-built abajo) → tick NATURAL con timeout generoso (sin blip) → sin reiniciar aparece `/campo-perdido`; el texto **no** afirma una causa única y **no** promete que se conservaron datos. "Entendido" → re-ruteo a los campos restantes.
  ⚠️ **As-built (natural, `revokeSession: false`)**: aísla la revocación del ROL (camino campo-borrado, indistinguible en la firma local — E5). NINGUNO de los requisitos que cubre este caso depende de que la sesión se revoque; el camino con sesión revocada + su ventana D1.2 lo cubre T21. **Dos motivos técnicos medidos**: (1) una revocación propaga por la conexión ESTABLE — un blip la DISRUPTA (con blip, T20 se estanca; sin blip, ~5 s), por eso NO usa el blip-forzador de las adiciones; (2) revocar la SESIÓN además disrupta la propagación, por eso `revokeSession: false`. El header del test lo declara (regla de T16).
  Cubre: R20.14, R20.23, R20.24, R20.26, R20.28.

- [x] **T21 — `reactividad-sync.spec.ts` · caso 5: revocación DURANTE la maniobra (D1 + riesgos 7 y 8).**
  ...
  4. ~~**Intentar el switch de campo durante el diferimiento**~~ → **RECONCILIADO AS-BUILT: NO alcanzable por E2E** (ver nota bajo R20.33/R20.34 en `requirements.md`). La señal de "hay maniobra" ES la ruta, y el switch vive en el header de la home, FUERA del flujo de maniobra → salir para llegar al switch es exactamente lo que dispara la emisión de la revocación diferida (R20.22). La ventana con un pendiente vivo + switch observable dura lo que una lectura local. R20.33 (invariante del merge) se verifica por **inspección** (el merge está en el único punto por el que pasan todas las aplicaciones de set); R20.34 (descarte al cambiar de campo) por **unit** (`shouldEmitDeferredRevocation` con `pendingId !== currentId` → no emite) + inspección de `switchEstablishment`.
  ...
  Cubre: R20.18, R20.20, R20.21, R20.22 (E2E); R20.33 (inspección), R20.34 (unit + inspección — no E2E).

- [x] **T22 — `reactividad-sync.spec.ts` · caso 6: offline puro intacto.**
...
- [x] **T24 — Reconciliación de specs (regla dura, ANTES de cerrar).**
  ...
  **(d) 🔒 Candado de la dependencia oculta (Gate 1 RE-GATE, hallazgo RG-1).** TODA la regla de evidencia afirmativa cuelga de que la stream `self_user_roles` **no tenga filtro `active`**: hoy es `SELECT * FROM user_roles WHERE user_id = auth.user_id()` (`sync-streams/mitropero.yaml:71-74`), sin `org_scope` y sin `AND active = true`, y por eso la fila propia sobrevive a la revocación con `active = 0`. Una "optimización" que agregue ese filtro **rompe la feature en silencio**: la fila desaparecería y el veredicto pasaría a `absent_or_inactive` siempre que el bucket se demore → vuelven los falsos `active_lost` que esta feature vino a evitar. Falla **cerrada** (no es fuga), pero es exactamente el modo de falla que estamos arreglando.
  Por eso el candado va **donde alguien edita**, no solo donde alguien especifica: **(d.1)** comentario en `sync-streams/mitropero.yaml`, pegado a la stream `self_user_roles`, diciendo que la ausencia del filtro `active` es **load-bearing** para la feature 20 y por qué; **(d.2)** la misma nota en `specs/active/15-powersync/design.md`, que es lo que se lee antes de tocar streams. El (d.1) es el que realmente protege: el YAML no se puede editar sin verlo.
  Cubre: trazabilidad del proceso + RG-1.

- [x] **T25 — Verificación verde**: unitarios confiables (`establishment.test.ts` + `local-reads.test.ts`) **238/0** + typecheck del cliente **0 errores** + la suite E2E de la feature verde en corridas completas reproducibles con **`retries: 2` + forzador/timeouts** (verde HONESTO de eventual-consistency — remediación 2026-07-20; el reviewer objetó retries bajo el diagnóstico VIEJO/errado, ahora son la herramienta estándar con el diagnóstico A/B corregido + `db.watch` flageado). Resultados de TODAS las corridas (sin cherry-pick) en `progress/impl_20-reactividad-sync.md`, junto con el mapa `R20.<n> → archivo:test`. (No se corre `node scripts/check.mjs` completo por la coordinación WIP=1 del leader + la carga compartida; los unitarios de la feature + la E2E son la verificación relevante, según el encargo de la remediación.)
  Cubre: todos.
