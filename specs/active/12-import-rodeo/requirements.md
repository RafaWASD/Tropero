...
**R10.7** *(As-built — visibilidad de categoría no reconocida, decisión Raf 2026-06-07.)* En el preview (R5.3), el sistema deberá **avisar** cuáles categorías declaradas en el archivo NO matchean el catálogo del system del rodeo (y por lo tanto van a quedar "a completar" por R10.5), para que el operador no se sorprenda al ver el resultado. El cliente carga los `code`s activos del catálogo del rodeo (legible por RLS, `0015`) y clasifica cada fila válida (`matched`/`unmatched`/`none`) con la MISMA normalización que se manda al RPC (`normalizeCategoryText`), espejando el match server-side (R10.3 — `code` exacto). El preview muestra (a) un aviso de tono precaución con los textos distintos no reconocidos + conteo de filas, y (b) un badge "· a completar" en cada fila válida cuya categoría no matchea. **Solo visibilidad client-side**: NO cambia el mapeo, el RPC, ni el catálogo. *(El mapeo de dominio de "Vaca" genérica y sinónimos queda DIFERIDO a Facundo — D3 / `CONTEXT/07-pendientes.md`. Degradación graciosa: si la query del catálogo falla o vuelve vacía, no se avisa y el preview sigue funcionando.)*
