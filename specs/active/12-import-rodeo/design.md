...
| `app/src/services/import-rodeo.ts` | service (I/O) | Pre-check de dedup contra existentes (queries a Supabase), escritura batch (split insert+select o RPC), insert de `import_log`. **Única capa que toca I/O.** | R7.2, R7.4, R8, R9, R11 |
