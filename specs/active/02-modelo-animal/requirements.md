...
**R6.2** El sistema deberá modelar la tabla `reproductive_events` con `(id, animal_profile_id, session_id, event_type, event_date, service_type, bull_id, semen_id, pregnancy_status, estimated_days, estimated_birth, calf_id, calf_weight, calf_sex, notes, created_by, created_at, deleted_at)`. `event_type` es enum: `service | tacto | birth | abortion | weaning | drying | rejection`. `pregnancy_status` enum: `empty | small | medium | large` → **vacía / chica (cola) / mediana (cuerpo) / grande (cabeza)** (ver `CONTEXT/03-flujos-maniobras.md` L53: "cabeza/cuerpo/cola equivale a grande/mediana/chica"). **Corrección 2026-06-03 (Raf, dominio Facundo)**: una versión previa de esta nota invertía el orden (`small`→cabeza, `large`→cola); el mapeo correcto es `small`=chica=**cola**, `large`=grande=**cabeza**, `medium`=mediana=cuerpo. La UI sigue la versión corregida.
...
> **Reconciliación as-built (2026-08-06, 🔴 A.5 del QA de maniobras en device) — "Estado actual" desempata igual en TODAS sus ramas.** La sección "Estado actual" de la ficha (el VALOR VIGENTE de cada medición tipada, derivado de la cronología por `deriveCurrentState`) tenía **dos** criterios de desempate para eventos del MISMO `event_date`: la rama reproductiva usaba `seq → created_at → eventId` y las ramas de **peso** y **condición corporal** saltaban directo a `eventId`, que es un **UUID v4 random** ⇒ con dos cargas del mismo día el vigente salía a cara o ceca (~50 %). Medido en el A07 sobre un animal repesado el mismo día: el peso acertó (318) y la condición mostró el valor VIEJO (2,25 en vez de 3,75). As-built: un único comparador `isNewerItem` (`event-timeline.ts`), con la escalera de la rama reproductiva, para las tres ramas. No hay criterio nuevo: se compartió el que ya estaba probado. El invariante lo cuida `src/utils/current-state-tiebreak-guard.test.ts`, que **deriva del código** la lista de ramas que `deriveCurrentState` maneja y corre la misma sonda sobre cada una (una rama nueva nace en rojo). Cubierto también por `e2e/qa-fixes-datos.spec.ts`.
...
**R14.9** (nueva, sesión 17 — baja/egreso desde la ficha · **UX aterrizada C3.3, 2026-06-07**) La pantalla deberá ofrecer una acción **"Dar de baja"** (egreso, `R4.14`) que permita elegir el motivo, capturar `exit_date` y, en la venta, opcionalmente `exit_weight` / `exit_price`, y confirmar. La acción solo deberá estar disponible cuando `status = 'active'` y el usuario sea `owner` del campo del animal **o** el operario que lo cargó (`created_by`, `R4.14`) — espejo del authz que enforça el RPC `exit_animal_profile`. Tras la baja, la ficha deberá mostrarse en modo "archivada" (badge con el motivo y la fecha de egreso) y ocultar las acciones de mutación (dar de baja / agregar evento), conservando la cronología. El animal sale de la tab Animales (filtro `status='active'`).

> **Aterrizaje de UX (C3.3, Gate 0 2026-06-07 — ver `context-c3.3-baja.md`)**. El design system ya está cerrado (A.1 done), así que R14.9 deja de ser TENTATIVA y baja a implementación:
> - **MVP expone 3 motivos** (no los 6 del enum `exit_reason`): `Venta → (sold, sale)`, `Muerte → (dead, death)`, `Transferencia → (transferred, transfer)`. Mapean 1:1 a `status`; una decisión por pantalla. Los otros 3 (`culling/theft/other`) quedan diferidos hasta validar su semántica de reporte con Facundo (el enum ya los soporta; agregarlos es cambio de UI, sin migración).
> - **Peso + precio** se capturan opcionales **solo en la baja por Venta** (analytics: precio/kg, ganancia de peso); ocultos en Muerte/Transferencia.
> - **No reversible desde la UI** en MVP (el RPC no tiene "reactivar"); confirmación destructiva clara.
> - La "Transferencia" de R14.9 es la **baja simple por egreso** (estado `transferred`), NO el re-parenting de historia entre campos del mismo usuario — eso es la feature 11.
> - **Online-only** con guard de conexión (offline = PowerSync/C5).
