...
| `alta-form-refinamiento` | alta: fecha DD/MM opcional separada del año + condición corporal stepper (comparte `ConditionScoreStepper` con la maniobra) + destildar opcionales (#3/#13/#14) — **frontend puro, sin migración** | done (Puerta 2, 2026-07-05) |
| `caravana-ficha` | ficha: asignar caravana electrónica (RPC existente) y visual/idv (UPDATE local, patrón CUT) desde la sección Identificación, solo lo vacío (R4.13); **+ bastoneo del EID desde la ficha (RCF.6): reusa el contrato de ingesta BLE + un "scanner acotado" exclusivo, con fallback manual** (#6) — **frontend puro, sin migración** | done (Puerta 2, 2026-07-07) |
...
| `parto-rodeo-caravana` | parto (`agregar-evento`): **rodeo del ternero** a nivel camada (picker preseleccionado al de la madre + leyenda, editable al mismo sistema → `calfRodeoId`, RPRC.1) + caravana visual del ternero (RPRC.2/3) (#4/#1a) — **frontend puro** (`register_birth` 6-arg ya deployado). RPRC.1 vigente; RPRC.2/3 (visual single-calf) **SUPERADAS por `parto-caravana-visual-por-ternero`** (visual por cría). Commit `ce51ab3` | done (Puerta 2, 2026-07-07) |
| `bastoneo-captura-alta-parto` | **generaliza el `TagScanSheet` (RCF.6) a modo CAPTURA**: el bastoneo del EID (con fallback manual dentro del sheet) se reusa en el **ALTA** (paso 4, `crear-animal`) y el **PARTO** (por ternero, `agregar-evento`) — donde el animal no existe todavía y el EID se captura al estado del form (sin RPC). Reemplaza el campo electrónico tipeable suelto por el CTA `TagScanCta` (compartido) + `CapturedTagRow` (read-only con "Cambiar", mis-scan corregible). **Frontend puro, sin migración** (Gate 1 N/A). Detalle en `design-caravana-ficha.md §10.6` | done (Puerta 2, 2026-07-10) |
| `identificadores-unificados` | modelo de **3 identificadores, TODOS opcionales**: electrónica (`tag_electronic`, 15 díg, única global), visual (`idv`, alfanum ≤15, única por campo), Nombre/Apodo (custom, ≤15, warning-soft por campo). **Elimina `visual_id_alt`** (4to, muerto): dropea la columna + el trigger de identidad (R4.2) + el fallback de parto; re-CREATE de `register_birth`/`create_animal`. Búsqueda unificada por los 3. Hero por apodo cuando el rodeo lo usa. Chip de lista "Sin electrónica" (ex "Sin caravana"). **CON BACKEND** (`0122`) + PowerSync. Ver `{context,requirements,design,tasks}-identificadores-unificados.md` | done (Puerta 2, 2026-07-09) |
| `combo-eje-central` | fix visual: el label de las **filas de combo/lista con tilde** ahora centra sobre el eje REAL de la fila en AMBOS estados (el ✓ ya NO lo corre a la izquierda). Componente compartido nuevo `app/src/components/ComboOptionRow.tsx` (slots simétricos: spacer izq. = slot del tilde der.) → reemplaza los combos duplicados de **parto** (`agregar-evento`, ex-`RodeoOptionRow`), **cría al pie** (`LinkCalfPrompt`, ex-`RodeoOptionRow`) y **lote del alta** (`crear-animal`, ex-`GroupOption`). El **picker de lote de maniobra** (spec 03, ex-`LoteOption` card bordeada) también pasa a usar `ComboOptionRow` (`size='comfortable'`) dentro de un contenedor bordeado → visualmente unificado con los combos inline (selección marcada SOLO por el tilde). `ComboOptionRow` expone `size: 'compact' | 'comfortable'` (default compact = listas inline; comfortable = sheets, tap target `$touchMin`) + `testID?`. Verificado por medición Playwright (ambas filas `textCenter == rowCenter`). **Frontend puro, sin migración. Nivel A (reconciliado in-place, sin delta-spec propio).** | done (2026-07-07) |
| `parto-caravana-visual-por-ternero` | la **caravana visual (idv)** del ternero al parto pasa a ser **POR CRÍA** (mellizos incluidos), simétrica con la electrónica (bastoneo). Cada `CalfBlock` gana su campo idv; se elimina el campo camada + la nota de mellizos. Ambas caravanas SIEMPRE opcionales (constraint de Raf). **CON BACKEND**: migración `0121` (`register_birth` computa el idv por cría con `coalesce(calf_idv elemento, p_calf_idv top-level)`; `p_calf_idv` conservado para cría al pie #15; fallback `visual_id_alt` both-null load-bearing por el trigger `animal_profiles_identity_check`). **SUPERA** RPRC.2.1/2.3/2.4 y RPRC.3.2/3.3. Gates: Gate 1 PASS · reviewer APPROVED · Gate 2 PASS 0 HIGH. Deployado + verificado (animal+SIGSA verdes, e2e 3/3, Gate 2.5 veto PASS). | done (deploy 2026-07-07) |
| `bastoneo-cria-al-pie` | **Run 2 del bastoneo** (RCF.6 generalizado): en la cría al pie (`LinkCalfPrompt`, fase ask) el CTA "Bastonear la caravana del ternero" abre el `TagScanSheet` en modo captura con `hideManualEntry`; `onScanSubmit(eid)` setea el query + `runSearch(eid)` → el EID **llena el buscador find-or-create** y avanza (no es captura pura: la cría al pie busca por EID **o** IDV). Completa la replicación del bastoneo en las **3 superficies** (alta/parto/cría al pie) + ficha. **Frontend puro, sin migración** (Gate 1 N/A). Nested sheet con scoped scanner exclusivo. Detalle en `design-cria-al-pie-alta.md §11` + `design-caravana-ficha.md §10.7` | done (Puerta 2, 2026-07-10) |
| `override-imputacion-categoria` | **Nivel A** (reconciliado in-place): en el alta year-only, en vez del midpoint ciego `AAAA-07-01`, `imputeBirthDateForCategory` (`animal-category.ts`, pura) imputa el midpoint del cruce [año ∩ ventana etaria de la categoría elegida ∩ pasado] → `compute_category(birth_date)` coincide con la elegida y el cron nocturno NO la flipea (`category_override=false`); cruce vacío o no age-derivable → fallback al midpoint ciego + pin. `categoryOverrideFor`/`compute*` intactos; toda la inteligencia vive en la fecha imputada. `validateBirthDate` ganó `precision` ('exact'\|'year'\|'none') aditivo. **Frontend puro, sin migración**. Reconciliado en `design-alta-form-refinamiento.md` (contrato `precision`) + `design-c6-categoria-espejo.md` + `requirements-alta-form-refinamiento.md` RAF2.1.3 | done (Puerta 2, 2026-07-10) |
| `nombre-apodo` | toggle por rodeo del "apodo" vía campo custom (migración `0119`, seed per-est deshabilitado) + baja del "Nombre/seña" built-in del alta (#2) — **SUBSUMIDO por `identificadores-unificados`** (el apodo quedó como uno de los 3 identificadores canónicos, ≤15 + warning-soft por campo). El mecanismo (0119 + toggle en `editar-plantilla`) sigue vigente. **CON BACKEND** (`0119`) | done (vía `identificadores-unificados`, Puerta 2 2026-07-09) |
| `lotes-venta` | **lotes operables: venta/descarte en tanda** (ítem A triage demo-facundo-padre + su padre). Cualquier lote (`management_groups`) puede registrar la salida en tanda de todos o parte de sus animales activos → **modo selección** de subconjunto (`app/lote/[id].tsx`) → `app/lote/venta.tsx` (motivo Venta/Muerte, fecha común + precio/peso comunes ajustables por animal con override) → **loop client-side de `exit_animal_profile`** (0044) vía la outbox (`enqueueExitAnimal`, N intents) + clear de membresía (`management_group_id → NULL`); no-atómico por diseño (resultado parcial > rollback). **Sugerencia post-tacto**: al cerrar una jornada con tacto, si hay vacías → `ExitJornadaSheet` fase `'terminated'` ofrece agregarlas a un lote (elegir/crear, default "Descarte" — `SugerenciaVaciasSheet`). "Venta simple" (Puerta 1): motivos = Venta/Muerte, sin `culling`. **Frontend puro, SIN migración/RPC nueva** (Gate 1 N/A; anti-IDOR de la tanda vía `fetchGroupMembers` ∩ ids + re-validación de la RPC). Ver `{context,requirements,design,tasks}-lotes-venta.md`. | done (Puerta 2, 2026-07-11) |
| `tratamientos` | **tratamientos en la ficha** (ítem E triage demo-facundo-padre + padre): tabla nueva `treatments` (header, ciclo `started_at`→`ended_at`) + aplicaciones = `sanitary_events` linkeadas (`treatment_id` FK). Iniciar/registrar-aplicación/finalizar desde la ficha (cualquier rol incl. peón); "en tratamiento" = DERIVADO; **marca teal-cian** (`treatmentText #106B7A`/`treatmentBg #DBEEF3`) + **pin arriba** en lista general Y del rodeo. **CON BACKEND**: migración `0123` (RLS + triggers de inmutabilidad SEC-TRT-01 + tenant-check SEC-TRT-03 + CHECKs 120/1000 + exención de gating acotada LOW-1) + stream `ev_treatments`. Gates: Gate 1 PASS + reviewer APPROVED + Gate 2 PASS 0 HIGH. `0123` DEPLOYADA + verificada (RLS suite 11/11 + regresión del gating verde) + stream `ev_treatments` deployada + fix de vía `intravenous` (fuera del enum, cazado en Gate 2). Gate 2.5: 13 capturas, token teal veto PASS. Ver `{context,requirements,design,tasks}-tratamientos.md`. | done (Puerta 2, 2026-07-11) |
