...
>
> **Consecuencia en el e2e (fix e2e-latentes 777, 2026-07-10)**: el test de alta year-only (`animals.spec.ts`, "RAF2.1.3") YA NO asierta el midpoint ciego `2022-07-01` — el as-built imputa consciente-de-categoría (para una Vaquillona-2022 hoy da `2022-07-02`, día 182 del año). Como el e2e corre con el `now` real (sin inyección) y el día exacto depende de la categoría/`now`, el test asierta el **contrato relevante**: año-solo persiste una fecha DENTRO del año (`/^2022-/`). La precisión del midpoint consciente la cubren los **unit deterministas** (`animal-category.test.ts` / `animal-birth-year.test.ts`, con `today` inyectable). El path de fecha EXACTA (RAF2.1.4) sigue asertando `2022-07-01` legítimo (sin imputación).
