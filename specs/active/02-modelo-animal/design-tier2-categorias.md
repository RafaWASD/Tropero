...
> **Por qué `apply_auto_transition` y no `UPDATE category_id` directo.** El cron cambia categoría **solo** vía `apply_auto_transition` (que setea el GUC `mitropero.is_auto_transition` y registra el history como `auto_transition`, `0031`/`0030`). Reusar el camino confiable da: (a) el registro en `animal_category_history` gratis (RT2.8.4b / RT2.10.3); (b) coherencia con todas las demás transiciones automáticas; (c) un solo lugar que escribe `category_id` automáticamente. `apply_auto_transition` está revocada de clientes (`0042`) pero el cron, al ser `SECURITY DEFINER`, corre como owner y puede invocarla.
