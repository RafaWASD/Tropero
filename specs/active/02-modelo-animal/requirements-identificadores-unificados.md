...
- **IDU.4.3** El sistema deberá encontrar un animal por coincidencia (exacta o parcial) de su `idv` alfanumérico, incluyendo `idv` que contengan letras.
  - *Reconciliación (as-built, 🔴 A.1 del QA en device 2026-08-06): el clasificador COMPACTABA el término (borraba espacio, guion, punto y barra) y corría el canal `idv` solo con esa versión, mientras el `idv` GUARDADO conserva los separadores. Consecuencia medida en el A07: tipear `PERF-00500` —el string exacto que la app muestra en su lista— no encontraba al animal y la pantalla ofrecía "Dar de alta" (= animal duplicado con la jornada repartida). El compactado era correcto para la caravana ELECTRÓNICA (que el operario tipea en grupos) y estaba mal aplicado al canal `idv`. As-built: el canal `idv` corre el match EXACTO con **dos términos** (`SearchPlan.idvExactTerms` = [tipeado tal cual, compacto]) y el match PARCIAL contra la **columna también compactada** (`REPLACE` anidado en `buildSearchLikeQuery`). El canal TAG sigue usando solo el compacto. Cubierto por `e2e/qa-fixes-datos.spec.ts` (buscador global, buscador del rodeo y manga).*
