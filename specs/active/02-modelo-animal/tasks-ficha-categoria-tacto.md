...
`requirements-ficha-categoria-tacto.md` + `design-ficha-categoria-tacto.md`.
**Gate 1 N/A** (sin migraciones / RPC / Edge / policies — ver design §9). **Gate 2 y Gate 2.5 SÍ.**
...
- [ ] **TCT.1** — `app/src/utils/animal-category.ts` (**colisión-safe**): **exportar** `ONE_YEAR_DAYS` y
...
- [ ] **TCT.2** — `app/src/utils/animal-category-picker.ts` (**colisión-safe**): `+ pickableCategories(
...
- [ ] **TCT.3** — `app/src/utils/category-pin.ts` (NUEVO, **colisión-safe**): `COHERENCE_WINDOWS` (construida
...
- [ ] **TCT.4** — `app/src/utils/animal-category-picker.test.ts` (**existente, colisión-safe**) +
...
- [ ] **TCT.5** — `app/src/services/powersync/local-reads.ts` (**⚠️ compartido**):
...
- [ ] **TCT.6** — `app/src/services/category-pin-core.ts` (NUEVO, **colisión-safe**): `decideCategoryPin({
...
- [ ] **TCT.7** — `app/src/services/category-pin-core.test.ts` (NUEVO, **colisión-safe**): `node:test` con
...
- [ ] **TCT.8** — `app/src/services/animals.ts` (**⚠️ compartido**): `+ setCategoryManual(profileId,
...
- [ ] **TCT.9** — `app/src/components/CategoryPickerSheet.tsx` (NUEVO, **colisión-safe**): sobre
...
- [ ] **TCT.10** — `app/app/animal/[id].tsx` (**⚠️ compartido**): fila **"Categoría"** en "Datos del animal",
...
- [ ] **TCT.11** — `app/app/animal/[id].tsx` (**⚠️ compartido**): montaje del `CategoryPickerSheet` al ROOT
...
- [ ] **TCT.12** — `scripts/run-tests.mjs` (**⚠️ compartido**): enganchar `category-pin.test.ts` y
...
- [ ] **TCT.13** — `app/src/utils/ficha-tacto-offer.ts` (NUEVO, **colisión-safe**):
...
- [ ] **TCT.14** — `app/src/utils/ficha-tacto-offer.test.ts` (NUEVO, **colisión-safe**): `node:test` con
...
- [ ] **TCT.15** — `app/app/animal/tacto.tsx` (NUEVO, **colisión-safe**): ruta full-screen con params
...
- [ ] **TCT.16** — `app/app/animal/tacto.tsx` (**colisión-safe**): resolución de los **buckets** —
...
- [ ] **TCT.17** — `app/app/animal/tacto.tsx` (**colisión-safe**): **re-validación al montar** con
...
- [ ] **TCT.18** — `app/app/animal/tacto.tsx` (**colisión-safe**): persistencia con los servicios EXISTENTES
...
- [ ] **TCT.19** — `app/app/_layout.tsx` (**⚠️ compartido**): `+ <Stack.Screen name="animal/tacto" />` junto a
  `animal/baja`. — *infra de navegación.*

- [ ] **TCT.20** — `app/app/animal/[id].tsx` (**⚠️ compartido**): (a) guardar el `RodeoDataKeyMap` COMPLETO de
...
- [ ] **TCT.21** — Verificar (sin código nuevo) que al volver de la captura la ficha refleja el tacto por
...
- [ ] **TCT.22** — `scripts/run-tests.mjs` (**⚠️ compartido**): enganchar `ficha-tacto-offer.test.ts`. —
  *infra de tests.*

## Fase 6 — Retiro del tacto sin gatear de "Agregar evento"

- [ ] **TCT.23** — Barrido previo: `grep -rn "Tacto" app/e2e/` + `app/src/**` para inventariar quién depende
  de la card/`TactoForm` de `agregar-evento` (candidato: `events.spec.ts`). **Antes** de borrar. — *evita un
  rojo sorpresa de E2E.*

- [ ] **TCT.24** — `app/app/agregar-evento.tsx` (**⚠️ compartido**): quitar la `TypeCard` "Tacto", el
...
- [ ] **TCT.25** — Ajustar los E2E que TCT.23 haya encontrado (el flujo de tacto se cubre ahora en
  `ficha-tacto.spec.ts`). — *no-regresión.*

## Fase 7 — Verificación del contrato de reportes (backend, read-only)

- [ ] **TCT.26** — `supabase/tests/reports/run.cjs` (**⚠️ compartido, pero suite propia**): 3 casos nuevos,
...
- [ ] **TCT.27** — `app/e2e/ficha-categoria.spec.ts` (NUEVO, **colisión-safe**) — Playwright web táctil real
...
- [ ] **TCT.28** — `app/e2e/ficha-tacto.spec.ts` (NUEVO, **colisión-safe**): (a) vaquillona sin veredicto
...
- [ ] **TCT.29** — **No-regresión de la manga**: correr `maniobra-carga.spec.ts` (tacto + aptitud en jornada)
...
- [ ] **TCT.30** — **Veto de diseño del leader** (skill `design-review`) antes de mostrarle nada a Raf:
...
- [ ] **TCT.31** — **Verificación de que Gate 1 no aplicaba**: `git diff supabase/migrations/
...
- [ ] **TCT.32** — Reconciliar specs al as-built (regla dura de `docs/specs.md`): si en implementación cambia
