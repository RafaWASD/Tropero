...
- [ ] **T1** — Migración `supabase/migrations/0123_treatments.sql`: enum `treatment_kind` + tabla `treatments`
...
- [ ] **T2** — En la misma `0123`: `ALTER sanitary_events ADD treatment_id uuid references treatments(id) on
  delete set null` + índice parcial `sanitary_events_by_treatment`. Cubre: RTR.2.2.
- [ ] **T3** — En la misma `0123`: triggers `treatments_force_establishment_id` (0077) +
...
- [ ] **T3b** — En la misma `0123`: **CREATE OR REPLACE de `tg_sanitary_events_gating`** (0091) con la exención
...
- [ ] **T4** — En la misma `0123`: RLS de `treatments` (SELECT/INSERT `has_role_in`; UPDATE `has_role_in` USING +
...
- [ ] **T5** — Stream `ev_treatments` en `sync-streams/rafaq.yaml` (JOIN-free, scope establishment,
  `deleted_at IS NULL`). Cubre: RTR.7.4.
- [ ] **T6** — Tests RLS (runner Node `supabase/tests/rls/`): (a) usuario sin rol en el campo NO lee/escribe un
...
- [ ] **T7** — `schema.ts`: `Table treatments` + `treatment_id` en `sanitary_events` + registrar `treatments` en
  `AppSchema`. Cubre: RTR.8.1, RTR.8.2.
- [ ] **T8** — `local-reads.ts`: builders CRUD-plano `buildStartTreatmentInsert` / `buildRegisterApplicationInsert`
  / `buildFinalizeTreatmentUpdate` (idempotente `ended_at IS NULL`). Cubre: RTR.1.2, RTR.2.2, RTR.3.2, RTR.3.4.
- [ ] **T9** — `local-reads.ts`: lecturas `buildAnimalTreatmentsQuery` + `buildTreatmentApplicationsQuery` (en
  curso primero; aplicaciones por fecha). Cubre: RTR.9.1, RTR.9.2, RTR.9.3.
- [ ] **T10** — `local-reads.ts`: inyección de `in_treatment` (EXISTS synced / `0` overlay) en
...
- [ ] **T11** — `services/treatments.ts`: `startTreatment` / `registerApplication` (mapeo `kind→event_type`,
...
- [ ] **T11b** — `utils/treatment-input.ts`: constantes `TREATMENT_PRODUCT_MAX_LENGTH=120` /
...
- [ ] **T12** — `animals.ts` (`toLocalListItem` + `AnimalListItem`): exponer `inTreatment` (lee `in_treatment` del
  row). Cubre: RTR.4.4.

## Fase C — Frontend (ficha + marca + pin)

- [ ] **T13** — `AnimalRow.tsx`: prop `inTreatment?` + marca sanitaria (chip/punto, color TBD Gate 2.5; NO reusar
  terracota/amber/CUT/verde; `labelA11y` + `lineHeight`). Cubre: RTR.4.4, RTR.4.5.
- [ ] **T14** — `(tabs)/animales.tsx`: pasar `inTreatment` a `AnimalRow` (el pin ya lo da el ORDER BY). Verificar
  que la lista del **rodeo** (con `rodeoId`) también pinnea/marca. Cubre: RTR.4.4, RTR.5.1, RTR.5.2.
- [ ] **T15** — `animal/[id].tsx`: sección "Tratamientos" (listar tratamientos + aplicaciones) + marca "en
...
- [ ] **T16** — Sheet "Iniciar tratamiento" (selector `kind` cerrado + `product_name` requerido, validado con el
  ...
  RTR.1.6, RTR.1.7.
- [ ] **T17** — Sheet "Registrar aplicación" (fecha default hoy + dosis/vía/próxima dosis opcionales) →
  `registerApplication`, solo en tratamiento en curso. Cubre: RTR.2.1, RTR.2.3, RTR.2.4, RTR.2.5, RTR.9.5.
- [ ] **T18** — CTA "Finalizar tratamiento" (confirmación inline) → `finalizeTreatment`; la marca/pin se quitan
  (refresh silencioso). Cubre: RTR.3.1, RTR.3.2, RTR.3.3, RTR.4.6, RTR.5.4.
- [ ] **T19** — Verificar que las aplicaciones siguen en el timeline (sanitary_events) sin regresión; *(opcional)*
  proyectar `treatment_id` en el payload del timeline para anotar el nodo. Cubre: RTR.9.4.

## Fase D — Offline + E2E + Gate 2.5

- [ ] **T20** — E2E (`app/e2e/`, fixtures de `./helpers/fixtures`): flujo iniciar → aplicar → finalizar en la
...
- [ ] **T21** — E2E/manual offline: iniciar/aplicar/finalizar sin conexión → la fila local aparece/actualiza al
