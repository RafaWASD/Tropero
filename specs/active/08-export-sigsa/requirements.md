...
1. **Raza desconocida: ¿bloquear o `OR`?** R8.2 bloquea el export si `breed_id` es NULL. La tabla oficial tiene `OR` (Otra Raza) como fallback bovino legítimo (`razas-senasa-codigos.md`, no `S/E` que es el genérico de cierre). Dado el plazo legal de 10 días hábiles, conviene que "no sé la raza" sea **un tap a `OR`**, no un bloqueo duro. Propuesta: mantener el bloqueo por `breed_id` NULL pero exponer `OR` como opción de un tap en el `BreedPicker`. Confirmar con Raf/Facundo.
2. **R10.2 (marcar declarado a mano)**: agregada por el spec_author más allá del Gate 0. Útil para declaraciones hechas por otra vía (oficina SENASA, SIGBIOTRAZA directo). Confirmar si entra al MVP o se difiere.
3. **RENSPA único global**: el diseño hace `renspa` único entre TODOS los establecimientos (no solo "un RENSPA por establecimiento"). Confirmar que es la intención (un RENSPA = una unidad productiva) y no un over-constraint que pueda morder.
4. **GATE DURO de formato** (ya marcado arriba): confirmar el formato exacto del TXT con upload real / login SIGSA con clave fiscal antes de cerrar 08.
