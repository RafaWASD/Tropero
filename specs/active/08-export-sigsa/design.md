...
- **2026-07-25 — RECONCILIACIÓN as-built del `BreedPickerSheet` (bugfix 🔴 MANGA "el teclado tapa el sheet", Raf device iOS)**: el sheet dejó de armar el esqueleto de bottom-sheet a mano y pasa al primitivo **`app/src/components/BottomSheetShell.tsx`** (shell canónico del repo, nacido de este bugfix — detalle completo en `specs/active/03-modo-maniobras/design.md` §"As-built v7"). Qué cambia acá: (a) el sheet **sube por encima del teclado** (primitivo `KeyboardAvoidingShell` — ver spec 03 §"As-built v11": en Android el `KeyboardAvoidingView` de RN era un no-op y el lift lo hace un `paddingBottom` = alto del teclado) — antes, al tipear en el buscador de razas, el teclado tapaba la lista de resultados y el "Cancelar"; (b) con el teclado arriba **se condensa** (se ocultan la descripción "La raza se usa para declarar el animal en SIGSA…" y el "Cancelar") y aparece/permanece una **X de cierre en el header** (`testID` `breed-sheet-close`); (c) el **campo de búsqueda pasó del header FIJO al primer lugar del BODY scrolleable**: con el teclado abierto, tres bloques fijos (título + descripción + buscador) se comían casi todo el alto útil y no quedaba lista visible; ahora el buscador arranca siempre a la vista y la lista conserva alto. (d) el guard anti "click huérfano" del scrim (doble-rAF) ya no vive en este archivo: lo aporta el primitivo y se re-arma en cada apertura (el shell monta/desmonta con `open`). Los `testID` existentes (`breed-sheet`, `breed-sheet-scrim`, `breed-sheet-search`, `breed-sheet-cancelar`, `breed-option-*`) **se preservan**; `sigsa-breed-renspa.spec.ts` (4/4) sigue verde y `e2e/sheet-teclado.spec.ts` agrega la regresión de filtrado + cierre por X. Veredicto del lift real en **device** (ADR-029: web no monta teclado virtual).
