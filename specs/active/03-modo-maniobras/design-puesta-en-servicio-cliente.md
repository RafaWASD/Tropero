...
> **AS-BUILT de B3 (2026-06-24, `impl_03-streamB-b3.md`) — pendiente veto liviano del leader + reviewer + Gate 2.** Se implementó la variante más explícita de las dos que el diseño ofrecía: en `event-input.ts`, `SERVICE_TYPE_OPTIONS` queda INTACTO como **catálogo completo del enum** (3 valores, incl. `natural`) — la fuente única de los labels y backward-compat conceptual del humanizador del timeline — y se agregó una const NUEVA **`SERVICE_TYPE_INPUT_OPTIONS`** = subset ofertable para la carga manual NUEVA (`['ai','te']`, derivada por `.filter(o => o.value !== 'natural')` de `SERVICE_TYPE_OPTIONS` → labels single-source). El `ServiceForm` de `agregar-evento.tsx` apunta a `SERVICE_TYPE_INPUT_OPTIONS` (no a `SERVICE_TYPE_OPTIONS`); el estado `serviceType` y los props del form se narrowearon a `ManualServiceType = 'ai'|'te'` (el enum completo `ServiceType` sigue siendo el tipo de `AddServiceInput.serviceType` → el subset es asignable; backward-compat del tipo intacta). Subtítulo de la card "Servicio" del paso 1: "Monta natural, IA o TE" → **"Inseminación o TE"** (el default que el diseño delegó). **NO tocado:** el enum DB `service_type_enum`, `reproductive_events`, `addService`/`AddServiceInput.serviceType` (tipo completo), `buildAddServiceInsert` (write-path de IA/TE), `humanizeServiceType`/`SERVICE_TYPE_LABELS` (mantienen `natural: 'Monta natural'` para el timeline histórico, RPSC.6.3), la IA de manga (`InseminacionStep`). **Gate 1 N/A** (frontend puro; `git diff -- supabase/` vacío). `check.mjs` VERDE end-to-end (incl. `operaciones_rodeo`, que ya no está rojo: el PAT fue rotado). e2e `events.spec.ts` 14/14 (4 reescritos a "Inseminación (IA)" + 1 nuevo B3 que siembra un `natural` histórico por admin y verifica que renderiza + que el alta ya no ofrece "Monta natural" pero sí IA/TE). Captura `tests/stream-b/b3-servicio-selector-{360,412}.png` (selector de 2 filas balanceado, sin recorte) para el veto liviano.
...
> **AS-BUILT del CABLEADO de B2 (2026-06-24, `impl_03-streamB-b2-wiring.md`) — diseño YA APROBADO por Raf (spike); esto es la plomería.** Enchufa los componentes/lógica del spike al flujo REAL de jornada de tacto, sin rediseñar nada. Frontend puro → **Gate 1 N/A** (no toca schema/RLS/Edge; `pregnancy_status` ya existe).
> - **Lectura del `service_months` del rodeo de la jornada (offline):** `buildRodeoServiceMonthsQuery(rodeoId)` (`local-reads.ts`, espeja `buildRodeoSystemQuery`) + `fetchRodeoServiceMonths(rodeoId)` (`rodeos.ts`) → `number[] | null` parseado con `parseServiceMonths` (FUENTE ÚNICA de B1, RPSC.3.7; sin fila / null / corrupto → `null` = "sin configurar", fail-safe, no crash). `carga.tsx` la carga al conocer el rodeo del ANIMAL (estado `rodeoServiceMonths`, patrón de `categoryCatalog`/`lastScrotalCm`; multi-tenant: el rodeo sale del animal real, nunca hardcodeado).
> - **`buckets` reales al `TactoStep` (RPSC.5):** `carga.tsx` computa `tactoBuckets = effectiveSizeBuckets(nMonths, override)` (memo) donde `nMonths = rodeoServiceMonths?.length ?? null` y `override = tactoMeasureSizeFromConfig(parseManeuverConfig(session.config))` → lo pasa por `ManeuverStep` al `TactoStep` (case `'tacto'`). NO se re-deriva la regla CCL en el frame (FUENTE ÚNICA `pregnancy-buckets.ts`, DD-PSC-3). 2 meses→cabeza/cola; 3..11→cabeza/cuerpo/cola; 1/12/sin-configurar/no-medir→`[]` → PREÑADA persiste `'large'` directo (DD-PSC-2, ya en el `TactoStep`).
> - **Config "¿medir tamaño?" en el wizard (RPSC.4):** el tacto pasó a ser una maniobra **configurable** en el `ManeuverReorderList` (el genérico ya soporta "configurable" vía `inlineConfig` no-null + `onOpenConfig`). `jornada.tsx`: `onOpenConfig('tacto')` → abre el `TactoConfigSheet` (default = `defaultMeasureSize(serviceMonthsCount)` del rodeo elegido, sugerido visible, override de un toque); `onTactoConfigSave(measureSize)` persiste el OBJETO `preconfig.tacto = { measureSize }` (shape que `tactoMeasureSizeFromConfig` lee). El inline de la fila del tacto muestra "Medí tamaño: Sí/No" (override) o el "Sugerido: …" derivado del rodeo (si no se configuró). El resumen de etapa 3 muestra el efectivo (override o sugerido). **[DECISIÓN del implementer]** `preconfig.tacto` es un objeto (no string de texto libre): `tactoMeasureSizeFromConfig(config)` (`maneuver-config.ts`, lector tolerante: ausente/shape-raro/no-boolean → `undefined` → cae al default del rodeo).
> - **DD-PSC-8 (resumen sin tamaño):** `summaryRows`/`describeStepValue` (`maneuver-sequence.ts`) reciben `opts.tactoMeasuredSize`. `carga.tsx` deriva `tactoMeasuredSize = tactoBuckets.length > 0` (MISMA fuente que los buckets, sin segunda regla) y lo pasa al `AnimalSummary` → un tacto preñado sin distinción de tamaño muestra solo **"Preñada"** (no "Preñada · Cabeza"), aunque persista `'large'` (DD-PSC-2). VACÍA y los tactos con tamaño no cambian.
> - **Backward-compat (RPSC.4.4):** un rodeo SIN `service_months` (NULL) → `effectiveSizeBuckets(null, undefined) = []` → PREÑADA va directo + resumen "Preñada"; NO rompe el flujo de tacto existente. Las e2e `maniobra-carga`/`maniobra-preview-transicion` que ejercitaban el sub-paso de tamaño se reconciliaron sembrando el rodeo con `serviceMonths:[10,11,12]` (3 meses → 3 bloques) — el flujo de tamaño se preserva donde el rodeo está configurado.
> - **Tests:** unit `pregnancy-buckets`/`service-months` (de B1/spike, 19+82) + nuevos: `maneuver-config.test.ts` (`tactoMeasureSizeFromConfig`, 4 tests), `maneuver-reads.test.ts` (`buildRodeoServiceMonthsQuery`), `maneuver-sequence.test.ts` (DD-PSC-8 en `describeStepValue`/`summaryRows`). e2e `maniobra-tacto-adaptativo.spec.ts` (4: 2 meses→cabeza/cola+server `large`; 1 mes→directo+resumen "Preñada"+server `large`; override "no medir" sobre 3 meses→directo; **360 web táctil** con anti-recorte de "PREÑADA"/"CABEZA"). `seedRodeo`/`seedEstablishmentWithRodeo` ganaron una opción `serviceMonths` (set directo de la columna vía service_role, fixture). `check.mjs` verde salvo el flake de Management-API (token `SUPABASE_ACCESS_TOKEN`) de la suite backend `operaciones_rodeo` (spec 10, ajena: este delta NO toca SQL).
...
- **PowerSync `rodeos.service_months` (RPSC.3.7, dependencia):** el implementer agrega `service_months: column.text` a la tabla `rodeos` del `AppSchema` (`schema.ts:148`). PowerSync materializa el `smallint[]` de Postgres como TEXT/JSON; `parseServiceMonths` (DD-PSC-5) lo convierte a `number[] | null` de forma tolerante. **Verificar** que la sync rule (`sync-streams/mitropero.yaml`, stream `est_rodeos`) incluye la columna (Stream A la agregó en la migración; el cliente debe declararla en el schema local o la lectura del rodeo no la trae). Si la stream NO la emite, es un gap de Stream A a anotar (no debería: `service_months` es columna de `rodeos`, que ya se sincroniza enteramente).

> **AS-BUILT del CABLEADO (2026-06-23) — verificación de la sync rule = CASO A.** El stream `est_rodeos` (`sync-streams/mitropero.yaml:109-114`) es `SELECT * FROM rodeos WHERE establishment_id IN org_scope AND deleted_at IS NULL`. Es **`SELECT *`** → `service_months` (columna de `0102`) **fluye sola** por el WAL sin tocar el YAML. **CASO A: NO se edita ni deploya `mitropero.yaml`** (no hay deploy server-side gateado para esta pieza). El único trabajo client-side fue declarar `service_months` en el schema local de PowerSync (`schema.ts`, tabla `rodeos` + `pending_rodeos`) + un overlay nuevo `pending_rodeo_service_months` para la edición optimista.

---

## 6. B3 — qué se quita y qué se conserva (detalle)

- **Se quita:** la opción **"Monta natural"** (`service_type='natural'`) del flujo "Agregar evento" de la ficha (`agregar-evento.tsx`). **AS-BUILT (B3):** vía la const NUEVA `SERVICE_TYPE_INPUT_OPTIONS` (`['ai','te']`, subset derivado de `SERVICE_TYPE_OPTIONS` que queda como catálogo completo) — el `ServiceForm` apunta a ella; ver el AS-BUILT de DD-PSC-6.
- **Se conserva:** IA (`ai`) y TE (`te`) en "Agregar evento" (carga reproductiva real per-vaca); la inseminación de manga (`InseminacionStep`, `service`+`ai`) **intacta** (RPSC.6.2); todos los eventos `service` históricos en el timeline (RPSC.6.3, `humanizeServiceType('natural')='Monta natural'` sin tocar).
- **No se toca:** el enum `service_type` DB, `reproductive_events`, `addService`/`buildAddServiceInsert` (`AddServiceInput.serviceType` sigue siendo el enum completo `ServiceType`), el aviso suave `reproductiveWarning('service', …)` (sigue válido para IA/TE: "figura preñada, ¿registrar el servicio igual?").
...
- **Tests (RPSC.6.5) — AS-BUILT:** `app/e2e/events.spec.ts` — los **4** tests que ejercitaban "Monta natural" (reproductivo→servicio, servicio-preñada, orden-timeline, servicio-no-preñada) se reescribieron a **"Inseminación (IA)"** + un test NUEVO de B3 (`B3 baja monta natural…`) que (a) verifica que el selector NO ofrece "Monta natural" y SÍ IA/TE, y (b) siembra un `service`+`natural` HISTÓRICO por admin (`seedReproductiveServiceEvent`, nuevo helper) y verifica que sigue en el timeline. + unit `event-input.test.ts` (`SERVICE_TYPE_INPUT_OPTIONS` no contiene `natural`, sí IA/TE, subset del catálogo) + `event-timeline.test.ts` `humanizeServiceType` (los 3, incl. `natural`) **sin tocar** (backward-compat). El gate-por-sexo (`events.spec.ts`, machos no ven Tacto/Servicio/Parto) **no cambia** (la card "Servicio" sigue, ahora IA/TE).
