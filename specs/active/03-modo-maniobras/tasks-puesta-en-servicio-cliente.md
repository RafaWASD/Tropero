...
- [ ] **T-B1.3** — `app/src/services/powersync/schema.ts`: agregar `service_months: column.text` a la tabla `rodeos`. Verificar que la stream `est_rodeos` (`sync-streams/mitropero.yaml`) emite la columna; si no, anotar gap de Stream A (no debería). Cubre: RPSC.3.7.
