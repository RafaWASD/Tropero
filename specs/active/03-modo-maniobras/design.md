...
| `skip-animal-maniobra` | **Saltear un animal en la carga rápida** (ítem C triage demo-facundo-padre): afordancia "Saltear" en la esquina sup-der del header (`SpikeIdentityHeader.onSkip`; el chip de progreso baja a la línea rodeo·categoría) → `SkipAnimalSheet` (bottom-sheet de confirmación, tono adaptativo, fail-closed). Abandona el animal sin cargarle maniobras y vuelve al identify-first **sin contarlo**; con datos parciales **descarta lo cargado** (soft-delete de las filas de evento de ese frame por ids de cliente estables — `collectManeuverDiscardTargets`/`discardManeuverEvents`, mismo espíritu que el borrado de vacunas huérfanas). `dientes` (propiedad) queda fuera del descarte (documentado). Reconciliado **in-place** (R5.15 en el baseline). **Frontend puro, sin migración** (Gate 1 N/A). | **SUPERSEDIDO parcialmente por `skip-por-paso-v2`** (Raf rechazó el skip per-animal como primario; el `SkipAnimalSheet` sobrevive **demovido a acción secundaria** en el overflow "⋯", el primario pasó a ser el skip POR-PASO) |
| `vacunas-aplica-no-aplica` (D2) | **Rediseño de la vacunación APLICA/NO-APLICA + endurecimiento de la etapa 2** (ítem D2 triage demo-facundo-padre; patrón confirmado por Raf). **Por animal**: se elimina el input libre + chips + autocompletar por-animal (D1) → **CHECKLIST grande** de las vacunas DEFINIDAS en la tanda; cada una APLICA/NO-APLICA (todas tildadas por default; tap destilda). Solo persisten los `sanitary_events` de las tildadas; 0 tildadas → CTA "Seguir sin aplicar", "Sin vacuna" (**preserva el path honesto de D1**). Cerebro puro `vaccine-checklist.ts` (`buildVaccineChecklist`/`appliedVaccineNames`); vista `SilentVaccinationStep.tsx`; wiring `carga.tsx` (JSX, `definedProducts`/`appliedProducts` — persist sin cambios). **Pre-maniobra (etapa 2)**: si Vacunación está elegida, **exige ≥1 vacuna definida** para continuar (`vaccinationMissingProducts` en `maneuver-wizard.ts`) → continue bloqueado + mensaje + marca de alto contraste "Faltan vacunas" en la fila (`jornada.tsx` + `ManeuverReorderList.tsx` inline `warn`). Reconciliado **in-place** (notas as-built bajo R6.1 y R1.7). **Frontend puro, sin migración** (Gate 1 N/A). | done (Puerta 2, 2026-07-11 — la pantalla in-maniobra; el refuerzo del CTA "Faltan vacunas" se cerró en `skip-por-paso-v2`) |
| `skip-por-paso-v2` | **Rediseño del SALTEAR a POR-PASO + refuerzo del CTA "Faltan vacunas"** (Puerta 2 demo-facundo-padre; el skip per-animal de `skip-animal-maniobra` fue rechazado por Raf). **Skip POR-PASO (primario)**: pill "Saltear <maniobra>" en el header (`SpikeIdentityHeader.onSkipStep`+`skipStepLabel`; helper puro `skipStepButtonLabel`) que saltea ESE paso y avanza al **siguiente del MISMO animal**; `{kind:'skipped'}` deliberado cuenta como HECHO (no bloquea `isSequenceComplete`/`firstUncapturedIndex`; resumen "Salteado", corregible — `maneuver-sequence.ts`). Corrección captura→salteado: soft-delete acotado al paso + reset de ids (`carga.tsx`). Solo pasos de fábrica (custom sin marcador de skip). **Skip animal-entero (secundario)**: `SkipAnimalSheet` intacto, demovido a overflow "⋯" (`onSkipAnimal`). **D2 enhancement**: chevron '>' de la fila "Faltan vacunas" pasa a CTA terracota (`ManeuverReorderList.tsx`, testID `selected-config-fix`). Reconciliado **in-place** (notas de reconciliación bajo R5.15 y R1.7). **Frontend puro, sin migración** (Gate 1 N/A). | done (Puerta 2, 2026-07-11) |
...
- **Buckets de sync**: agregar `sessions` y `maneuver_presets` a las sync rules de PowerSync (`sync-streams/mitropero.yaml` (raíz del repo) + el `AppSchema` de `app/src/services/powersync/schema.ts`), scoping por `establishment_id` del usuario (mismo patrón que spec 02). Las 5 tablas de evento ya están en sync; el `session_id` viaja con ellas.
...
- **Drag burbuja**: arrastrar por el grip (gesto inmediato) levanta la fila en estado "burbuja" — **escala ~1.04 + sombra/elevación fuerte + esquinas más redondas + sigue el dedo 1:1**; los hermanos se **corren animados** (spring) para hacer lugar; **spring** al soltar; **háptica** al agarrar y al soltar (`@/utils/haptics`, vía `Vibration` de RN — el proyecto NO tiene `expo-haptics`; mismo idioma que `services/ble/feedback.ts`, web-safe/degrada-en-silencio) **[CORRECCIÓN 2026-08-06]** El motivo citado acá era FALSO: `expo-haptics` no tiene postinstall ni dependencias (verificado con `npm view expo-haptics scripts dependencies`) y **ADR-013 §Capa 4 ya lo listaba en el stack**. La dep está instalada desde esa fecha y la usa `services/ble/feedback.ts`. Estos consumidores (reorder / rueda) siguen con `Vibration` **por otra razón, que sí vale**: piden ticks de 8–18 ms sin semántica de éxito/error, que es lo que `Vibration` hace bien y `NotificationFeedbackType` no expresa. Migrarlos es una decisión de UX de estas pantallas, no un arrastre. Ver spec 04 R4.1.. **Fluidez = prioridad #1**: gesto + animación en el **hilo de UI** (reanimated worklets + gesture-handler) — el JS thread solo recibe el commit del reorder.
...
- **As-built v3-bis (bugfix reportado por Raf en device iOS, 2026-07-25) — el AUTO-SCROLL del drag queda ACOTADO A LA REGIÓN de seleccionadas (R1.12 / reconciliación R1.12-a).** La v3 topaba el auto-scroll **solo hacia arriba** (`Math.max(0, next)`); hacia abajo corría hasta el **final del contenido del ScrollView** y el disparador era puramente "el dedo entró en la `EDGE_ZONE`", sin preguntar si quedaba algo por revelar → sostener el grip de una maniobra cerca del borde inferior mandaba la página al fondo de TODO (pool + custom + "Detalle de la tanda" + CTA) y **la lista que se está ordenando desaparecía de pantalla**.
  - **Cómo quedó**: el contenedor de la región reordenable (el de alto `n*ROW_HEIGHT`) pasó de `View` de Tamagui a **`Animated.View` con `useAnimatedRef` + `collapsable={false}`** (sin el `collapsable` Android aplana la vista y `measure()` devuelve **`null`** — reanimated detecta el NaN de la vista aplanada y lo convierte en `null` con un warning de flattening; el efecto es el mismo: sin medida no se auto-scrollea). El `useFrameCallback` **mide la región en coordenadas de pantalla** con `measure()` (worklet, hilo de UI) **solo cuando hay auto-scroll pedido** (`autoScrollDir !== 0`) y delega el cómputo a **`autoScrollDelta` — función PURA en `app/src/utils/reorder-autoscroll.ts`**, testeada con `node:test`: baja mientras `regionBottom + MARGIN > viewportBottom`, sube mientras `regionTop - MARGIN < viewportTop`, recorta el paso al remanente exacto (aterriza con el aire pedido, sin overshoot) y mantiene el piso duro de offset 0. `MARGIN = AUTO_SCROLL_REVEAL_MARGIN = 24px` (geometría de gesto, const nombrada como `EDGE_ZONE`/`AUTO_SCROLL_SPEED`, NO token: < `EDGE_ZONE` 72 para que al frenar el dedo siga sobre las filas; ~30% de `ROW_HEIGHT` para que la última fila quede despegada del borde; > el gap de 8px entre filas para que se lea como aire).
  - **Fail-closed**: si `measure()` devuelve `null` (vista no layouteada / plataforma sin soporte) o la geometría no es finita, **no se auto-scrollea** — mismo criterio que el guard preexistente de `viewportHeight <= 0`; nunca se cae al comportamiento sin tope.
  - **Alternativas DESCARTADAS** (quedan documentadas en el código para que no vuelvan): (a) gatear por "el ítem arrastrado ya llegó al extremo de sus bounds" — si el ítem está en el último slot pero el fondo de la región sigue fuera de pantalla, el operario NECESITA que el scroll siga hasta revelarlo para ver dónde va a caer; el corte correcto es por VISIBILIDAD de la región y el "no scrollea de más" sale solo; (b) hardcodear "más de N maniobras (5)" — N es un síntoma de la altura del viewport, no una constante: con el clamp, si la región entra entera el delta da 0 y el auto-scroll simplemente no se mueve.
  - **Lo que NO cambió**: el clamp del ítem arrastrado (`dragY ∈ [-index*ROW_HEIGHT, (total-1-index)*ROW_HEIGHT]`), la compensación dedo↔ítem por `scrollDelta`, el disparador por `EDGE_ZONE`, el commit del reorder (`onReorder(from, to)` → `moveManeuver`), los `testID` y el test hook `frozenDragIndex`. `jornada.tsx` solo sumó `testID="jornada-scroll"` en el `Animated.ScrollView` (ancla del e2e que mide el scroll real).
- **As-built v4 (iteración UX 3, feedback de Raf 2026-06-14) — preconfig de tanda INLINE + BOTTOM SHEET (R1.7/R1.8).** El preconfig de tanda dejó de vivir en la **sección huérfana del fondo** de la etapa 2 (la v1/v2/v3 tenían un bloque "Detalle de la tanda" con un `FormField` suelto + chips de autocompletar abajo de todo) y pasó a **inline en la fila de la maniobra + un bottom sheet enfocado**:
  ...
  - **As-built v6 (bugfix 🔴 MANGA, Raf 2026-07-22, U5) — el CUERPO del sheet quedaba INVISIBLE en NATIVO.** Raf reportó (iOS, con captura) que el sheet de preconfig de **Vacunación** aparecía con el **cuerpo vacío**: solo el título "Vacunación", el subtítulo y Guardar/Cancelar, pero **NO el input** para agregar vacunas → no se podían cargar vacunas → la marca "Faltan vacunas" no se iba. **Solo en NATIVO** (en web renderiza bien → por eso la E2E web no lo cazó). **Causa raíz**: el cuerpo era `<ScrollView flex={1}>` (= `flexGrow:1 flexShrink:1 flexBasis:0%`) dentro del `YStack` exterior con `maxHeight:85%` pero **SIN altura fija** (se dimensiona por contenido). En nativo (Yoga), cuando el contenido total (header + input + footer) es **CORTO** y NO llega al cap del 85%, el padre queda a la altura del contenido → NO hay "espacio libre" que el `flexGrow:1` pueda absorber → el ScrollView resuelve a su `basis:0%` → **altura 0 → cuerpo invisible**. (Contraste as-built: `BulkConfirmSheet` usa un `<ScrollView>` SIN flex — content-sized, capado por el `maxHeight` del padre — y anda; `CustomFieldSheet` usa `flex:1` pero su contenido es SIEMPRE alto → clampea el padre al `maxHeight` y no colapsa; vacunación tiene contenido corto → colapsa.) **Fix**: el body pasa a **`<ScrollView flexShrink={1}>`** (`grow:0 shrink:1 basis:auto`) conservando `style={{ minHeight: 0 }}` (necesario en web) y con header/footer en `flexShrink:0`. Con `basis:auto`: contenido **CORTO** → el ScrollView se dimensiona al contenido (el input SE VE); contenido **ALTO** (muchos chips + sugerencias + teclado) → el padre clampea al `maxHeight:85%` y, como header/footer no encogen, ESTE ScrollView (shrink:1) absorbe todo el faltante, se achica y **scrollea internamente** con el footer siempre abajo. Solo cambió el sizing del cuerpo — la lógica multi/single, chips, autocompletar y el guard del backdrop (v5) quedan intactos. `CustomFieldSheet.tsx` tiene el MISMO patrón latente (`flex:1`) pero hoy no está roto (su contenido siempre es alto) → scope aparte (backlog del leader). **Bug NATIVE-ONLY** → veredicto DEVICE (Raf); en web se verifica que NO hubo regresión: `maniobra-config-sheet-race.spec.ts` sigue verde (3/3) + captura web `app/e2e/captures/config-sheet-body.capture.ts` (sheet abierto con el input + "Usadas antes" visibles + vacuna cargada como chip).
  - **As-built v7 (bugfix 🔴 MANGA, Raf 2026-07-25) — el TECLADO tapaba TODO el sheet → primitivo `BottomSheetShell`.** Raf reportó (device iOS, build preview-dev, con captura): al enfocar el input del sheet de **Vacunación** (etapa 2) **el teclado tapaba el sheet entero** — solo quedaba visible el TÍTULO; el input, los chips ya agregados, el "+", las sugerencias y los DOS CTAs caían debajo del teclado ("no se ve lo que estás escribiendo, no tenés feedback de si escribiste la vacuna bien o mal, y no ves ningún CTA para confirmar"). **Causa raíz: bug de CLASE, no de instancia** — NINGÚN bottom sheet del repo tenía keyboard-avoidance: el patrón as-built (copiado a mano en cada sheet: `View absolute inset0 $scrim justifyContent:flex-end` + backdrop `Pressable` + `YStack maxHeight` anclado abajo) deja que iOS dibuje el teclado ENCIMA sin empujar nada. **Fix: primitivo `app/src/components/BottomSheetShell.tsx`** (hermano de `FooterActionShell`, exportado desde `components/index.ts`), que pasa a ser **el shell canónico de bottom sheet del repo** y encapsula: (1) backdrop `$scrim` tappable **con el guard anti "click huérfano"** (doble-rAF, as-built v5 — movido acá con su comentario; ya no se copia por sheet); (2) **el shell del teclado** envolviendo la columna → el sheet **SUBE** por encima del teclado. *(v7 decía acá: `KeyboardAvoidingView` con `behavior='padding'` en iOS y, en Android, "lo resuelve el `adjustResize` del `softwareKeyboardLayoutMode` default de Expo". La primera mitad sigue siendo cierta; la segunda **era falsa en el build real** y por eso en Android el teclado tapaba el sheet entero. Reconciliado en la **v11**: hoy es el primitivo `KeyboardAvoidingShell`.)*; (3) esqueleto **header fijo (`flexShrink:0`) / body `ScrollView flexShrink:1 + minHeight:0` / footer fijo (`flexShrink:0`)** — el `flexShrink:1` del body es el fix v6 preservado (**NUNCA `flex:1`**), y la COLUMNA del sheet también va `flexShrink:1` para que con el teclado arriba se ACHIQUE en vez de desbordarse por el tope (el título se iría de pantalla); (4) **safe-area inferior robusta** reusando la lógica PURA de U2 (`computeSafeBottomInset` + `resolveFooterPaddingBottom` de `utils/footer-action.ts`: blindaje frame-0 de Android edge-to-edge + sin doble espacio con el teclado abierto) — **as-built desde la unidad «aire» (2026-07-26)**: el shell la pide con el hook compartido `useSafeBottomInset()`, cuya fórmula es `max(insetVigente, insetArranque, $navBottomMin) + (Android ? $navBarGap : 0)` (el aire se suma **solo en Android**, donde el inset ES la barra de navegación; en iOS el inset ya es aire pintado con el fondo de la app); (5) **CONDENSACIÓN con el teclado arriba** (decisión pura `sheetCondensation`, `utils/sheet-shell.ts`, testeada): se ocultan la descripción del header y el CTA **secundario**; quedan SIEMPRE título + contenido cargado + input + CTA primario; (6) **X de cierre en el header, SIEMPRE presente** (target `$icon`=48 ≥44 + `hitSlop`, testID `<sheet>-close`) — Nielsen #3 control y libertad: con el teclado arriba es la única salida visible, y absorbe la función del "Cancelar" condensado; (7) `keyboardShouldPersistTaps="handled"` en el body → con el teclado abierto, tocar un chip de sugerencia lo agrega al PRIMER toque (antes el primer tap solo bajaba el teclado y se perdía). **Sheets migrados**: `ManeuverConfigSheet`, `CustomFieldSheet`, `SavePresetSheet` (spec 03) + `BreedPickerSheet` (spec 08). Se preservan TODOS los testID existentes (`maneuver-config-sheet`/`-scrim`/`-input`, `config-chip-*`, `config-suggestion-*`, `custom-field-sheet`/`-scrim`/`-scroll`, `save-preset-sheet`/`-scrim`, `breed-sheet`/`-scrim`) y se agregan los `<sheet>-close`. **Cambios de contenido del patrón "escribir → agregar → chip"** — aplican a los **DOS** sheets que lo tienen: el preconfig de **Vacunación** (`ManeuverConfigSheet`) y el editor de **Opciones** del `CustomFieldSheet`. El fix-loop del 2026-07-25 los unificó por decisión del leader (misma interacción → misma forma, ley de Jakob; el primer pase había reordenado solo el de preconfig): (a) el **input va ARRIBA de los chips** (antes los chips iban arriba en los dos) — con el teclado abierto, cada vacuna agregada CRECÍA el contenido por encima del input y lo empujaba fuera de la vista; ahora el input queda clavado en el tope del body y el chip nuevo aparece JUSTO DEBAJO (feedback donde ya está el ojo, sin scroll automático); (b) en modo **multi** (vacunación) Enter **agrega y MANTIENE el teclado** (`submitBehavior="submit"`, API de RN 0.77+ verificada en el RN 0.85.3 de este repo, + `blurOnSubmit={false}` como par para react-native-web, que todavía no lee `submitBehavior`) con `returnKeyType="next"`; en **single** (inseminación) sigue `done` + blur (el input ES el valor, no hay nada más que agregar). Mismo criterio en el input de OPCIONES del `CustomFieldSheet`. **Cobertura**: unit `src/utils/sheet-shell.test.ts` (condensación: qué se suelta y qué NUNCA — la X existe siempre) + e2e `sheet-teclado.spec.ts` (3 vacunas seguidas con Enter sin perder el foco ni cerrar el sheet; input arriba de los chips; **viewport 412×420 ≈ alto útil con el teclado abierto** → título + input + CTA primario dentro del viewport, sin recorte por arriba; la X cierra los 4 sheets) + capturas `e2e/captures/sheet-teclado.capture.ts` (9 estados) + regresiones existentes verdes (`maniobra-config-sheet-race` 3/3, `maniobra-customfield-validacion`, `maniobra-custom`, `maniobra-custom-gestion`, `maniobra-rutinas-gestion`, `maniobra-wizard`, `maniobra-sanitaria`, `maniobra-offline`, `sigsa-breed-renspa`). **LÍMITE de la verificación (ADR-029)**: react-native-web NO monta teclado virtual (`Keyboard` nunca emite → `useKeyboardVisible()` queda false), así que **el lift real y la condensación NO son observables en web** — el veredicto es **DEVICE (Raf)**. Lo que web sí ejercita es la geometría con alto útil recortado. **Sheets NO migrados** (fuera del scope de este fix, mismo patrón latente): `ExitJornadaSheet`, `NuevaJornadaConfirmSheet`, `OtherRodeoSheet`, `SyncRechazoSheet`, `TactoConfigSheet`, `LotePickerSheet`, `BulkConfirmSheet`, `TreatmentStartSheet`/`TreatmentApplicationSheet`, el age-sheet de `CircunferenciaEscrotalStep` y `TagScanSheet` (territorio de otra terminal). Los que tienen **input de texto** (`CircunferenciaEscrotalStep` no; `TagScanSheet` sí) siguen expuestos al mismo bug hasta migrarlos → backlog.
  - **As-built v8 (iteración UX 4, decisión de Raf 2026-07-25) — AUTO-GUARDADO: se van "Guardar" y "Cancelar" (R1.7/R1.8).** El sheet tenía **commit diferido**: los chips vivían en estado local (`items`) y sólo se persistían al tocar "Guardar". Dos defectos: (a) **confirmación redundante** — "Agregar" (o Enter) ya ES el gesto de commit y el chip que aparece ya ES el feedback; "Guardar" pedía confirmar lo ya confirmado, y en 🔴 manga cada tap se paga; (b) **descarte silencioso (el grave)** — el sheet tiene CUATRO salidas (Guardar, Cancelar, la X del header y el tap en el scrim) y las últimas tres llamaban a `onClose` sin persistir: cargabas cuatro vacunas, rozabas el scrim con el guante y se perdían las cuatro **sin feedback** (Nielsen #5, prevención de errores). Auto-guardar acá es barato porque este preconfig **no se escribe en la DB**: sólo actualiza el estado del wizard; la persistencia real ocurre aguas abajo en la etapa 3 (`createSession`/`createPreset`), que ya es una confirmación explícita. As-built:
    - **Los DOS modos auto-guardan** (`multi`/Vacunación y `single`/Inseminación). Se descartó auto-guardar sólo el multi: dos sheets abiertos desde la MISMA lista comportándose distinto sería peor (Nielsen #4, consistencia).
    - **multi**: cada `addItem` y cada `removeItem` commitea en el acto (`onCommit(joinMultiPreconfig(next))`). Quitar el último chip commitea `''` = **borrar** el preconfig (el caller borra la clave y la fila vuelve al hint / a la marca "Faltan vacunas" de D2). Esto **supersede** el argumento de v4 de "Guardar siempre habilitado para poder BORRAR": el borrado ya es expresable sin CTA.
    - **single**: commitea en el `onChangeText` del input (valor trimmeado).
    - **Texto tipeado sin agregar**: la regla que vivía en el viejo `handleSave` (si quedó texto en el input que no se agregó, entra) se conserva y se mueve al CIERRE — vale para **todas** las vías (CTA, X del header, scrim, y el arrastre del shell) porque el flush vive en el `onClose` que el sheet le pasa a `BottomSheetShell`, y el shell rutea las cuatro por ahí. Regla pura y testeada: **`pendingCloseCommit`** (`utils/maneuver-wizard.ts`), apoyada en **`addMultiPreconfigItem`** (dedup case-insensitive + trim; `null` = no hay nada que commitear, así un duplicado no dispara un commit idéntico).
    - **Footer**: un único CTA primario full-width **"Listo"** que sólo CIERRA (mismo camino que la X). Este sheet ya **no** le pasa `secondaryFooter` al shell; `CustomFieldSheet`, `SavePresetSheet` y `BreedPickerSheet` lo siguen usando (la condensación del secundario sigue siendo contrato del shell).
    - **Separación commit ↔ cierre en el caller** (`jornada.tsx`): el viejo `onConfigSave` persistía **y** cerraba en el mismo paso; ahora `onConfigCommit` **sólo** persiste (N veces, con el sheet abierto) y el cierre es `setConfigManeuver(null)` (una vez). El `onConfigCommit` trae un **guard de no-op**: si el valor trimmeado no cambió devuelve el MISMO objeto de estado. Cubre exactamente dos casos — ediciones que sólo tocan **whitespace** y el **re-commit idempotente del flush de cierre**. **NO** evita el re-render por tecla del modo `single` (cada tecla produce un valor distinto → objeto nuevo → re-render de la etapa 2, incluida `ManeuverReorderList`, que no está memoizada). Se decidió **no** memoizarla en este delta: el costo real en device no está medido, la lista está tapada por el scrim mientras el sheet está abierto, y un `React.memo` con props de callback no memoizados no haría nada → backlog del leader. El sheet se monta con `key={configManeuver}` (estado fresco garantizado por maniobra).
    - **× del chip = target manga, sin aplanar la jerarquía**: con el auto-guardado la × pasó a ser la ÚNICA acción destructiva y su efecto es **inmediato** (ya no hay "Cancelar" que lo deshaga) → su área tocable sube de un ícono de 18px con `hitSlop={8}` (~34px efectivos, bajo el mínimo de 44). **Corrección del veto visual del leader (2º fix-loop)**: el primer pase usó `$touchMin`=56 y eso infló **el chip entero** al alto del botón primario, con el mismo relleno `$primary` → con 3-4 vacunas quedaban cuatro bloques verdes pesados apilados justo arriba del "Listo" (también verde) y el ojo dejaba de distinguir cuál era la acción: **jerarquía aplanada** (Nielsen #8). Fitts pide **área táctil** ≥44, no un pill visual de 56 — el método de medición (el `boundingBox` del e2e) había terminado dictando el visual. As-built: el pill mide **`$4`=44** y la × ocupa **todo su alto** (`height="100%"`) con `minWidth="$4"` → se conservan los 44×44 de área sin que el chip compita con el CTA. El **verde se queda** (verde = "elegido" es el lenguaje ya establecido: la fila de maniobra seleccionada es verde; cambiarlo rompería consistencia). El texto va `flexShrink:1` + el chip `maxWidth:100%` para que el `flexWrap` no se rompa con nombres largos. La × se implementa como **pieza Tamagui con `onPress`** (no un `Pressable` de RN envolviendo un Tamagui con `pressStyle` — en nativo new-arch eso roba el responder), con `testID` `config-chip-remove-<valor>` para el oráculo de tamaño del e2e.
    - **`handleClose` memoizado** (`useCallback`, con lo tipeado leído de un `ref`): el shell lo recibe como `onClose` y lo mete en los `useMemo` de sus gestos; sin memoizar, esos gestos se reconstruían en cada render — o sea por tecla en modo `single`. No era un bug de correctitud (RNGH actualiza in-place), sí churn gratis.
    - **Intactos**: el orden del cuerpo (input primero, chips debajo — decisión de v7 por el teclado), el autocompletar (R1.8), el Enter que no baja el teclado en multi, el guard anti click-huérfano del scrim (vive en el shell) y el round-trip coma-separado.
    - **Cobertura**: unit `maneuver-wizard.test.ts` (`addMultiPreconfigItem` + `pendingCloseCommit`, 6 casos nuevos: 37 → 43) + e2e `maniobra-config-sheet-race.spec.ts` (test nuevo: 3 vacunas agregadas → cierre por **scrim táctil** → persistieron; sacar los chips uno por uno → el último deja el preconfig limpio y la fila vuelve a reclamar CON EL SHEET ABIERTO; × ≥44px medida por `boundingBox`) + el caso "tipear sin Agregar → cerrar por scrim → entró igual" (assert INVERTIDO del test viejo, que verificaba el descarte) + `maniobra-wizard.spec.ts` (footer con un solo CTA: `Guardar`/`Cancelar` count 0) + `sheet-teclado.spec.ts` (cerrar por la X ya no descarta) + capturas `app/e2e/captures/config-sheet-autosave.capture.ts` (11 estados).
  - **As-built v9 (bugfix 🔴 MANGA, Raf 2026-07-25, device iOS build f9b943f7) — QUIÉN ES DUEÑO DEL GESTO DE DESCARTE.** Dos bugs de la misma clase, arreglados juntos. **(1) El arrastre hacia abajo destruía la jornada.** `app/app/_layout.tsx` declaraba `maniobra` con `presentation:'modal'`, y expo-router 56 **hereda modal hacia adelante**: en `expo-router/build/react-navigation/native-stack/utils/getModalRoutesKeys.js` toda ruta posterior a un modal **sin `presentation` explícita** entra a `modalRouteKeys`, y `NativeStackView.native.js` resuelve `presentation = isPresentationModal ? 'modal' : 'card'` → `maniobra/jornada`, `maniobra/identificar` y `maniobra/carga` se presentaban como **page-sheets apilados** (`modal` → `UIModalPresentationAutomatic` en `react-native-screens/ios/RNSScreen.mm`), **cada uno con su propio dismiss interactivo**. Consecuencias observadas por Raf: desde cualquier etapa del wizard un arrastre hacia abajo **descartaba la pantalla sin confirmación** (mientras el ‹ del header es un back POR ETAPA), y con una jornada **activa** el mismo gesto **salteaba el `ExitJornadaSheet`** (cierre guardado, R10.7) — el peor caso; además el arrastre del grabber de un bottom sheet caía a ese gesto y cerraba la pantalla de abajo en vez del sheet. **Fix**: `presentation:'fullScreenModal'` + `gestureEnabled:false` explícitos en las **tres** rutas. En iOS `fullScreenModal` → `UIModalPresentationFullScreen`, para el que UIKit **no instala gesto de descarte**, y de paso la presentación pasa a coincidir con lo que el código ya afirmaba ("pantalla completa"): se recupera el alto que comían los sheets apilados; `gestureEnabled:false` es el segundo cerrojo (setea `modalInPresentation` → `presentationControllerShouldDismiss` devuelve NO). **Android sin cambios** (`ScreenViewManager.kt` mapea `modal` y `fullScreenModal` al mismo `StackPresentation.MODAL`, y expo-router fuerza `gestureEnabled=false` en Android). **Web sin cambios** (`NativeStackView.js` solo mira `presentation` para las presentaciones transparentes). **Navegación intacta**: `router.back()`/`backOr(router,'/maniobra')`/`router.replace`/`canDismiss()+dismissAll()` operan sobre el stack de JS (`POP`/`POP_TO_TOP`, `expo-router/build/global-state/router.js`), ajenos a la presentación; el landing `maniobra` **queda modal a propósito** (una pantalla sin estado que perder: ahí el swipe-down es correcto). Queda FUERA de alcance, mismo mecanismo: `/crear-animal` empujada desde la identificación sigue heredando modal y descartable por gesto (ahí el gesto pierde el alta en curso, no la jornada) → `docs/backlog.md`. **(2) El `BottomSheetShell` no era dueño de su gesto.** El shell dibujaba un grabber (`showGrip`, default `true`, ningún consumidor lo pisa) **sin ningún gesture handler** — significante sin affordance (Norman) que además hacía lo contrario de lo prometido (lo atendía el modal de abajo), cuando la convención de iOS (Jakob) es que el sheet **frontal** es el que se cierra primero. Ahora el shell implementa **arrastre-para-cerrar** con `react-native-gesture-handler` + `reanimated`, y las decisiones son **puras y testeadas** (`app/src/utils/sheet-gestures.ts` + `.test.ts`, 23 casos): ANCLA = el **header** (grabber + título), que arrastra siempre; desde el **body** solo con el `ScrollView` en el tope (`onScroll` interno → `.enabled(bodyAtTop)` del gesto — con contenido scrolleado el arrastre es del operario, no del sheet); traslación **solo hacia abajo** (no hay snap points); al soltar cierra por **distancia** (`SHEET_DISMISS_RATIO` 25% del alto medido del sheet, con piso `SHEET_DISMISS_MIN_DISTANCE` 64px para los sheets cortos) **o** por **flick** (`≥900 px/s` con ≥24px de recorrido), un flick hacia arriba **cancela** (arrepentimiento), y si no alcanza vuelve con spring; **fail-closed** ante cualquier medida no finita (volver es recuperable; cerrar de más pierde lo cargado). **DOS detectores en vistas DISJUNTAS, con su `inert` propio** (corrección del reviewer, fix-loop del mismo día): uno sobre el **header** y otro sobre el **contenido del ScrollView**; el **footer NO es ancla** de arrastre (ahí viven los CTAs). El primer pase montaba el detector del cuerpo sobre la envoltura del sheet ENTERO → los dos Pan trackeaban el mismo toque en el grabber y compartían un único flag `inert`: el `onFinalize` del gesto perdedor (que RNGH dispara también en FAILED/CANCELLED) lo reseteaba a `false` después de que el ganador lo puso en `true`, y en **iOS no hay orquestador** de RNGH (la exclusión la hace UIKit y el orden Failed↔Began es interno) → con el teclado arriba el arrastre podía **cerrar el sheet igual**, descartando lo tipeado en los sheets sin auto-guardado. Con detectores disjuntos + un flag por instancia el invariante no depende del orden de dispatch. El detector del cuerpo va sobre un `View` propio DENTRO del contentContainer (que absorbe el `gap`) y **no** sobre el `ScrollView`: con el detector en el `ScrollView` de Tamagui, gesture-handler-web aplica su `touchAction` a un nodo que no es el scroller y el pan del cuerpo queda **muerto en web** — eso se **midió ad-hoc durante el desarrollo** (dump del DOM + el control at-top del e2e dando traslación 0) y **no es cobertura viva**: la alternativa no está en el árbol, así que el spec no puede ejercitarla. Lo que **sí** queda lockeado (y falsificado: cae con `touch-action: none` si se saca la prop) es que el detector del cuerpo lleve `touch-action: pan-y`, leído del testID `<sheet>-body-drag` — que es **la vista del propio detector**, donde gesture-handler-web escribe el estilo (`GestureHandlerWebDelegate`), no el scroller. Sin esa prop, el default `none` de RNGH **mataría el scroll táctil del body de los 4 sheets en web** (clase 🔴 manga). **CRASH 🔴 EN DEVICE Y SU FIX (Raf, iOS build 76f0837c)**: la primera versión de esa rama hacía `runOnJS(Keyboard.dismiss)()` y **arrastrar el grabber con el teclado abierto crasheaba la app entera** (crash NATIVO, no error de JS). Causa, verificada sobre la salida REAL de babel del componente + el serializador de `react-native-worklets` 0.8.3 (no sobre la hipótesis inicial de "se pierde el `this`", que quedó **refutada**: el `dismiss()` de RN 0.85 no usa `this`): el plugin captura en el `__closure` el **identificador raíz** de la expresión → capturaba el objeto `Keyboard` ENTERO (instancia de la clase `KeyboardImpl`); `createSerializable` solo clona objetos con prototipo `Object.prototype`, host objects y TurboModules, así que una instancia de clase cae en `inaccessibleObject()` → en el runtime de UI queda un **Proxy que tira ante cualquier acceso**; por eso no explotaba al abrir el sheet sino al **leer** `Keyboard.dismiss`, o sea **solo en la rama del teclado abierto**, y un throw dentro de un callback de gesto en el hilo de UI (release) revienta nativo. **Fix**: callback JS propio y estable (`dismissKeyboard = useCallback(() => Keyboard.dismiss(), [])`) → el closure captura una función común (`remoteFunction`) y `runOnJS` la agenda sin tocar módulos; verificado recompilando con babel que el `__closure` pasó de `{…, Keyboard}` a `{…, dismissKeyboard}`. **Auditados los 6 worklets del shell**: el resto captura shared values, funciones workletizadas de `sheet-gestures.ts` (confirmado en la salida de babel: las 4 llamadas desde el hilo de UI tienen `__workletHash`), números y `onClose` (los 4 consumidores pasan arrow functions o `useCallback` locales (nunca un método de módulo), ninguno un método). **Guard permanente**: `app/src/components/worklet-callbacks-guard.test.ts` escanea `app/app` + `app/src` y falla si a `runOnJS`/`scheduleOnRN` se le pasa un `X.y` (falsificado: con la línea vieja el test cae y señala archivo:línea). **CONTENCIÓN GENERAL (del mismo pase)**: los 5 callbacks del gesto quedaron **envueltos en `try/catch`**, porque el problema de fondo es más grande que esa línea — *cualquier* excepción no atrapada dentro de un worklet mata la app (el guard de worklets, `callGuardDEV`, **solo existe en builds de debug**: su propio archivo dice "Used only with debug builds"). Ahora un error degrada a "el gesto no hace nada" (fail-closed, misma política que la geometría no finita) en vez de cerrar la app en medio de una jornada; en **DEV se re-lanza** para no tapar el error (ahí sí hay `callGuard` y sale por el canal normal). La recuperación de cada `catch` solo escribe shared values, que en la práctica no pueden re-tirar (son host objects serializables). NO es una garantía absoluta: si lo que tiró dentro del `try` fue justamente el acceso a `inert` o `translateY`, la escritura del `catch` tiraría igual y volveríamos al mismo abort. El riesgo es ~0 porque son `useSharedValue` legítimos del propio componente, pero el contrato no lo excluye. **Con el teclado ARRIBA el arrastre BAJA EL TECLADO y NO cierra** (`sheetDragIntent`, una sola conducta): lo que se está tipeando no se pierde por un gesto, el idiom ya existe (`keyboardDismissMode:'on-drag'`) y no se pelea con el re-layout del shell del teclado (en Android, desde la v11, ese re-layout es un `useAnimatedStyle` que corre en el MISMO hilo de UI que este gesto); la salida sigue a un toque en la X (que no se condensa nunca). **Estructura**: el sheet se envuelve en un `Animated.View` que **toma las restricciones de caja** (`flexShrink:1` + `maxHeight`) que antes tenía el `YStack` — un `maxHeight` en % necesita un padre de alto definido, y el padre pasó a ser la envoltura de alto auto; el `YStack` conserva `flexShrink:1` y se comporta igual (contenido corto → se dimensiona al contenido; contenido alto/teclado → se achica cediendo al body, **nunca `flex:1`**, bug U5). En web el detector del cuerpo va con `touch-action:'pan-y'` para no comerse el scroll táctil (el default de RNGH es `none`); el del header queda en `none`. **Intactos**: guard anti click-huérfano del scrim (doble-rAF), condensación con teclado, esqueleto header/body/footer, safe-area, `maxHeight`, todos los `testID` (se suma `<sheet>-grip`) y la API `secondaryFooter` (la usan 3 consumidores). **(2-bis) El body se cortaba al ras del CTA, sin aviso (veto visual del leader).** El `contentContainerStyle` no tenía `paddingBottom` y el indicador de scroll está apagado → con el body desbordado (p. ej. Vacunación con el alto útil recortado por el teclado) el último elemento quedaba **rebanado pegado al botón**, sin aire ni señal: se lee como layout roto en vez de "seguí scrolleando". Al vivir en el primitivo lo heredaban los 4 sheets. As-built: **peek** (aire al final del contenido, mismo token `$6` que el peek de `FooterActionShell`) + **fade + chevron ▾** en el borde inferior del body cuando queda contenido oculto abajo, con la **misma decisión pura** que el resto del repo (`shouldShowScrollPeek` de `utils/footer-action.ts` — una sola fuente de verdad del affordance, compartida con `FooterActionShell` y las listas de maniobra). El fade es `pointerEvents="none"` (no intercepta scroll, arrastre ni taps) y cuelga de una envoltura del `ScrollView` con **la misma geometría flex que tenía el ScrollView** (`flexShrink:1` + `minHeight:0`, basis auto) → no es el patrón del bug U5, que era `flex:1` (basis 0%) colapsando a altura 0 con contenido corto. Capturado en el peor caso (412×420 con 6 vacunas cargadas). **(3) El BACK FÍSICO de Android cerraba la RUTA, no el sheet.** Mismo defecto de clase, en la plataforma donde el gesto no es un descubrimiento accidental sino el botón que el operario usa todo el tiempo: no había **ningún** `BackHandler`/`beforeRemove`/`usePreventRemove` en toda la app, así que con un sheet abierto el back hacía **pop de la ruta** (en el wizard, chau jornada). El shell registra ahora un `BackHandler` mientras está montado que **cierra el sheet por el MISMO `onClose`** (ahí vive el flush de lo tipeado-sin-agregar del `ManeuverConfigSheet` — un `setState(false)` directo perdería el dato) y **CONSUME** el evento (regla pura `sheetBackPress`). Sheets superpuestos: suscripción **única al montar** + orden inverso de RN ⇒ atiende el de más arriba. Solo Android (predicado puro `sheetBackHandlerApplies`, testeado). **PRECONDICIÓN DE ADOPCIÓN** (declarada en el contrato del DS): el handler se registra al **MONTAR**, no al "abrir" — el shell no tiene noción de abierto/cerrado. Los 4 consumidores desmontan al cerrar, así que hoy solo intercepta con sheet a la vista; un consumidor que lo dejara montado detrás de un toggle de visibilidad **se comería todos los back de Android de la app en silencio** (ese patrón existe en el repo: `carga.tsx` renderiza `LotePickerSheet` siempre montado con prop `open`). La guarda a nivel de PANTALLA (que el back retroceda de etapa en `jornada` y abra el `ExitJornadaSheet` en `identificar`) es de la otra terminal, en sus archivos. **Cobertura**: unit `sheet-gestures.test.ts` (23: arrastre + back + gate de plataforma) + e2e `app/e2e/sheet-arrastre.spec.ts` (arrastre corto NO cierra y vuelve / largo SÍ cierra / el sheet sigue al dedo — anti-falso-verde / gate de scroll con control at-top / **el footer no arrastra y su CTA sigue tappable**) + capturas `app/e2e/captures/sheet-arrastre.capture.ts` (8 estados, incluida "la jornada sigue viva tras cerrar el sheet por arrastre") + regresiones verdes (`sheet-teclado` 3/3, `maniobra-config-sheet-race` 4/4, `maniobra-customfield-validacion`, `maniobra-custom` 2/2, `maniobra-custom-gestion` 5/5, `maniobra-wizard`, `maniobra-sanitaria` 6/6, `maniobra-rutinas-gestion` 4/4, `maniobra-offline`, `sigsa-breed-renspa` 4/4, `maniobra-identify` 16/16) + `check.mjs` RC=0. **LÍMITE (ADR-029)**: el gesto real con el dedo, que el modal de la jornada ya no se descarte, **el back de Android** y la conducta con el teclado arriba son **veredicto de DEVICE** (react-native-web no tiene gesto modal de iOS, ni teclado virtual, ni back físico).
  - **As-built v10 (delta 2026-07-25) — BACK DE HARDWARE de Android en las 3 pantallas del flujo (R1.2 / R10.7 / R5.15).** Mismo defecto que el gesto de descarte de la v9, por **otro camino** y en la plataforma donde el back no es un descubrimiento accidental sino el botón que el operario usa todo el tiempo. Grep autoritativo previo al delta: **no existía ni un `BackHandler`, ni un `beforeRemove`, ni un `usePreventRemove` en todo el repo** → el back físico hacía `pop` de la ruta sin pasar por ninguna guarda (el `presentation:'fullScreenModal'` + `gestureEnabled:false` de la v9 **no lo cubre**: eso sólo toca el dismiss interactivo de iOS). Consecuencias: en `jornada.tsx` el back desde la etapa 2/3 **destruía la configuración entera** (el ‹ sí retrocede de etapa); en `identificar.tsx` **salteaba el `ExitJornadaSheet`** con la jornada ACTIVA; en `carga.tsx` abandonaba el animal dejando **huérfanas** las filas de evento que R5.8 ya persistió por paso.
    - **Regla**: el back de hardware hace **exactamente** lo que el chevron ‹ de esa pantalla, reusando SU handler (nada de un segundo camino que después se desincronice). Donde no hay ‹ (`carga.tsx`, a propósito), va a la **salida guardada** de esa pantalla.
    - **Mecanismo**: hook `app/src/hooks/useHardwareBack.ts` (`BackHandler` + `useFocusEffect`), acotado al FOCO (las pantallas del stack quedan montadas al navegar: sin el foco habría dos listeners vivos) y **gateado a Android** por el predicado puro `shouldRegisterHardwareBack` (iOS no tiene botón atrás; en web `BackHandler` no emite). El handler **siempre consume** (`return true`) → nunca se popea por atrás de una guarda.
    - **Decisiones puras** en `app/src/utils/maniobra-back.ts` (`jornadaBackAction` / `identifyBackAction` / `cargaBackAction`), testeadas en Node (`maniobra-back.test.ts`, 18 casos): con una guarda ya abierta el back la cierra **por su camino existente** (el mismo del scrim / "Cancelar" / "Seguir en la jornada"), y **ninguna pantalla corre su salida mientras hay un modal arriba** (invariante con test propio).
    - **PRECEDENCIA sheet > pantalla — LEÍDA en la fuente, no asumida**: `node_modules/react-native/Libraries/Utilities/BackHandler.android.js` guarda los handlers en un array (`push`) y el dispatcher itera `for (i = length-1; i>=0; i--)` devolviendo en el PRIMERO que retorna truthy → el último registrado corre primero y corta la cadena (incluida la del navigation container, que es la que popearía). Como el sheet monta DESPUÉS que la pantalla, gana.
    - **La condición de esa precedencia son DOS cosas, no una** (corregido en el 2º fix-loop: esta entrada decía sólo la primera). Vale mientras **(i)** el registro de la pantalla sea ESTABLE entre re-renders — de eso se ocupa el hook (`useCallback(..., [])` + handler en un `ref`) — **y (ii)** la pantalla NO se RE-ENFOQUE con un sheet ya montado: `useFocusEffect` de expo-router re-ejecuta su callback en **cada** evento de `focus` (`node_modules/expo-router/build/useFocusEffect.js`), o sea que un re-foco RE-SUSCRIBE a la pantalla y la manda al FINAL del array, robándole la precedencia al sheet. Hoy (ii) es **inalcanzable** (el wizard es la única pantalla con el hook que hospeda sheets del shell, y ninguno de ellos navega), pero alcanza **un solo `router.push` agregado adentro de cualquiera de esos sheets** para volverlo alcanzable — y el síntoma sería un **back muerto**, invisible para web, E2E y unit.
    - **Por eso la decisión NO da la precedencia por garantizada** (degrada explícito en vez de callarse): `jornadaBackAction` separa el sheet que la pantalla **no puede** cerrar del que **sí**. `'defer-to-preconfig-sheet'` queda sólo para el `ManeuverConfigSheet` —su cierre arrastra el flush del texto tipeado (As-built v8) y desde la pantalla no hay forma de ejecutarlo— y el caller **avisa por `console.warn` en dev**; `SavePresetSheet`/`CustomFieldSheet` caen en `'close-shell-sheet'`, un **cierre de último recurso** que no corre en el camino feliz pero convierte un desliz de precedencia en un cierre correcto en vez de un botón inerte. Con test de invariante: `defer` está acotado al preconfig y ningún otro estado lo devuelve.
    - **Una sola fuente de verdad para "el sheet está MONTADO"** (corregido en el 2º fix-loop): la bandera que alimenta la decisión y la guarda de render salen de la MISMA expresión (`preconfigSheetMounted` = `configManeuver && FREE_TEXT_PRECONFIG[configManeuver]`; `customSheetMounted` = `isOwner && customSheetOpen`). Antes se calculaban por separado y podían divergir —bandera en true con la guarda de render en false → deferir a un sheet que no está en pantalla = back muerto— alcanzable con un flip de rol a no-owner con el sheet abierto.
    - **Cobertura y su límite (ADR-029)**: `BackHandler` no emite en web → **Playwright no puede disparar el back físico**; el comportamiento real es **veredicto de device Android**. La E2E nueva (`app/e2e/maniobra-back-hardware.spec.ts`, 2 casos) blinda el **contrato que el back espeja**: el ‹ del wizard vuelve de la etapa 3 a la 2 **con la config intacta** y de la 2 a la 1 sin salir; el ‹ de la identificación abre el `ExitJornadaSheet` y "Seguir en la jornada" lo cierra sin navegar. **Se probó y se DESCARTÓ** un assert de "consola limpia" sobre el `console.error` del stub de `BackHandler` de react-native-web: sacando el gate de plataforma ese error NO aparece en el export web de este repo (un probe en el mismo lugar sí se captura) → el assert no podía fallar nunca y se sacó en vez de dejar un verde falso. El gate a Android queda entonces como **contrato de plataforma** (no registrar un listener que no puede disparar), cubierto por unit.
  - **As-built v11 (bugfix 🔴 device Android, Raf 2026-07-26, APK release `7402575a`) — EL TECLADO TAPABA EL SHEET ENTERO EN ANDROID → primitivo `KeyboardAvoidingShell`.** La v7 dio por resuelto Android delegando en el `adjustResize` de la ventana. **Eso hoy es falso** y el bug volvió tal cual en la otra plataforma: al enfocar el input de "Vacunación" en el Samsung de Raf el teclado tapa el sheet completo (en iOS el mismo sheet sube bien). Fallan DOS mecanismos apilados: **(a)** `KeyboardAvoidingView` con `behavior={undefined}` cae en la rama `default` de RN, que renderiza un `<View>` pelado — en Android **no hace nada**; **(b)** el `adjustResize` en el que se apoyaba está **desactivado por el edge-to-edge** del build (`gradle.properties edgeToEdgeEnabled=true` → `BuildConfig.IS_EDGE_TO_EDGE_ENABLED` → `ReactNativeApplicationEntryPoint.setEdgeToEdgeFeatureFlagOn()` → `WindowCompat.setDecorFitsSystemWindows(window, false)`): la ventana deja de encogerse y en todo `ReactAndroid` las tres referencias al inset del IME (`ReactRootView.java:971,976,978`) viven en un método que **solo emite un evento a JS**. No se puede apagar (`@expo/prebuild-config` removió la llave de `app.config.ts` —"Android 16 makes edge-to-edge mandatory"—, `/android` está gitignoreado y `expo-modules-core` lo re-enciende con default `?: true`). **(c)** Tampoco alcanzaba con cambiarle el `behavior` a `'padding'`: la altura que RN emite (`ReactRootView.java:978`, `imeInsets.bottom - barInsets.bottom`) **le resta la barra de navegación** (~48dp en ese device), cálculo del mundo pre-edge-to-edge. **Fix**: primitivo **`app/src/components/KeyboardAvoidingShell.tsx`** (iOS/web, byte-idéntico a lo que había: `behavior='padding'` en iOS, `<View>` en web) **+ `KeyboardAvoidingShell.android.tsx`** (resolución por extensión de plataforma, como `AppleSignInButton.native.tsx` — un hook no puede llamarse condicionalmente) que aplica `paddingBottom = alto del teclado` con **`useAnimatedKeyboard` de Reanimated 4.3.1** (ya instalado, cero deps nuevas): su nativo hace la corrección que a RN le falta (`isNavigationBarTranslucent` → `systemBarBottomInset = 0` → **inset COMPLETO del IME**), y `EDGE_TO_EDGE` lo activa solo. **Total del padding, sin doble conteo**: el shell descuenta el teclado entero (que bajo edge-to-edge ya incluye la franja de la barra de navegación) y el footer aporta solo `keyboardOpenGap` = `$2` (`resolveFooterPaddingBottom` descarta la safe-area con el teclado arriba) → el CTA queda a **$2 del borde del teclado**. **Los 4 call sites** migran al primitivo: `BottomSheetShell`, `FooterActionShell`, `maniobra/carga.tsx` y `AuthScreenShell` (el login: ahí el CTA no está en un footer fijo, el mecanismo es el SCROLL y lo que lo habilita es que el viewport se achique). **Guard de clase** `app/src/components/keyboard-avoiding-guard.test.ts` (registrado en `run-tests.mjs`): prohíbe el `KeyboardAvoidingView` de RN fuera del primitivo, exige que la base conserve el `behavior='padding'` de iOS, que el `.android.tsx` aplique de verdad `paddingBottom: height.value`, y que los 4 call sites sigan usando el shell. **`useAnimatedKeyboard` está `@deprecated`** (apunta a `react-native-keyboard-controller`): decisión tomada — deprecated ≠ roto, y meter un módulo nativo nuevo contra RN 0.85 + new arch + Expo 56 justo cuando iOS no se puede re-testear es peor riesgo. Migración anotada en `docs/backlog.md`. **Límite (ADR-029)**: en web no hay teclado virtual (`Keyboard` de RNW nunca emite y el KAV es un `<View>` inerte) → **este bug es estructuralmente invisible desde la E2E**; el veredicto es DEVICE. Lo que sí se verifica en web es que iOS/web no se movieron.
  - **As-built v12 (bugfix 🔴 device Android, Raf 2026-07-28, APK `a3b8d804` / commit `56beff3`) — ABRIR UN SHEET BAJA EL TECLADO.** Con el input de caravana de `identificar.tsx` ENFOCADO y el teclado ABIERTO, tocar la ‹ del header —que abre el `ExitJornadaSheet` (R10.7)— dejaba **el teclado arriba** y del sheet solo asomaba una franja de ~25px: sus dos botones ("Terminar jornada" / "Salir sin terminar") quedaban TAPADOS. **Causa doble**, y la v11 solo describía media: (a) el `ExitJornadaSheet` **no monta `KeyboardAvoidingShell`** —no tiene campos de texto, así que el guard del teclado no se lo exige— o sea que para él **el lift nunca existió**; (b) aunque lo montara, el shell tiene un límite declarado: si MONTA con el teclado ya abierto arranca en altura **0** hasta el próximo evento de insets. ⚠️ El header de `KeyboardAvoidingShell.android.tsx` despachaba ese límite como *"paridad con iOS, no regresión"* y afirmaba que *"el flujo del bug reportado no lo toca"*: **las dos afirmaciones eran falsas** y quedaron corregidas en el archivo. **Fix (capa 1, y es la conducta correcta de producto)**: abrir un overlay modal es SALIR del contexto de escritura —el input queda detrás del scrim, inalcanzable— → se descarta el teclado. Hook `app/src/hooks/useDismissKeyboardOnOpen.ts` (decisión pura `shouldDismissKeyboardOnOpen` en `utils/sheet-shell.ts`, testeada), que dispara **solo en el flanco cerrado→abierto** (si disparara en cada render, un sheet con input propio cerraría su propio teclado en cada tecla). Adoptado en los **22 archivos que dibujan un `$scrim`**: `BottomSheetShell` (cubre a `ManeuverConfigSheet`, `CustomFieldSheet`, `SavePresetSheet`, `BreedPickerSheet`) + los 21 hechos a mano — `ExitJornadaSheet`, `CandidatePicker`, `OtherRodeoSheet`, `SugerenciaVaciasSheet`, `SkipAnimalSheet`, `SyncRechazoSheet`, `NuevaJornadaConfirmSheet`, `ConfirmDeleteSheet`, `CustomFieldActionsSheet`, `PresetActionsSheet`, `TactoConfigSheet`, `LotePickerSheet`, `DientesStep` (CutPromptSheet), `CircunferenciaEscrotalStep` (AgeAdjustSheet), `FindOrCreateOverlay`, `TagScanSheet`, `LinkCalfPrompt`, `BulkConfirmSheet`, `TreatmentStartSheet`, `TreatmentApplicationSheet`, `MarkDeclaredSheet`. Los que viven SIEMPRE montados detrás de una prop de visibilidad pasan esa prop (`LotePickerSheet`, `SugerenciaVaciasSheet`, `LinkCalfPrompt`, `MarkDeclaredSheet`, y `FindOrCreateOverlay` con `state !== null`); el resto usa el default. **ÚNICA EXCEPCIÓN, semántica (`claimsKeyboard`)**: el sheet que `autoFocus`ea su propio input no está SALIENDO del contexto de escritura sino **ENTRANDO** a uno → lo declara y el shell no descarta nada. Encontrada **falsificando, no razonando**: `SavePresetSheet` (el único `autoFocus` del repo) perdía el foco de su input con el descarte puesto, porque en web el `commitMount` de React enfoca al hijo dentro del MISMO commit, o sea ANTES del efecto del padre. **Guard**: `app/src/components/sheet-keyboard-dismiss-guard.test.ts` — todo archivo con `$scrim` llama al hook o monta `BottomSheetShell` (que está en la misma semilla, así que la cobertura de sus 4 consumidores queda anclada); chequea además que el hook no sea decorativo, que un sheet con prop `open` no lo llame sin argumento, y las DOS direcciones de `claimsKeyboard` (todo sheet con `autoFocus` lo declara **y** todo `claimsKeyboard` declarado muestra evidencia de auto-foco en el archivo — marcarlo de más apaga el descarte y revive el bug en silencio). **Cierre de la enumeración**: la semilla `$scrim` garantiza una sola dirección (todo `$scrim` del repo es un backdrop modal anclado abajo), no la recíproca, así que una regla secundaria enumera los overlays a pantalla completa por su GEOMETRÍA (`StyleSheet.absoluteFill` / `<Modal` de RN / capa absoluta con los 4 insets en 0 y sin `pointerEvents="none"`) y exige que cada uno SIN `$scrim` esté nombrado con su razón. Hoy hay exactamente uno: `EstablishmentSwitcherDropdown` (spec 01 R6.8.1) pinta su backdrop con `$textPrimary` @0.18 y queda fuera del invariante porque **se ancla ARRIBA** (`top={anchorTop}`, bajo el header — el teclado se dibuja desde abajo y no puede taparlo), no tiene campos de texto propios y su único call site es la home, que tampoco los tiene. Las tres razones están ancladas en el test: si alguna deja de valer, cae. **Capa 2 (sembrar la altura al montar) EVALUADA Y DESCARTADA**: la única fuente al montar es `Keyboard.metrics()`, cuyo `height` es el de RN —el que está mal bajo edge-to-edge (`imeInsets.bottom - barInsets.bottom`)— y el término de corrección no es verificable desde web ni desde el unit; un término equivocado da un lift equivocado, peor que no levantar nada. Queda en `docs/backlog.md` con el razonamiento, atado a la migración a `react-native-keyboard-controller`. **Cobertura y su límite (ADR-029)**: que el IME baje es veredicto de DEVICE (RNW no monta teclado virtual). `e2e/sheet-baja-teclado.spec.ts` verifica el MECANISMO por su única huella web —`Keyboard.dismiss()` de RNW blurea el input enfocado— usando el ÚNICO disparador que no toca el foco por su cuenta: un **bastonazo** (con un click o un Enter el browser desenfoca igual y el test pasaría sin el fix: se descubrió falsificando). Falsificado en las dos direcciones: sin el hook cae el test del mecanismo; con el hook disparando en cada render cae el de no-regresión del sheet con input propio.
...
> **DM1-UI-1 (reconciliación as-built, chunk M1-UI) — fuente del autocompletar de preconfig (R1.8).** El wizard (etapa 2) ofrece preconfig de texto libre para las maniobras que lo admiten (vacuna(s) → `vacunacion`, pajuela → `inseminacion`) con **autocompletar** (`filterAutocomplete`, util pura). La **fuente** del histórico de autocompletar en M1-UI son los valores de `preconfig` YA usados en los **presets** del establecimiento (`fetchPresets` → `config.preconfig`), NO una query distinct sobre los eventos tipados (`sanitary_events.product_name` / pajuela de `reproductive_events`). Razón: ese distinct-query exigiría un SQL builder + service nuevos (capa de servicios, cerrada por M1-SERVICIOS) y la pajuela no tiene columna tipada limpia. NO cambia el *qué* de R1.8 — los presets SON valores previamente cargados por el campo. El helper es source-agnóstico → M2/M3 enchufan una fuente más rica (valores cargados en la manga) sin reabrir esto. (As-built v4: el `history` se pasa al `ManeuverConfigSheet`, que filtra las sugerencias con `filterAutocomplete` contra lo tipeado — antes vivía en el `PreconfigField` del fondo de la etapa, ya removido.)
  - **As-built v13 (unidad «barrida de teclado», 2026-07-27) — el fix de la v11 cubría 4 archivos; faltaban 23 superficies MÁS.** Raf verificó la v11 en device (el sheet de Vacunación sube) y **enseguida encontró el mismo bug** en `app/app/maniobra/identificar.tsx`. Causa: la v11 arregló la población de los archivos que montaban un `KeyboardAvoidingView` **mal configurado**; existía una **segunda población** —superficies con **AUSENCIA total** de mecanismo— que el guard de la v11 no podía ver por construcción (busca el uso incorrecto de un componente, no la falta de él). Eran **23 superficies** con campo de texto y cero keyboard-avoidance, **7 de ellas 🔴 manga**. **Esto corrige dos afirmaciones de la v7**: (a) *"Sheets NO migrados … `TagScanSheet` (territorio de otra terminal). Los que tienen input de texto siguen expuestos al mismo bug hasta migrarlos → backlog"* — eran **SEIS** sheets a mano con input (`TagScanSheet`, `FindOrCreateOverlay`, `SugerenciaVaciasSheet`, `TreatmentStartSheet`, `TreatmentApplicationSheet`, `LinkCalfPrompt`), no uno, y la entrada del backlog registraba solo el primero; (b) el alcance del contrato: **no es "todo sheet con input"**, es **toda SUPERFICIE con un campo de texto** (pantallas incluidas). **De esta spec**: `maniobra/identificar.tsx` (la columna header + hero `flex:1` + banda de entrada manual va dentro del primitivo; los 4 sheets hermanos quedan AFUERA — cada uno trae el suyo y anidar dos shells descuenta el teclado dos veces) y `maniobra/_components/SugerenciaVaciasSheet.tsx` (sheet a mano: el shell envuelve la columna DENTRO del scrim, que sigue cubriendo la pantalla entera). **`maniobra/carga.tsx` no cambia de conducta**: su composición `useKeyboardVisible + useSafeBottomInset + resolveFooterPaddingBottom` se reemplazó por el hook que la encapsula (abajo), con valores idénticos. **Hook nuevo `useKeyboardAwareBottomInset(opts)`** (`app/src/hooks/useSafeBottomInset.ts`): reserva canónica con el teclado cerrado, **solo `$2`** con el teclado abierto. Existe porque el shell y la safe-area se apilan: el shell sube el contenedor el alto ENTERO del teclado y la safe-area del SO queda tapada → reservarla otra vez deja **~64dp de hueco muerto** (Android 3 botones). Se descartó volver `useSafeBottomInset` keyboard-aware: cambiaría las ~40 llamadas que **no** suben con el teclado (el bottom-nav, que lo dibuja el Navigator fuera de todo shell), en un cambio que **web no puede ver** (`Keyboard` de RNW nunca emite) y que por lo tanto solo se detectaría en device. Los 3 sitios de la v11 (`FooterActionShell`, `BottomSheetShell`, `maniobra/carga`) migraron al hook → **una** definición, cero copias. **Pantallas de TAB**: `(tabs)/_layout.tsx` suma `tabBarHideOnKeyboard: true` — el bottom-nav vive FUERA de la pantalla, así que ningún shell puede subirlo y quedaba entre el contenido y el teclado (su alto entero de hueco muerto). **Guard DADO VUELTA** (`app/src/components/keyboard-avoiding-guard.test.ts`): a la REGLA A (nadie monta el componente de RN fuera del primitivo) se le suma la **REGLA B** — enumera estáticamente **todo** archivo con entrada de texto (`<TextInput>` / `<Input>` / `<TextArea>`, más el cierre transitivo por los componentes que los envuelven) y exige que cada uno esté **clasificado**: cubierto (calculado por PUNTO FIJO desde el primitivo, no por lista escrita a mano → si un shell intermedio deja de montarlo, cae todo lo que colgaba de él), PARTE reusable declarada con motivo (y entonces el guard exige cobertura en cada consumidor), o EXCEPCIÓN con motivo **y** marcador en la cabecera del propio archivo. Un archivo nuevo con un input sin clasificar deja el guard en ROJO. Falsificado con 3 mutaciones sobre el árbol real (archivo nuevo sin clasificar / sacarle el wrap a `identificar` / que `AuthScreenShell` deje de montar el primitivo → 9 pantallas de auth en rojo), revertidas. **Límite del guard, declarado**: es de granularidad de ARCHIVO (verifica que el archivo monte un contenedor, no que CADA input esté adentro) — el caso real que lo motiva es `crear-animal` ↔ `LinkCalfPrompt`, que se cerró dándole al prompt su propio shell. **Límite (ADR-029)**: web no monta teclado virtual → **el lift sigue siendo invisible desde la E2E**; el veredicto es DEVICE, Android **e iOS** (estas 23 estaban rotas en las dos plataformas). **Fix-loop del reviewer (2026-07-27)**: (a) la afirmación *"la semilla es completa por construcción"* era **falsa** —un `import { TextInput as Campo }` se le escapaba al tag JSX— y se cerró sumando el handler `onChangeText` a la semilla + un test que la contrasta contra un **oráculo independiente** (`keyboardType=|secureTextEntry|multiline=|autoCapitalize=`): 0 archivos sin clasificar; (b) los 4 guards que escanean archivos suman una **auto-verificación de cobertura** (`app/src/utils/scan-coverage.ts`): piso de archivos escaneados + balance de llaves por archivo post-blanqueo, o sea el guard verifica su PROPIA entrada antes de declararse verde; (c) la métrica del blanqueo viejo se corrigió a **556 líneas de código invisibles en 6 archivos** de `app/app`+`app/src` contra `fc4d164` (la v13 decía 1008/57, que no reproduce).
...
- **SilentVaccinationStep**: título + chips + input + autocompletar viven dentro de la card que ocupa el alto (scrollea adentro si entran muchas vacunas); CTA abajo. Top-aligned (los chips agregados empujan hacia abajo, UX natural de form multi-add). *(**SUPERADO por el delta D2 `vacunas-aplica-no-aplica`**: el input+chips+autocompletar por-animal se reemplazó por un CHECKLIST APLICA/NO-APLICA de las vacunas definidas en la tanda — la card dominante figura-fondo se conserva; las filas del checklist scrollean adentro. Ver "Deltas posteriores" + notas as-built R6.1/R1.7.)*
...
- **E2E del sub-estado `manual`**: `transport==null` NO es reproducible con el adapter-mock en web (siempre tiene transporte) → se fuerza con un flag SECUNDARIO de E2E `__MITROPERO_BLE_E2E_MANUAL__` (doble-gateado por `__MITROPERO_BLE_E2E__` → `ProviderMode='manual'` → `instantiateTransport` devuelve null). Sin superficie de producción (prod siempre `'auto'`; ninguna marca seteable desde la UI).
...
### 6.bis.9-bis Bugfix U3 (as-built, 2026-07-21) — el alta desde una jornada de TACTO NO captura preñez (dato duplicado)

Bugfix U3 de `docs/plan-mejoras-2026-07-20.md` (Tier 1, atómico). Frontend puro (no toca backend/schema/RLS). Continúa el hilo de §6.bis.9 (alta desde la manga → carga). Raf lo reportó en device: al dar de alta una hembra **durante** una jornada que mide tacto, el ALTA preguntaba la preñez; pero la maniobra la volvía a pedir sobre ese mismo animal → **dos** eventos `reproductive_events` `tacto` el mismo día (dato duplicado/basura en el momento de carga).

**Causa raíz**: el alta guiada (`crear-animal.tsx`) capturaba la preñez (campo "Estado de preñez" → `addTacto` post-create) sin saber que la jornada activa iba a tactar a ese mismo animal. Todas las categorías que muestran el campo de preñez en el alta (`multipara`, `vaca_segundo_servicio`, `vaquillona_prenada` — `animal-category-fields.ts`) están en `PROVEN_FEMALE_CATEGORY_CODES`, así que el `TactoStep` de la maniobra SIEMPRE les aplica (`appliesToAnimal('tacto', …)`, §6.bis "as-built v3") → la duplicación era sistemática, no un edge.

**As-built (fix)**:
- **`app/src/utils/maneuver-config.ts`** — helper puro `sessionMeasuresPregnancy(config)` = ¿la jornada incluye la maniobra `tacto`? (la única cuyo evento reproductivo es un `tacto` de preñez — `MANEUVER_DATA_KEY_REQS.tacto`). `tacto_vaquillona` (aptitud) NO cuenta. Unit-testeado.
- **`app/app/crear-animal.tsx`** — con `maneuverSessionId` presente, lee la sesión del SQLite local (`getSessionById`, offline) y computa `sessionMeasuresPreg = sessionMeasuresPregnancy(session.config)`. `showPregnancy = categoryFields.includes('pregnancy') && !sessionMeasuresPreg` → suprime el CAMPO (render) **y** el `addTacto` post-create (vía `pregnantCaptured = showPregnancy && …`). La maniobra queda como ÚNICA dueña del tacto → 1 solo evento.
- **Default seguro (sin regresión / sin pérdida de dato)**: `sessionMeasuresPreg` arranca `false` y solo pasa a `true` si la lectura de la sesión resolvió OK y la jornada mide preñez. Mientras carga / si la lectura falla → preñez visible (comportamiento actual). La carrera es invisible: la lectura arranca en el montaje (paso 1) y el campo de preñez vive en el paso 4 del wizard.
- **Efecto en el `categoryOverride`**: al no capturar preñez, `pregnantCaptured=false` → para `vaquillona_prenada` el override pasa de `false` a `true` (categoría PINEADA a la elegida por el operario — comportamiento "vaca comprada" A5, benigno; para `multipara`/`vaca_segundo_servicio` el override YA era `true`, no cambia). La categoría elegida se preserva; la maniobra registra el tacto+.

**Reconciliación**: NO cambia el *qué* de R4.1 (find-or-create inline + alta→carga sigue igual); REFINA el contrato del alta-en-contexto-maniobra: el alta cede la captura de preñez a la jornada cuando ésta mide tacto. Fuera de una jornada de tacto (alta normal o jornada sin tacto) el alta pregunta preñez como siempre. Nota de reconciliación agregada en `requirements.md` bajo R4.1. Cross-ref en spec 02 (alta guiada, refinamiento B de preñez). Verificado con `app/e2e/maniobra-alta-prenez-dup.spec.ts` (suppression + 1 solo tacto server-side + control jornada-sin-tacto) + unit `sessionMeasuresPregnancy`.
...
### 11.5 Delta de sync rules (feature 15, `sync-streams/mitropero.yaml`) — frontera WAL (R13.20/R13.21)

> **El archivo real es `sync-streams/mitropero.yaml` (raíz del repo), NO `app/powersync/sync-rules.yaml`** (ese path del header viejo del spec no existe as-built; la regla viva está en la raíz, ADR-025/026). El `AppSchema` local vive en `app/src/services/powersync/schema.ts`.
...
>    - **Reconciliación (as-built v7, 2026-07-25)**: el `flex:1` del cuerpo de este FIX 3 quedó **superado**. Tanto `CustomFieldSheet` como `ManeuverConfigSheet` (y `SavePresetSheet` + `BreedPickerSheet`) ya no arman el esqueleto a mano: usan el primitivo **`BottomSheetShell`**, cuyo body va **`flexShrink:1` + `minHeight:0`** (NUNCA `flex:1` — bug U5 de colapso a altura 0 en nativo con contenido corto). Header/footer fijos se conservan tal cual, y el shell relaya `scrollViewRef` / `onBodyLayout` / `onBodyContentSizeChange`, con el `testID="custom-field-scroll"` todavía en el viewport scrolleable → **el oráculo de geometría del e2e no cambia**. Lo que SÍ cambió (fix-loop 2026-07-25, ver más abajo): el editor de Opciones pasó a **input → error inline → chips**, así que el error inline sigue PEGADO al input (ahora entre input y chips, no al final del bloque) y el **cálculo del scroll-al-campo descuenta la cola de chips** del alto crítico. El contrato (scroll-al-campo + borde `$terracota` + mensaje inline) se conserva; su geometría no. Ver "As-built v7" arriba.
...
- **Mecánica del wheel picker** (R14.5): scroll vertical inercial con **Reanimated** (`Animated.ScrollView` + `useAnimatedScrollHandler` para el transform drum en el UI thread; el momentum/fling + `decelerationRate="fast"` los da el motor de scroll NATIVO del `ScrollView`, el más fiable en react-native-web) + **snap al valor** (`snapToInterval` = alto de una celda `$wheelCell=64`) + **tick háptico por valor** al cruzar cada celda. Rango **20–50 cm**, paso **0,5 cm** → 61 valores (`20,0; 20,5; …; 50,0`). Valor centrado = seleccionado; el confirm toma el valor centrado.
  - **AS-BUILT (háptica) — reconciliado:** el spec proponía `expo-haptics selectionAsync`, pero **el proyecto NO tiene `expo-haptics` en deps a propósito** (superficie de postinstall, ADR-011; ver cabecera de `app/src/utils/haptics.ts`). **[CORRECCIÓN 2026-08-06]** El motivo citado acá era FALSO: `expo-haptics` no tiene postinstall ni dependencias (verificado con `npm view expo-haptics scripts dependencies`) y **ADR-013 §Capa 4 ya lo listaba en el stack**. La dep está instalada desde esa fecha y la usa `services/ble/feedback.ts`. Estos consumidores (reorder / rueda) siguen con `Vibration` **por otra razón, que sí vale**: piden ticks de 8–18 ms sin semántica de éxito/error, que es lo que `Vibration` hace bien y `NotificationFeedbackType` no expresa. Migrarlos es una decisión de UX de estas pantallas, no un arrastre. Ver spec 04 R4.1. El tick se implementa con el idiom YA establecido del repo: `hapticTick()` en `haptics.ts` (módulo `Vibration` de RN, import perezoso web-safe, best-effort — degrada en silencio en web). Sin nueva dependencia. Si el dev build suma un canal háptico más rico, se enchufa ahí sin tocar el componente.
...
### 12.5 Delta de sync rules (`sync-streams/mitropero.yaml` + `schema.ts`) — frontera WAL (R14.16)
...
> **🔧 RECONCILIACIÓN AS-BUILT (fix-loop M7, 2026-06-20) — R13.30 RESUELTO a OPCIÓN B (MVP parcial); la Opción A queda como FAST-FOLLOW.** La 1ra pasada aplicó el split `buildCustomAttributesViewQuery` (sin filtro `deleted_at`, surface `deleted_at` → `isDeleted` display-only) esperando preservar el histórico. **El e2e cazó que NO alcanza:** la sync-stream **`est_field_definitions_custom`** (`sync-streams/mitropero.yaml` l.243) hace `WHERE establishment_id IN org_scope AND deleted_at IS NULL` → al soft-deletear, la fila de `field_definitions` se **PRUNEA del device** → el INNER JOIN del display (`custom_attributes JOIN field_definitions`) **no encuentra el `label`** → el valor histórico desaparece de la ficha. El §13.5 original asumió que la fila seguía local tras el soft-delete; la **frontera WAL lo contradice**. **Raf decidió la Opción B (2026-06-20):**
...
- **2026-06-13 — Fold de M5 — datos/maniobras custom — desde `context-m5-custom-maniobras.md`.** Se agregó la **§11 (delta backend de M5)** como **especificación de diseño** (no se aplicó DDL), cubriendo US-13 (R13.1–R13.24). **§1-§10 NO se tocaron** (el backend de M1-M4 sigue done; el SQL de §2/§4 intacto). Contenido de §11:
  ...
  - **§11.5** delta de sync rules sobre el archivo REAL `sync-streams/mitropero.yaml` (corrige el path viejo `app/powersync/sync-rules.yaml`, inexistente): `catalog_field_definitions` se restringe a `establishment_id IS NULL` + 3 streams custom nuevas scope establishment (frontera WAL R13.20/R13.21). Lo audita Gate 1.
