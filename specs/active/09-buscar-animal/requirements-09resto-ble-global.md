...
> **RECONCILIACIÓN as-built (bugfix 2026-07-29, reporte de Raf en device Android).** RB8.1 se acota: el chip se muestra **solo cuando hay un transporte instanciado** (`provider.transport != null`); sin transporte NO se renderiza. Motivo: el chip es a la vez indicador **y** atajo de conexión (RB8.3), y sin transporte su tap era un `connect()` no-op — un significante que promete una acción que no puede cumplir. Además, sin transporte el único `ConnectionStatus` alcanzable es `'off'` (el provider ni siquiera suscribe un `onStatus`), o sea que el chip no informaba nada variable: era chrome permanente en el header de una tab anunciando algo inusable en ese dispositivo, y **contradecía** al hero de `maniobra/identificar`, que dos elementos más abajo dice "El bastón no está disponible en este dispositivo". La condición es **"no hay transporte"**, NO "es Android": cuando la Fase 4 de spec 04 construya el adapter SPP, el chip reaparece solo sin tocar código. En web el transporte siempre existe (web-serial) → RB8.1 se cumple ahí sin cambios.
