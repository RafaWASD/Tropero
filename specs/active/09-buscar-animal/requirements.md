...
Spec 09 **consume** los primitivos de spec 02 (`animals`, `animal_profiles`, `animal_events`, `rodeos`, `categories_by_system`, `field_definitions` + `system_default_fields` + `rodeo_data_config` (plantilla de datos, ADR-021), `management_groups` (lote, ADR-020), función `animal_timeline()`, RLS, motor de form dinámico por rodeo). **No** redefine ni amplía el modelo de datos. Las únicas excepciones potenciales — donde aparece tensión con R4.13 de spec 02 — quedan documentadas como TODOs en R12 (no se resuelven en este spec).
...
- **Form dinámico por rodeo** — El motor vive en spec 02 (`design.md` § "Motor de form dinámico por rodeo"). **Distinción importante (ADR-021)**: los campos del *form de alta del animal* (sexo, raza, pelaje, peso de entrada, etc.) son columnas de `animal_profiles` y en MVP son los mismos para `(bovino, cría)` (hardcodeados). Esto es **distinto** de los *data_keys de eventos* que el rodeo tracquea (`rodeo_data_config` + `field_definitions`), que determinan qué **eventos/maniobras** se pueden cargar sobre el animal (consumidos al "+ agregar evento" en EDIT y por el gating de spec 03), no qué campos tiene el form de alta. Categoría inicial autocalculada (R4.7 de spec 02) mostrada read-only con opción de override manual.
- **Lote (management_group) en CREATE/EDIT** — La asignación de lote (ADR-020) está disponible tanto al crear como al editar un animal, consumiendo `management_groups` de spec 02 (R2.14..R2.17). Es **opcional** (un animal sin lote se agrupa por categoría, R2.16 de spec 02). La UI concreta del selector de lote es **TENTATIVA** (sujeta a design system); spec 09 solo declara que el flujo lo ofrece. Crear/renombrar/borrar lotes es owner-only y vive en la gestión de lotes de spec 02 (T4.5), no en el form de alta.
...
- **Terminología canónica** (ADR-016 + ADR-020) — "rodeo" + "sistema" en todo el spec para la unidad productiva. "Lote" (`management_group`, ADR-020) es un concepto **distinto y real**: es la agrupación de manejo (tercer eje), nunca un sinónimo de rodeo. En la UI el `management_group` se muestra como "Lote".
...
>
> **AS-BUILT (2026-06-13, chunk "BLE global"):** R2 está **IMPLEMENTADA** y el refinamiento incremental ya ocurrió — ver `requirements-09resto-ble-global.md` (RB1..RB9) + `design-09resto-ble-global.md` + Gate 0 `context-09resto-ble-global.md` (DEC-1..5). Deltas sobre el texto original de abajo: el overlay es **bottom-sheet** (no modal full-screen, DEC-1); BLE sin-match va **directo a CREATE** con el TAG precargado (el intermedio de dedup opción A se difiere al chunk siguiente, DEC-2); el caso "TAG activo en otro campo" ofrece **transferir con re-parenting** (spec 11, online-only, DEC-3) en vez del "alta en este campo" del R3.4 original; **live-rescan** (DEC-4); chip de conexión mínimo (DEC-5). R2.3 (suspender en MODO MANIOBRAS) queda cableado vía `useStickListenerControls` aunque spec 03 cliente no exista aún. Validado con mock (E2E `app/e2e/baston.spec.ts` 4/4) + renders vetados; el bastón físico real (`spp-android`) sigue gated por el dev build Android. **Para probar en web con hardware: `docs/test-baston-web.md`.** Quedan de spec 09: dedup opción A/B (R7/R8) y pairing pulido (R9).
...
**R4.5** La pantalla deberá renderizar los campos del form según el `system_id` del rodeo seleccionado, consumiendo el motor de form dinámico documentado en `design.md` de spec 02 § "Motor de form dinámico por rodeo". Para el MVP `(bovino, cría)`, los campos del form (columnas de `animal_profiles`, hardcodeados para cría) deberán incluir como mínimo: `species_id` (oculto/preseleccionado si solo hay uno activo), `sex` (radio macho/hembra, obligatorio), `birth_date` (date picker, opcional), `breed` (texto libre, opcional), `coat_color` (texto libre, opcional), `entry_date` (date picker, opcional), `entry_weight` (numérico, opcional), `entry_origin` (texto libre, opcional), y opcionalmente **lote** (`management_group_id`, selector de los `management_groups` activos del establishment o "sin lote" — ADR-020, R2.15 de spec 02). La categoría inicial se autocalcula según R4.7 de spec 02 y se muestra read-only con opción de override manual (R4.8 de spec 02). *(Nota: los campos de este form son atributos del animal; los **data_keys de eventos** que el rodeo tracquea —`rodeo_data_config`— no afectan este form, sino qué eventos se pueden cargar después.)*
...
- **Paso 1**: el sistema deberá pedir al usuario que elija el tipo de evento. Los tipos cargables desde acá son los 5 tipados de spec 02 R6.1..R6.5 (**peso** / **reproductivo** —servicio, tacto, parto, etc.— / **sanitario** —vacuna, tratamiento, etc.— / **condición corporal** / **muestra de lab**) + **observación libre** (`animal_events`, R6.10..R6.13). El label de cada tipo se define con el copy del design system. *(Los tipos del enum original del ADR-017 —`salud`, `reproduccion`, `traslado`, `pesaje`, `identificacion`— **no** aplican: el modelo Híbrido los reemplazó por las 5 tablas tipadas; ver Changelog de spec 02.)* En MVP los 5 tipados se cargan principalmente desde MODO MANIOBRAS (spec 03); la carga unitaria desde acá está permitida pero sujeta al gating de `rodeo_data_config` del rodeo del animal cuando spec 03 implemente la capa DB (un evento de tacto requiere que el rodeo tenga `prenez` habilitado).
...
**R6.4** Si la query de R6.3 retorna vacío (caso: usuario nuevo en el establishment, nunca tocó animales), el sistema deberá usar como default **el primer rodeo activo creado en el establishment** (`select id from rodeos where establishment_id = ? and active = true and deleted_at is null order by created_at asc limit 1`). Si el establishment no tiene ningún rodeo activo (caso: owner aún no completó el wizard de "Crear rodeo" de R2.6 de spec 02 — recordar que **no hay rodeo autogenerado** desde el refinamiento 2026-05-27 de spec 02), el sistema deberá bloquear el flujo de alta con un mensaje accionable ("Creá un rodeo antes de cargar animales") que lleve al usuario al wizard de creación de rodeo. La R6 nunca debe asumir que existe un rodeo "principal" autogenerado.
...
>
> **AS-BUILT (2026-06-13, chunk "dedup A/B" — FINAL, Runs 1-4 done):** la opción A se aterriza en `requirements-09resto-dedup.md` (RD3/RD4/RD8) + `design-09resto-dedup.md` + Gate 0 `context-09resto-dedup.md` (DEC-1..3, D-a..D-d). Deltas sobre el texto de abajo: la intermedia NO es una pantalla aparte (`AssignTagSearchScreen`) sino un **nuevo modo del bottom-sheet** `FindOrCreateOverlay` (`assign_or_create`, DEC-3) con lista de candidatos `noTag` (`updated_at DESC`, R7.2) + **buscador** (`searchAnimals` scopeado a `noTag`, D-a) + CTA "Es un animal nuevo → dar de alta" (R7.5); si hay **0 candidatos**, salta directo a CREATE (la intermedia vacía no aporta, RD3.2). La asignación (R7.4) NO la hace el cliente con un UPDATE directo: la hace el **RPC `assign_tag_to_animal` SECURITY DEFINER** (migración **0089**, deployada) invocado **offline** vía outbox + RPC-mapping de spec 15 (DEC-2) — `animals` está fuera del sync set (ADR-026 b1); el guard `IS NULL` (R12.2) vive dentro del RPC; idempotencia **state-based** (replay = TAG ya aplicado → `replay:true`, **RATIFICADA por Gate 1**, sin columna/tabla de audit nueva); authz = **cualquier rol activo** (D-d, **RATIFICADA por Gate 1**). Al asignar offline, la ficha del animal puede mostrar "sin caravana" hasta el próximo sync + re-focus (lectura LOCAL no reactiva, staleness documentada — el E2E lo verifica con un oráculo server, no en la ficha). **Gate 1 PASS 0 HIGH** (`security_spec_09resto-dedup.md`) + Gate 2 por run; deploy aplicado por el leader (gated). E2E formal: `app/e2e/baston-dedup.spec.ts` escenarios (a) asignar a candidato → ficha + caravana puesta (oráculo server), (b) "es nuevo" → CREATE precargado, (a') 0 candidatos → CREATE directo.
...
>
> **AS-BUILT (2026-06-13, chunk "dedup A/B" — FINAL, Runs 1-4 done):** la opción B se aterriza en `requirements-09resto-dedup.md` (RD5/RD6/RD7) + `design-09resto-dedup.md` + Gate 0 `context-09resto-dedup.md`. La `BulkTagAssignmentScreen` (`app/app/asignar-caravanas.tsx`) consume **su propio** listener BLE en modo asignación, una **cola de sesión** (`useReducer`), asignación **1×1** + **contador** visible (R8.5), entry points tab Animales (filtro `noTag` + CTA "Asignar caravanas en masa") + tab Más (D-c, todos los roles), y CTA "Bastoneé un animal nuevo, no está en la lista" → CREATE con el EID precargado (R8.6). La asignación (R8.3) usa el mismo **RPC `assign_tag_to_animal`** offline (outbox + RPC-mapping, DEC-2) que la opción A; el candidato sale de la lista de sesión **client-side** (`excludedProfileIds`; el denorm baja por la stream al sincronizar). **CORRECCIÓN del plan**: el anti-stacking con el overlay global NO usa `useBusyWhileMounted` (era **inviable** — el provider de spec 04 gatea `listening = enabled && !busy` ANTES de entregar a CUALQUIER suscriptor, así que `busy=true` mataría también el listener propio de la masiva); se resuelve haciendo el `FindOrCreateOverlay` global **route-aware** (`useSegments()` → en la ruta `asignar-caravanas` el overlay no abre nada), sin tocar `ble/*` (gateado). Ver `design-09resto-dedup.md` §4.5. **CORRECCIÓN de R8.4 (dup-TAG)**: el dup NO se surfacea "por el canal de status existente al sincronizar" (ese canal as-built, `connector.ts::surfaceUploadRejection`, es solo `console.warn` sin copy user-facing, y `assign_tag_to_animal` no tiene overlay → un rechazo al sync es invisible; inventar un canal nuevo está prohibido, RD6.3). La defensa as-built es **prevención CLIENT-SIDE al bastonear** (RD6.1): `onTagRead` corre `lookupByTag` ANTES de encolar → un EID ya asignado (`mode:'edit'`/`'transfer'`) muestra el `DupNoticeBanner` ("Esa caravana ya está asignada") **sin encolar**, sin perder la sesión; el residual que igual rebote al sync (race auto-sanante / dup cross-tenant casi imposible) lo maneja la maquinaria existente (`permanent_reject` → descarte + log) — **LIM documentada** (RD6.2/RD6.3). Todo offline-first. **Gate 1 PASS** (RPC) + deploy aplicado (gated). E2E formal: `app/e2e/baston-dedup.spec.ts` escenarios (c) masiva 2 bastoneos → contador en 2 + ambos candidatos salen de la lista, (d) EID ya asignado NO encola + avisa (RD6.1).
...
>
> **AS-BUILT (2026-06-13, chunk "dedup A/B" — FINAL, Runs 1-4 done):** la compatibilidad R4.13.a (`NULL → valor`) se materializa en el RPC `assign_tag_to_animal` (migración **0089**, `requirements-09resto-dedup.md` RD1, `design-09resto-dedup.md` §1). El UPDATE `NULL→valor` corre **server-side** dentro del RPC SECURITY DEFINER (no un UPDATE de cliente directo — `animals` está fuera del sync set, ADR-026 b1) con el guard `WHERE tag_electronic IS NULL` (R12.2) **además** del trigger `tg_animals_block_tag_change` de 0036 (que permite NULL→valor, bloquea valor→valor 23514). 0 filas afectadas = race (otro device puso la caravana) → `23514` accionable. La identidad propaga a `animal_profiles.animal_tag_electronic` por el trigger 0079 → la UI lee el denorm offline. La idempotencia del replay (outbox at-least-once) se resuelve **por estado ya aplicado** (`animals.tag_electronic` ya = el TAG → no-op `replay:true`, scopeado al animal derivado, sin oráculo cross-tenant) — **RATIFICADA por Gate 1** (DA-1); NO se agregó columna `last_assign_op_id` ni tabla de audit. El audit granular de R12.4 sigue **diferido** (el `updated_at` cubre el MVP). Los tests DB de R12.3 (los 3 casos NULL→valor / valor→valor / valor→NULL) viven en spec 02; el chunk agrega su propia suite del RPC (6 escenarios + extra, `supabase/tests/animal/run.cjs`, verde contra el RPC deployado). El E2E formal verifica la cadena completa offline→sync→RPC→propagación 0079 con un oráculo server (`waitForServerTagAssigned`).
