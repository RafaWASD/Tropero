...
> **RECONCILIACIÓN as-built (bugfix 2026-07-29 — sin transporte en el dispositivo).** RD5.1/RD5.2 se completan con el caso que ninguno de los dos contemplaba: **un dispositivo donde el bastón no se puede instanciar** (native manual-first hoy; el adapter SPP es Fase 4 de spec 04). Como esta pantalla NO tiene entrada manual, ahí la cola nunca se llena y el estado vacío es el único alcanzable — pedía *"Bastoneá para empezar"*, o sea lo único imposible en ese dispositivo. As-built: con transporte, RD5.2 se cumple sin cambios; **sin transporte el estado vacío informa que el bastón no está disponible en ese dispositivo y apunta a la carga de a una desde la ficha del animal**. RD5.1 NO se acota: los dos entry points siguen visibles siempre (la funcionalidad existe y funciona con bastón; ocultarlos la volvería indescubrible). Fundamento completo y alternativa descartada en `design-09resto-dedup.md` §4.6.
