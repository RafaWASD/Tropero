...
- [x] **T7.3 (bugfix 2026-07-29)** — Sin transporte instanciado, el chip NO se renderiza. El "no-op en native" de T7.2 era el bug que Raf reportó en device (*"el botón de conectar bastón en android no me está funcionando"*): el chip decía "Conectar bastón" y el tap no hacía nada. Cubre: **RB8.1 (reconciliado), RB8.3**.
  - _Aceptación_: en un dispositivo sin transporte el chip no existe; con transporte queda idéntico. **[✓]** Decisión en la función PURA `bleConnectionView(status, { hasTransport })` → `null` sin transporte (corte ANTES del switch). Vista pasada a type-only (ícono como clave + `CHIP_ICONS` en el componente) para poder testearla en `node:test`. `identificar.tsx` pasa `right={conectable ? <BleConnectionChip/> : undefined}` (el slot del header envuelve al hijo en un View → un chip nulo dejaría un flex item vacío + el gap `$2` de sobra). Unit `ble-connection-view.test.ts` (5) + E2E `baston-chip.spec.ts` (2), **falsificados en las dos direcciones** (quitar el guard → (a) rojo; ocultar siempre → (b) rojo). Capture Gate 2.5: `e2e/captures/baston-chip-sin-transporte.capture.ts` (12 shots, 2 pasadas).

## T8 — E2E Playwright con MockAdapter (4 escenarios §9 Gate 0)

- [x] **T8.1** — Wiring de test: exponer un handle (`window.__mitroperoBle.tagRead(eid)` / `connectMock()`) bajo flag de E2E que enchufe al `MockAdapter` montado en el provider de la raíz (mode `mock` en E2E). Mantenerlo FUERA de la superficie de producción (solo bajo flag — Gate 2 lo revisa). Cubre: **§7.2 design**.
  - _Aceptación_: desde Playwright se puede inyectar un EID y una transición de conexión al provider de la raíz. **[Run 2 ✓]** Flag = marca DELIBERADA `window.__MITROPERO_BLE_E2E__` (Playwright la pone vía `addInitScript` ANTES del bundle; en prod NUNCA se setea). `ble-e2e-flag.ts::isBleE2E()` → `mode='mock'` + monta `BleE2EBridge` (lee `api.transport` que en mock ES un `MockAdapter` → publica `window.__mitroperoBle.{tagRead,connectMock,disconnectMock}`). Doble guard en el bridge. Sin la marca: ni mock ni handle (Gate 2). Vive en `_components/` (NO en `services/ble/`, que es firma de spec 04).
