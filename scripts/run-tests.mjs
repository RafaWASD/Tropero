...
run(
  ...
  //
  // ⚠️ `app/src/utils/scan-coverage.test.ts` + `app/src/services/ble/error-text.test.ts` +
  // `app/src/services/ble/log-device-identifier-guard.test.ts` son el follow-up del Gate 2 del mismo
  // delta (2026-08-17, §7.1 y §7.2):
  //   · `scan-coverage.test.ts` es EL FRENO de la allowlist del meta-guard, que se estrenó con su
  //     primera entrada. Sin freno, una allowlist se vuelve la salida de emergencia de todos y el guard
  //     que la tiene deja de significar algo (verde por no estar mirando, una capa más arriba). Exige
  //     motivo escrito EN EL LUGAR, tope, que el archivo exista, que la exención esté GANADA contra el
  //     árbol real —si sobra, rojo— y que la puerta sea UNA. Y demuestra, en las dos direcciones, que
  //     con la entrada puesta el mutante de dos líneas ya no voltea a los nueve guards.
  //   · `error-text.test.ts` vigila el convertidor único error→texto. Su test principal DERIVA de
  //     `node_modules/react-native-ble-plx` la lista de códigos cuya plantilla interpola `{deviceID}`:
  //     un upgrade de la lib que agregue uno se pone rojo ANTES de que el id del bastón empiece a viajar
  //     a los breadcrumbs. Una tabla copiada a mano es el bug del union `RejectReason`, otra vez.
  //   · `log-device-identifier-guard.test.ts` es el guard sobre LA AUSENCIA: enumera las superficies que
  //     loguean y exige que ninguna lea el texto de un error por su cuenta ni interpole un identificador
  //     de dispositivo en un `message`. Arreglar los tres call sites no cerraba nada — la forma se
  //     re-escribe sola en el próximo adapter (MFi todavía no tiene módulo nativo).
