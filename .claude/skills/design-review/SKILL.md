...
### Contenedores con contenido dinámico, sheets y formularios (bugs recurrentes — vetar SIEMPRE)
Estos tres se le escaparon al leader en sesión 26 (CustomFieldSheet) y Raf los marcó como UX básico que NO se debe olvidar. Son distintos del recorte de descendentes (ese es tipográfico; estos son de **layout que se mueve cuando el contenido crece**).

- **Títulos que se recortan al CRECER el contenido (no solo por descendentes)**: cuando un contenedor tiene alto ACOTADO (un sheet con `maxHeight` 90%, un modal, una card con tope) y adentro hay contenido que **crece dinámicamente** (un banner de error que aparece, una lista que se llena, un campo que se expande, el teclado que sube), ese crecimiento puede **empujar el header/título fuera del área visible o taparlo**. El bug clásico (CustomFieldSheet): el `FormError` apareció, creció el contenido por encima del `maxHeight`, y **recortó el título de arriba**. **Barrer TODOS los títulos** imaginando el peor caso de contenido (error visible + lista larga + teclado abierto), no solo el estado vacío feliz.
- **Patrón canónico de sheet/modal con tope de alto**: **header FIJO** (`flexShrink: 0`, nunca se comprime ni se va) + **body SCROLLEABLE** (`ScrollView` con `flex: 1` que absorbe el crecimiento) + **footer FIJO** (CTA siempre visible y alcanzable). Así el contenido que crece scrollea DENTRO del body y jamás come el título ni el botón primario. Si el body no tiene `flex: 1`, el contenido lo empuja y rompe el tope → recorte. Vetar que todo sheet con contenido variable use este esqueleto.
- **Validación de formulario = scroll-al-campo + borde rojo + error inline** (Nielsen #9, "reconocer/recuperar de errores"): cuando una validación falla, NO mostrar un banner de error global arriba que **tape el título** ni dejar al operario adivinando qué campo arreglar. El patrón correcto: (a) **scrollear automáticamente al campo** ofensor (medir su geometría con `onLayout` si el error crece el contenido), (b) **borde rojo** (`$terracota`) en ese campo, (c) **mensaje inline** pegado al campo (no flotando lejos). Manga: el operario tiene que saber QUÉ tocar de un vistazo, sin leer un párrafo arriba.
...
- [ ] **Título no se recorta al CRECER el contenido (vetar SIEMPRE)**: en sheets/modales/cards con alto acotado, imaginar el PEOR caso (error visible + lista llena + teclado abierto) y verificar que el header/título nunca queda tapado ni empujado fuera. Distinto del recorte de descendentes.
- [ ] **Sheet con contenido variable = header fijo (`flexShrink:0`) + body `ScrollView flex:1` + footer fijo**: el contenido que crece scrollea dentro del body, nunca come el título ni el CTA.
- [ ] **Validación de formulario = scroll-al-campo + borde rojo (`$terracota`) + error inline** (no un banner global que tape el título; el operario tiene que saber QUÉ campo tocar de un vistazo).
