...
**Safe areas**: respetar insets siempre — patrón `paddingBottom = max(insets.bottom, $navBottomMin)`. Nada tocable/importante bajo el home indicator (iOS ≈34px) ni la gesture bar (Android). **Blindaje frame-0 Android edge-to-edge (U7/U2)**: `useSafeAreaInsets().bottom` puede reportar `0` en los primeros frames (el `SafeAreaProvider` raíz no está sembrado con `initialWindowMetrics`); por eso el patrón robusto es `max(insetVigente, initialWindowMetrics?.insets.bottom ?? 0, $navBottomMin)` — codificado en `computeTabBarInsetLayout` (bottom-nav) y `computeSafeBottomInset` (`FooterActionShell`). Con el **teclado abierto**, la safe-area la tapa el teclado → un footer fijo NO debe reservarla (dejaría un hueco): usa un respiro chico (`resolveFooterPaddingBottom`).
