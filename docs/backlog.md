...
**✅ ACOTADO (2026-07-21, feature 21 `21-watched-queries`)**: los 3 consumidores de la feature 20 (`EstablishmentContext`, `RodeoContext`, `lotes.tsx`) ya están MIGRADOS a watched queries reales — los 2 contextos con **`db.onChange(handler, { tables })`** (la primitiva que NOTIFICA el cambio de tabla y re-corre la resolución existente; nombre verificado en `node_modules`, NO `db.watch(sql,…)`) y `lotes.tsx` con **`useQuery`**. El disparador `lastSyncedMs` (el proxy no determinista) se eliminó de esos 3 archivos; la reactividad ahora es determinista (~1,5 s, medido: aviso de revocación a <300 ms del disparo) y la E2E `reactividad-sync.spec.ts` pasó a determinista SIN retries ni forzador (D3). Patrón + plan de migración incremental en **ADR-030**. **QUEDA PENDIENTE**: migrar el RESTO de la app (las 5 pantallas focus-only `miembros`/`use-reports`/`animal/[id]`/`export-sigsa`/`maniobra` + el resto), con el patrón ya establecido — el `lastSyncedMs`/`useStatus` sigue siendo el disparador ahí. Entrada original abajo por trazabilidad.
...
**Qué**: el helper `app/src/utils/nav.ts` `backOr(router, fallback)` (canGoBack ? back : replace(fallback)) se aplicó SOLO a las 3 pantallas del flujo (`agregar-evento`, `animal/[id]`, `crear-animal`). Quedan `router.back()` "pelados" en: `cambiar-email`, `editar-plantilla`, `editar-campo`, ~~`invite`~~ (✅ hecho, ver abajo), `crear-campo`, `crear-rodeo` (back condicional), `maniobra` (modal), `invitar`, `miembros`, `rodeos`.
**✅ PARCIAL — `invite` HECHO (2026-07-22, bugfixes invite — `progress/impl_invite-fixes.md`)**: el "Cancelar" de la fase `paste` de `invite.tsx` pasó de `router.back()` pelado a `backOr(router, '/(tabs)')` (fallback = home, el destino lógico tras un invite abierto en frío). Es prereq rebrand-safe de deep-links/U8a (cold-start/deep-link = stack vacío). El resto de las pantallas de la lista SIGUE pendiente (barrido no urgente).
