...
**Nota 2026-08-16 (rebrand fase 2) — el sweep necesita TRES dominios, no uno.** El namespace de la suite E2E (`app/e2e`) pasó de `@rafaq-e2e.test` a **`@mitropero-e2e.test`**; el de las suites backend (`supabase/tests/*/run.cjs` + `scripts/seed-facundina.mjs`) sigue en **`@rafaq-test.local`** (se renombra en su propia pasada). O sea que hoy conviven en el remoto: `@rafaq-test.local` (backend, vivo), `@mitropero-e2e.test` (E2E, vivo) y **`@rafaq-e2e.test` (E2E, residuo histórico de antes del rebrand)**. Un purgado que enumere sólo el primero —como decía esta entrada— deja los otros dos adentro. El teardown de la E2E NO barre por dominio (borra por ids trackeados + prefijo `RUN_TAG` en el nombre del establishment), así que el rename no rompió ningún barrido: sólo agrandó la lista que este sweep manual tiene que cubrir.
...
- **Corto**: purgar la data de e2e del remoto (prefijos `animal_test_` / `bantest_` + los TRES dominios de fixtures: `@rafaq-test.local` (backend, vivo), `@mitropero-e2e.test` (E2E, vivo desde el rebrand fase 2) y `@rafaq-e2e.test` (E2E, residuo pre-rebrand) + los establecimientos/roles de test) — coordinar con la entrada 2026-06-05; incluir la limpieza de los **roles espurios de Raf**.
