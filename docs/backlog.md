...
**Origen**: fix-loop del chunk M7 de spec 03 (gestión de datos custom). El reviewer + el e2e cazaron que R13.30 ("la ficha sigue mostrando el valor de un dato custom borrado") NO se honra end-to-end: la sync-stream `est_field_definitions_custom` (`sync-streams/mitropero.yaml` l.243) filtra `deleted_at IS NULL` → al soft-deletear, la definición se prunea del device → el INNER JOIN del display no resuelve el `label` → el valor histórico desaparece. **Raf eligió la Opción B (MVP)**: no se cambia la stream; el cliente lo asume y la confirmación de borrado ADVIERTE que las cargas previas dejarán de verse (R13.30/R13.31 reconciliados al as-built).
...
**Próximo paso sugerido**: antes del beta real, purgar la data de e2e del remoto (identificable por el prefijo `animal_test_` / los CUATRO dominios de fixtures de la nota 2026-08-17 / `bantest_` etc.) con un script de limpieza cuidadoso (respetando FKs: events → profiles → animals → users). Después, opcionalmente, `VALIDATE CONSTRAINT` de los 2 tags + bajar el tope a 32. Coordinar con la suite e2e (que debe limpiar lo suyo; ver si el cleanup de los helpers está fallando y dejando residuo).
**Nota 2026-06-10 (Gate 2 T7, LOW-2)**: la suite nueva `sync_streams` limpia por ids trackeados, pero ante un kill duro puede dejar huérfanos namespaced (hoy `@mitropero-test.local`, antes `@rafaq-test.local`) — el sweep de esta entrada los cubre; aplica a todas las suites contra remoto.
**Nota 2026-08-17 (rebrand, dominio de fixtures) — el sweep necesita CUATRO dominios, no uno ni tres.** Los dos namespaces de fixture se rebrandearon en pasadas distintas y **nadie migra los usuarios ya creados**, así que en el remoto conviven cuatro:

**Magnitud MEDIDA** (2026-08-17, `auth.admin.listUsers` paginado sobre DEV, read-only): **452 usuarios de auth en total, de los cuales 443 son basura de test (98%)**. Sólo 9 son cuentas reales (8 `gmail.com` + 1 `privaterelay.appleid.com`).

| Dominio | Usuarios hoy | Último creado | Estado | Quién lo crea/creó |
|---|---:|---|---|---|
| `@rafaq-test.local` | **410** | 2026-08-11 | **residuo** | las 15 suites backend, antes del 2026-08-17 |
| `@rafaq-e2e.test` | **33** | 2026-08-09 | **residuo** | la suite E2E, antes de la fase 2 |
| `@mitropero-test.local` | 0 | — | **vivo** | las 15 suites backend (`supabase/tests/*/run.cjs`) + `scripts/seed-facundina.mjs --bootstrap`, desde 2026-08-17 |
| `@mitropero-e2e.test` | 0 | — | **vivo** | la suite E2E (`app/e2e`), desde el rebrand fase 2 (2026-08-16) |

Los dos vivos están en 0 porque **su cleanup funciona**: verificado en vuelo el 2026-08-17 (una corrida de la suite `rls` creó 9 usuarios `@mitropero-test.local`, observados con el poller, y el total volvió a 452 al terminar). O sea que los 443 residuos **no** son "cada corrida deja basura": son el acumulado histórico de corridas interrumpidas antes del subtest `cleanup` (Ctrl+C / timeout), más lo que se creó antes de que hubiera cleanup.

Un purgado que enumere sólo el primero —como decía esta entrada originalmente— deja los otros tres adentro. **Ningún barrido automático mira el dominio** (verificado en las dos puntas: el teardown de la E2E borra por ids trackeados + `establishments.name LIKE '<RUN_TAG>%'`, y las suites backend por ids trackeados; `git grep -nE "like\(.email|ilike\(.email|listUsers"` sobre `supabase/tests` da cero). O sea que los renames no rompieron ningún barrido: sólo agrandaron la lista que este sweep MANUAL tiene que cubrir. Los residuos son huérfanos de corridas interrumpidas (Ctrl+C / timeout antes del cleanup).
...
- **Corto**: purgar la data de e2e del remoto (prefijos `animal_test_` / `bantest_` + los CUATRO dominios de fixtures: `@mitropero-test.local` (backend, vivo desde 2026-08-17), `@mitropero-e2e.test` (E2E, vivo desde el rebrand fase 2), `@rafaq-test.local` (backend, residuo pre-2026-08-17) y `@rafaq-e2e.test` (E2E, residuo pre-fase 2) + los establecimientos marcados `SEED-TEST` del `--bootstrap` de `seed-facundina` + los establecimientos/roles de test) — la tabla completa está en la nota 2026-08-17 de la entrada 2026-06-05; incluir la limpieza de los **roles espurios de Raf**.
...
- ✅ **Las CUATRO puntas del repo** apuntan al dominio nuevo: `INVITE_BASE_URL`, los defaults de `invite_user` y `resend_invitation`, y **el string del botón "Copiar el link" de la página publicada** (`docs/marketing/landing-proximamente/invite.html`), que es literalmente lo que el invitado copia y pega. Una regla de `brand-name-guard.test.ts` las compara entre sí **y contra el `<link rel="canonical">` del sitio publicado**, para que no puedan moverse todas juntas a un dominio que nadie compró — que es la forma exacta que tuvo este bug.
- ✅ **La quinta punta, resuelta.** Se verificó que el secret `APP_URL` **no existe** ni en DEV ni en PROD, así que nada pisaba el default. Se decidió **dejarlo sin setear**: es la única punta que ningún test puede ver, y crearla sería volver a meter el punto ciego. Ver la nota en `specs/active/01-identity-multitenancy/design.md`.
- ✅ **`invite_user` v9 y `resend_invitation` v8 desplegadas en DEV** (11/08/2026). Verificado bajando el cuerpo de las funciones desde la API: el código que quedó arriba dice `mitropero.com.ar` y **cero** ocurrencias del dominio muerto. En PROD sólo está `health`; estas dos nunca se desplegaron ahí, así que no había nada que actualizar.
- ⛔ **En DEV el link ya sale bien. Falta probarlo con una invitación real de punta a punta.**
