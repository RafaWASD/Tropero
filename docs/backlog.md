...
**Residuales conocidos que esto cerraría** (hoy fallan en E2E, PRE-EXISTENTES al fix — verificados contra baseline sin el fix):
- `animals.spec.ts:52` (tail, línea 107): tras un alta OFFLINE, el stepper del Inicio "Cargá tu primer animal" no pasa a "Cargaste" de forma confiable (race overlay-clear ↔ llegada de la fila sincronizada; el conteo local es correcto, es timing de re-render). El cuerpo del test (el animal aparece en la LISTA) SÍ pasa con el fix.
- `animals.spec.ts:500` (C3.3 baja): tras una baja OFFLINE, la ficha no muestra "Vendido el {fecha}" — el overlay de exit (`pending_status_overrides`) marca `status` pero NO `exit_date`, así el badge sale "Vendido" sin fecha → el regex `/Vendido el /` no matchea. Es un gap del overlay de exit (T6), no del fix de gate/lectura. Fix puntual posible: que el overlay de exit lleve `exit_date` (cliente) hasta que la RPC corra.
- `establishments.spec.ts:29` (crear campo desde onboarding → bloqueo de rodeo): `refreshEstablishments(newId)` lee el SQLite local ANTES de que el campo recién creado sincronice DOWN → el set viejo no incluye el campo nuevo → no transiciona a `active`/rodeo-block. Es el swap de lectura T3 + el caso "post-first-sync incremental" que el fix DEFIRIÓ explícitamente (reactividad ante altas propias post-sync = `useFocusEffect`/refresh manual). `useQuery` o un await del sync-down del campo nuevo lo cerraría.
**Próximo paso sugerido**: spec/ADR de migración del data layer de campo a `useQuery` watchable (post-estabilización del overlay), + fix puntual del `exit_date` en el overlay de exit. No bloqueante: el camino CRÍTICO (aterrizar en home con datos, lista poblada) anda.
