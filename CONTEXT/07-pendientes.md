...
### 🟡 ABIERTO — Decisiones de spec 08 (export SIGSA) para Facundo (2026-06-13)
Surgidas al pre-resolver la Puerta 1 de spec 08 (Raf cerró 3 de 4 decisiones; estas esperan a Facundo):
- **Raza desconocida: ¿`OR` o `S/E`?** Cuando el productor no sabe la raza exacta, ¿qué código espera SENASA en la **declaración de dispositivos**: `OR` (Otra Raza, bovina) o `S/E` (Sin Especificar, genérica)? Ambos son códigos oficiales. Decisión interina de Raf: el BreedPicker bovino ofrece `OR` de un tap (soft-block si no hay raza). Falta que Facundo confirme `OR` vs `S/E`. (Relacionado con el bullet "Razas para el catálogo SENASA" de arriba.)
- **RENSPA: ¿1:1 estricto con un campo, o se puede repetir/transferir?** ¿Un RENSPA puede aparecer legítimamente en dos campos/personas distintas (ej. **venta del campo** → nuevo dueño lo registra), o es estrictamente único e intransferible por establecimiento físico? Define si en la DB el RENSPA es único global / único por dueño / texto libre opcional. Hoy el diseño de spec 08 lo hace único **GLOBAL** (bloquearía usos legítimos cross-tenant con error confuso; el RENSPA ni va en el TXT, es solo recordatorio en pantalla). **Recomendación del leader: relajar.** Bloquea la aplicación de la migration 0092 de spec 08 hasta cerrarla.
- **Gate duro de formato — upload real a SIGSA (lo hace Facundo)**: subir un archivo de prueba (2-3 animales) a SIGSA web con clave fiscal y reportar: (1) ¿`;` al final del último registro?, (2) ¿espacios?, (3) ¿restricción de rango de fechas de nacimiento?, (4) validación exacta del RFID (¿ceros a la izquierda?, ¿checkdigit ISO?), (5) ¿acepta mayúsc/minúsc mixtas? **Gatea el `done` de feature 08.** ⚠ Sacar la tabla de razas desde la pantalla de **DECLARACIÓN DE DISPOSITIVOS**, no la de movimiento (trampa del código `SI` = San Ignacio en dispositivos vs Simmental en movimiento; ver `specs/active/08-export-sigsa/razas-senasa-codigos.md` s25).

### 🟡 ABIERTO — Aplicabilidad de maniobras por categoría/sexo (spec 03, 2026-06-16)
Surgió al cablear la carga rápida de MODO MANIOBRAS (Raf cazó que **pesaje** y **pesaje de ternero** se aplicaban juntos al mismo animal, y que **tacto** se ofrecía a machos). Ya cerrado e implementado (`app/src/utils/maneuver-applicability.ts`): `tacto`/`tacto_vaquillona` → solo hembras; `pesaje_ternero` → solo ternero/ternera y `pesaje` → no-ternero (mutuamente excluyentes); `raspado` → machos. Falta cerrar con Facundo la **matriz fina** (qué maniobra se excluye de qué categoría):

- **Tacto aptitud (vaquillonas) vs tacto de preñez — el caso difícil.** La categoría ya distingue casi todo: una `vaquillona` (≥1 año, sin servicio, sin tacto+) → **aptitud reproductiva**; con un tacto+ pasa a `vaquillona_prenada`, y las paridas (`vaca_segundo_servicio`/`multipara`) → **preñez**. El hueco: una vaquillona **servida pero todavía no diagnosticada** sigue siendo categoría `vaquillona` → por categoría es indistinguible de una pre-servicio. **Pregunta:** ¿las vaquillonas de **reposición** (pre-servicio → aptitud) y las **entoradas/servidas** (→ preñez) se manejan en **lotes/rodeos SEPARADOS**? Si SÍ (lo normal en cría), el gating por rodeo (el owner habilita `tacto_vaquillona` en el rodeo de reposición y `prenez` en el de entore) + el filtro por categoría/sexo lo resuelve **sin preguntarle nada al operario**. Si conviven en el mismo rodeo, haría falta un marcador de "entorada" (más data model). → Define si alcanza con el modelo actual o hace falta el marcador.
- **Raspado:** hoy aplica a **todos los machos**. ¿Debería ser **solo toros/toritos** (un novillo castrado no se raspa)?
- **Sangrado (brucelosis):** ¿a qué categorías aplica? ¿Solo hembras adultas? ¿Hay corte de edad (p. ej. >18 meses)? ¿Machos también?
- **Dientes:** ¿excluir terneros/as de la maniobra de dientes? (hoy aplica a todos; solo el **prompt CUT** excluye terneros, R6.8). Un ternero no se "boquea" para envejecer.
- **Condición corporal:** ¿excluir terneros/as, o aplica a todos?
